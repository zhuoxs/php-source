...
!function(t, e) {
    ...
    return d.prototype.init = function() {
        ...
        var p = this;
        p.src && (p.__oneTouchStart = function(t) {
            p.touchX0 = Math.round(t.x), p.touchY0 = Math.round(t.y);
        }, p.__oneTouchMove = function(t) {
            var e, o;
            if (p.touchended) return p.updateCanvas();
            e = Math.round(t.x - p.touchX0), o = Math.round(t.y - p.touchY0);
            var n = Math.round(p.rectX + e), r = Math.round(p.rectY + o);
            p.outsideBound(n, r), p.updateCanvas();
        }, p.__twoTouchStart = function(t, e) {
            var o, n, r;
            p.touchX1 = Math.round(p.rectX + p.scaleWidth / 2), p.touchY1 = Math.round(p.rectY + p.scaleHeight / 2), 
            o = Math.round(e.x - t.x), n = Math.round(e.y - t.y), r = Math.round(Math.sqrt(o * o + n * n)), 
            p.oldDistance = r;
        }, p.__twoTouchMove = function(t, e) {
            var o, n, r, a, i, c, u, d = p.oldScale, s = p.oldDistance, h = p.scale, f = p.zoom;
            p.newScale = (o = d, n = s, r = f, a = t, i = e, c = Math.round(i.x - a.x), u = Math.round(i.y - a.y), 
            o + .001 * r * (Math.round(Math.sqrt(c * c + u * u)) - n)), p.newScale <= 1 && (p.newScale = 1), 
            p.newScale >= h && (p.newScale = h), p.scaleWidth = Math.round(p.newScale * p.baseWidth), 
            p.scaleHeight = Math.round(p.newScale * p.baseHeight);
            var l = Math.round(p.touchX1 - p.scaleWidth / 2), g = Math.round(p.touchY1 - p.scaleHeight / 2);
            p.outsideBound(l, g), p.updateCanvas();
        }, p.__xtouchEnd = function() {
            p.oldScale = p.newScale, p.rectX = p.imgLeft, p.rectY = p.imgTop;
        });
    }, d;
});