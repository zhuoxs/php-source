...
class Alipay implements GatewayApplicationInterface
{
    ...
    public function pay($gateway, $params = []){
        if(!empty($params['request_return'])){
            $request_return = $params['request_return'];
            unset($params['request_return']);
        }
        ...
        if (class_exists($gateway)) {
            return $this->makePay($gateway,$request_return);
        }
     ...
     * @return Response|Collection
     */
    protected function makePay($gateway,$request_return)
    {
        $app = new $gateway();
        if ($app instanceof GatewayInterface) {
            if($request_return > 0){
                $this->payload['request_return'] = $request_return;
            }
