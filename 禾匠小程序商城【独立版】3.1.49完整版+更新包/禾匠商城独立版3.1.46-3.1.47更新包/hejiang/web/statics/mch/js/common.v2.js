...
$(document).on('click', '.upload-group .upload-file', function () {
    var btn = $(this);
    if (btn.hasClass('upload-disabled')) {
        return;
    }
    ...
    $.upload_file({
        ...
        start: function () {
            $('.upload-group .upload-file').addClass('upload-disabled');
            $('.upload-group .upload-file').attr('data-original-title', '上传中，请稍等');
            btn.btnLoading(btn.text());
        },
        complete: function () {
            $('.upload-group .upload-file').removeClass('upload-disabled');
            $('.upload-group .upload-file').attr('data-original-title', '上传文件');
        },
        success: function (res) {
            btn.btnReset();
            if (res.code === 1) {
