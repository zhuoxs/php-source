...
$info=pdo_getall('yzfc_sun_newsclassify',array('uniacid'=>$_W['uniacid']));
foreach ($info as $key =>$value){
    $info[$key]['createtime']=date('Y-m-d H:i:s',$value['createtime']);
}
if($_GPC['op']=='delete'){
    $isnews=pdo_getall('yzfc_sun_news',array('cid'=>$_GPC['id']));
    if($isnews){
        message('删除失败,请先到新闻列表修改新闻分类','','error');
    }else{
        $res=pdo_delete('yzfc_sun_newsclassify',array('id'=>$_GPC['id']));
        if($res){
