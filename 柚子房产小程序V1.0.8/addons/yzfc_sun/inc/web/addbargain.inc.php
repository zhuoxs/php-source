...
if($_GPC['id']){
    $info=pdo_get('yzfc_sun_course',array('id'=>$_GPC['id']));
    if($info['cid']>0){
        $nowtype=pdo_get('yzfc_sun_courseclassify',array('id'=>$info['cid']));
    }
    if($info['tid']>0){
        $nowteacher=pdo_get('yzfc_sun_teacher',array('id'=>$info['tid']),array('id','name'));
    }
...
$class=pdo_getall('yzfc_sun_courseclassify',array('uniacid'=>$_W['uniacid']));
$teacher=pdo_getall('yzfc_sun_teacher',array('uniacid'=>$_W['uniacid'],'status'=>1,'sid'=>0));
$school=pdo_getall('yzfc_sun_school',array('uniacid'=>$_W['uniacid'],'status'=>1));
foreach ($school as $key =>$value){
    $school[$key]['teacher']=pdo_getall('yzfc_sun_teacher',array('uniacid'=>$_W['uniacid'],'sid'=>$value['id'],'status'=>1));
}
...
//    var_dump($join);exit;
        ...
        $res=pdo_insert('yzfc_sun_course',$data);
        $cid=pdo_insertid();
        if($res){
//            $join=$_GPC['signchoose'];
...
//                pdo_insert('yzfc_sun_course_teach',$tea);
//            }
        ...
        $res=pdo_update('yzfc_sun_course',$data,array('id'=>$_GPC['id']));
        if($res){
...
if($_GPC['op']=='select_school'){
    $teacher=pdo_getall('yzfc_sun_teacher',array('uniacid'=>$_W['uniacid'],'sid'=>$_GPC['sid'],'status'=>1));
