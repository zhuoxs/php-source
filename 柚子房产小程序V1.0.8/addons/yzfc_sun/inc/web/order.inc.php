...
$sql = "SELECT * FROM ims_yzfc_sun_course_sign where ".$where." ORDER BY id DESC LIMIT ".(($page-1) * $size).','.$size;
//var_dump($sql);exit;
...
if($list){
    foreach ($list as $key =>$value){
        ...
        $list[$key]['cname']=pdo_get('yzfc_sun_course',array('id'=>$value['cid']),array('title'))['title'];
        if($value['sid']>0){
    ...
    $total=pdo_fetchcolumn("select count(*) from ims_yzfc_sun_course_sign where ".$where);
