...
$sql = "SELECT * FROM ims_yzfc_sun_user where ".$where." ORDER BY id DESC LIMIT ".(($page-1) * $size).','.$size;
$list = pdo_fetchall($sql);
foreach ($list as $key =>$value){
    $list[$key]['time']=date('Y-m-d H:i:s',$value['time']);

}
$total=pdo_fetchcolumn("select count(*) from ims_yzfc_sun_user where ".$where);
$pager = pagination($total, $page, $size);

if($_GPC['op']=='admin'){
    $isadmin=pdo_get('yzfc_sun_user',array('isadmin'=>1,'uniacid'=>$_W['uniacid']),array('id'));
    if($isadmin){
        message('设置失败,只能有一个超级管理员哦','','error');
    }else{
        $res=pdo_update('yzfc_sun_user',array('isadmin'=>1),array('id'=>$_GPC['id']));
        if($res){
...
if($_GPC['op']=='qxadmin'){
    $res=pdo_update('yzfc_sun_user',array('isadmin'=>0),array('id'=>$_GPC['id']));
    if($res){
