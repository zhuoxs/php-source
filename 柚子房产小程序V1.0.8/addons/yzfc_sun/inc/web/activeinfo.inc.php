...
$info=pdo_get('yzfc_sun_active',array('aid'=>$_GPC['aid'],'uniacid'=>$_W['uniacid']));
...
//	 p($_GPC);die;
    ...
    if(empty($_GPC['id'])){
        $res = pdo_insert('yzfc_sun_active', $data,array('uniacid'=>$_W['uniacid']));
        ...
        $res = pdo_update('yzfc_sun_active', $data, array('aid' => $_GPC['id'],'uniacid'=>$_W['uniacid']));
    }
    if($res){
