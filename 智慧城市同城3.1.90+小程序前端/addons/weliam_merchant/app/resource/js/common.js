...
common.uploadImageByWxJs = function(num, onck, inname) {
	...
	$('body').on('click', onck, function() {
        ...
        if(nownumber >= num) {
            common.alert('图片已达最大数量');
            return false;
        }
        wxJs.chooseImage(num - nownumber, function(data) {
            var imgstr = '';
            for(var i = 0; i < data.length; i++) {
                imgstr += '<li class="weui-uploader__file"><img src="' + data[i][0] + '"><input value="' + data[i][1] + '" type="hidden" name="' + inname + '"></li>';
            }
...
//创建url
common.createUrl = function(routes, params) {
	var strs = new Array();
	strs = routes.split("/");
	var url = './index.php?i=' + window.sysinfo.uniacid + '&c=entry&m=weliam_merchant&p=' + strs[0] + '&ac=' + strs[1] + '&do=' + strs[2];
	if(params) {
...
//使用腾讯地图获取地理位置
common.Geolocation = function(onComplete, onError, ischeck) {
	var checkloc = common.cookie.get('Geolocation');
	ischeck = ischeck || 'yes';
	if (!checkloc) {
		if(window.sysinfo.inwechat == 'yes') {
			wx.ready(function(){
		        wx.getLocation({
		            ...
		            success: function (res) {
		            	var newres = {};
		            	newres.lat = res.latitude;
		            	newres.lng = res.longitude;
		                showPosition(newres);
		            },
		            cancel: function (res) {
		            	onError(res);
	        ...
	        var geolocation = new qq.maps.Geolocation(window.sysinfo.mapkey, "myapp");
	        geolocation.getLocation(showPosition, onError, {timeout:4000, failTipFlag:true}); 
	    }
	} else {
		data = JSON.parse(checkloc);
		showPosition(data);
	}
	
    function showPosition(position){
    	if (!checkloc) {
    		common.cookie.set('Geolocation',JSON.stringify(position), window.sysinfo.cachetime);
    	}
		if (window.sysinfo.guide == 'yes' && ischeck == 'yes') {
			common.Checklocation(position.lat, position.lng);
		}
		onComplete(position);
	}
};
...
//使用IP获取所在城市
common.CitySearch = function(onComplete, onError) {
...
//判断用户当前地区是否是其定位地区
common.Checklocation = function(lat, lng) {
	$.post(common.createUrl('area/region/get_geocoder', {'lat':lat,'lng':lng}), function(res) {
		if (res.status == 0) {
			var locstr = res.result.address_component.city + res.result.address_component.district;
			var ishas = (typeof locstr == 'string') && (locstr.indexOf(window.sysinfo.areaname) != -1);
			var checkloc = common.cookie.get('Checklocation');
			if(!ishas && !checkloc) {
				var cutcity = res.result.address_component.city.substring(0, 2);
				var cutdistrict = res.result.address_component.district.substring(0, 2);
				var newarea = {};
				$.post(common.createUrl('area/region/get_area'), function(result) {
					var data = result['list'];
					var location = result['location'];
					//只有未开启精确定位时 才进行当前位置的判断
					if(location != 1) {
						//匹配县级地区
						$.each(data, function (key, val) {
							var scop = val.name.substring(0, 2);
							if (cutdistrict == scop) {
								newarea = val;
								return false;
							}
						...
						if (!$.isEmptyObject(newarea)) {
							$.confirm('系统检测到您当前位于' + newarea.name + '，是否切换到' + newarea.name, '系统提示',
								function () {
									window.location.href = window.location.href + '&areaid=' + newarea.id;
								},
								function () {
									common.cookie.set('Checklocation', 1, 7200);
								}
							);
						}
					}
...
//强制关注购买
common.ForceFollow = function(type, id) {
	if(window.sysinfo.inwechat != 'yes' || !type || !id || window.__wxjs_environment === 'miniprogram'){
		return false;
	}
	$.post(common.createUrl('common/wlCommon/memberFollow'), {type:type,id:id}, function(data) {
		if (data.status == 0) {
			var popupHTML = '<div class="popup popup-forcefollow" style="bottom: 0;height: 17rem;top: auto;position: fixed;"><div class="content native-scroll"><div class="content-inner"><div class="content-block text-center" style="margin: 1rem 0 0 0;"><p><img src="'+ data.result.message +'" style="width: 180px;margin-bottom: 10px;"></p><p>长按识别，扫码关注</p></div></div><div class="fix-bottom"><a class="weui-btn weui-btn_default external close-popup" href="javascript:;">取消</a></div></div></div>';
			$.popup(popupHTML);
		}
	}, 'json');
};
...
//浏览历史
localStorage.removeItem('memoryArr');
common.History_pag = function(hisDataItem) {
	//history.replaceState(hisData, '', hisData.hisUrl);
	//sessionStorage.hisData = JSON.stringify(hisData);
	
	
	var hisData = [];
	var key_ma = JSON.parse(sessionStorage.getItem("hisData"));
	if(key_ma !== "" && key_ma !== null){
		hisData = JSON.parse(sessionStorage.hisData);
	}
	var me_index = -1;
	hisData.forEach(function(item, index){
		if(item.hisUrl == hisDataItem.hisUrl){
			me_index = index;
			return false;
		}
	});
	if(me_index >= 0){
		hisData.splice(me_index, 1, hisDataItem);
	}else{
...
/*
common.History_pag_do = function(dataSelector) {
	window.onload = function(){
		if(window.history.state && window.history.state.data){
			$(dataSelector).html(window.history.state.data);
