...
$(document).ready(function(){ 
	...
	$("#form_post").click(function(){
		var flag = $("#form_post").attr('flag');
		if(flag == 1){
			uploadcomment($(this).attr("tieziid"),$("#pinglun_input").val());
		}else if(flag == 3){
			uploadcomment2($(this).attr("tieziid"),$("#pinglun_input").val(),3);
		}else{
			uploadcomment2($(this).attr("tieziid"),$("#pinglun_input").val(),2);
		}
		return false;
	});
	if( $("#FL_pl").length>0 )
...
function dianzanlist(o,tid,murl)
{
	o.children("em").children("img").attr("color",murl+"/plugin/pocket/app/resource/images/zan2.png");
	o.css("color","#ed414a");
	$.ajax({
		type: "post",
		dataType: "json",
