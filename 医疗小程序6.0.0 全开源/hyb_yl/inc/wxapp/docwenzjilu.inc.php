...
$op=$_GPC['op'];
if($op =='display'){
   $res = pdo_fetchall("select a.*,c.u_thumb  from(select max(m_id) as m_id from". tablename('hyb_yl_chat_msg_wz') ."where ifkf!=3 and docid='{$docid}' group by docid ) b left join ".tablename('hyb_yl_chat_msg_wz')." a on a.m_id=b.m_id  left join".tablename("hyb_yl_userinfo")."as c on c.openid=a.f_id ");
