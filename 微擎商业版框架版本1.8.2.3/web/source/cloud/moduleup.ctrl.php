...
if ($step == 'files' && $_GPC['d'] == 'prepare') {
    ...
    if(!empty($upgrade)){
      ...
      if(!$ret){
        itoast('系统错误！',create_url('cloud/mkinstall'),'error');
        die;
      }
    } 
    ...
    if(empty($filelist)){
      ...
      itoast($message['message'],create_url('cloud/mkinstall'),'success');
    }
    ...
    if($_GPC['f'] == 0 && $_GPC['d'] != 0){
      	...
      	if(!mkdirs($back)) {
          itoast('创建回滚目录失败，请返回重新尝试！',create_url('cloud/mkinstall'),'error');
      	  die;
        }
