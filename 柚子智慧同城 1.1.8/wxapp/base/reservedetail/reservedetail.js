...
t.Base({
    ...
    onShow: function() {
        ...
        }, "/base/reservedetail/reservedetail?id=" + this.data.id)
    },
    ...
    onLoadData: function() {
        ...
        Promise.all([t.api.apiStoreGetBanner({
            ...
            var r = t[1].data,
                n = 0;
            r.only_num > 0 && r.limit_num > 0 && 1 == r.is_vip ? (r.only_num > r.limit_num ? (n = r.limit_num - r.ygfree_num) > 0 ? e.setData({
                memberfree: 1
            }) : (n = 0, e.setData({
                memberfree: 0
            })) : r.only_num < r.limit_num && ((n = r.only_num - r.ygfree_num) > 0 ? e.setData({
                memberfree: 1
            }) : (n = 0, e.setData({
                memberfree: 0
            }))), e.setData({
                goods: r,
                residue_num: n
            })) : r.only_num > 0 && 0 == r.limit_num && 1 == r.is_vip && ((n = r.only_num - r.ygfree_num) > 0 ? e.setData({
                memberfree: 1
            }) : (n = 0, e.setData({
                memberfree: 0
            })), e.setData({
    ...
    spTap: function(a) {
        var e = this,
            ...
            d = ",",
            u = "";
        if (r[o].status = !0, r[o].attr_list.forEach(function(t, a) {
            t.status = !1
        }), r[o].attr_list[i].status = !0, s.attr_group_list.forEach(function(t, a) {
            1 == t.status && n++
    ...
    addNum: function() {
        ...
        if (e.attr_group_list) {
            ...
            if (a + 1 > i.stock) return t.tips("达可预约量上限！"), void this.setData({
                num: a
            })
        }
        return a + 1 > e.stock ? (t.tips("达可预约量上限！"), void this.setData({
            num: a
        })) : e.limit_num && a - (e.limit_num - e.yg_num) == 0 ? (t.tips("达可预约量上限！"), void this.setData({
            num: a
        })) : void this.setData({
            num: a + 1
        })
    ...
    formSubmit: function(a) {
        ...
        if (1 === this.someJudge()) {
            ...
            if (0 == i.use_attr) {
                ...
                t.api.apGoodscheckGoods(n).then(function(a) {
                    t.navTo("/base/reserveorder/reserveorder?gid=" + e.data.id + "&attr_ids=0&num=" + e.data.num + "&goods_type=2")
                }).
                catch (function(a) {
                    t.tips(a.msg)
                })
                ...
                if (0 == r.stock) return void t.tips("库存不足!");
                var d = {
                    gid: e.data.id,
                    num: e.data.num,
                    attr_ids: r.attr_ids,
                ...
                t.api.apGoodscheckGoods(d).then(function(a) {
                    t.navTo("/base/reserveorder/reserveorder?gid=" + e.data.id + "&attr_ids=" + r.attr_ids + "&num=" + e.data.num + "&goods_type=2")
                }).
                catch (function(a) {
                    t.tips(a.msg)
                })
    ...
    prewImg: function(t) {
        for (var a = this, e = t.currentTarget.dataset.index, i = t.currentTarget.dataset.idx, o = a.data.olist[e].imgs, s = [], r = 0; r < o.length; r++) s[r] = a.data.imgRoot + o[r];
        wx.previewImage({
            urls: s,
            current: s[i]
        })
    ...
    onShareAppMessage: function(t) {
        ...
        return {
            ...
            path: "/base/reservedetail/reservedetail?id=" + a.data.id + "&s_id=" + a.data.user_id
        }
    },
    onPosterTab: function() {
        ...
        if (this.data.info.pic && "" != this.data.info.pic) if (this.setData({
            ...
            var e = {
                link: "base/reservedetail/reservedetail?id=" + this.data.id + "&s_id=" + this.data.user_id,
                width: 120
            };
