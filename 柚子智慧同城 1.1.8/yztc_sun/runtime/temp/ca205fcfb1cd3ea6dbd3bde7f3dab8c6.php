<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:98:"/www/wwwroot/app.jishuizhibei.com/addons/yztc_sun/application/admin/view/ccoustomize/menuicon.html";i:1554424771;s:90:"/www/wwwroot/app.jishuizhibei.com/addons/yztc_sun/application/admin/view/common/index.html";i:1553823402;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
...
<script type="text/html" id="linkTypes">
    {{# if(d.link_type == 1){ }}内链{{# } }}
    {{# if(d.link_type == 2){ }}外部小程序{{# } }}
    {{# if(d.link_type == 3){ }}客服消息{{# } }}
    {{# if(d.link_type == 4){ }}分类信息{{# } }}
    {{# if(d.link_type == 5){ }}商家{{# } }}
</script>
<script>
    layui.use('table', function () {
        ...
        table.render({
            elem: '#laytable'
            ,url:"<?php echo adminurl('get_menuicon_list'); ?>"
            ,cols: [[
    ...
    $('#btnAddm').click(function () {
        var index = layer.open({
            ...
            maxmin: true, //开启最大化最小化按钮
            area: ['893px', '600px'],
            content: "<?php echo adminurl('addmenuicon'); ?>",
        });
        layer.full(index);
        return false;
    })
    ...
    $(document).on('click','.btnEditm',function(e){
        ...
        var index = layer.open({
            ...
            maxmin: true, //开启最大化最小化按钮
            area: ['893px', '600px'],
            content: "<?php echo adminurl('addmenuicon'); ?>&id="+id,
        });
        layer.full(index);
        return false;
    })
    ...
    $('#btnBatchDeletem').click(function () {
        ...
        if (data.length > 0){
            ...
            layer.confirm('即将删除 '+data.length+' 条记录', {
                ...
                var url = "<?php echo adminurl('deletem'); ?>";
                $.post(url, {ids:ids.join(',')}, function(data){
                    if (typeof data == 'string'){
                        data = $.parseJSON(data);
                    }
