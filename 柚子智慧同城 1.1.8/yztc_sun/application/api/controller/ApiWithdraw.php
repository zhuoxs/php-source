...
class ApiWithdraw extends Api
{
    ...
    public function getWithDrawSet(){
        ...
        if(!$withdrawset){
            error_json('请联系管理人员设置后台相关提现设置,在进行申请提现');
        }
        if($store['ptcc_rate']>0){
            $withdrawset['cms_rates']=$store['ptcc_rate'];
        }
        success_json($withdrawset);
    }
    //申请提现
    public function setWithDraw(){
        ...
        if($withdrawset['is_open']!=1){
            error_json('提现申请已关闭不能提现');;
        }
        if($money<1){
            error_json('提现金额最少￥1');
        }
        if($money<$withdrawset['min_money']){
            error_json('最低提现金额为￥'.$withdrawset['min_money']);
        }
        if($store['balance']<$money){
            error_json('余额不足');
        }
        //手续费
        $ratesmoney=0;
        ...
        //减少商户金额
        Db::name('store')->where(array('uniacid'=>$_W['uniacid'],'id'=>$store_id))->setDec('balance',$money);
        //增加商户金额明细
        ...
        if($withdraw['is_state']==1){
            ...
            if($arr['return_code']=='SUCCESS'&&$arr['result_code']=='SUCCESS'){
                ...
                //提现失败返还
                //增加商户金额
                Db::name('store')->where(array('uniacid'=>$_W['uniacid'],'id'=>$store_id))->setInc('balance',$money);
                //更新提现状态
