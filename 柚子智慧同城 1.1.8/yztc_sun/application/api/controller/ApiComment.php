    ...
    public function comment(){
        ...
        if($orderinfo['order_status']==40){
            ...
            if($detailinfo['is_evaluation']==0){
                ...
                if($res){
                    $detail->allowField(true)->save(['is_evaluation'=>1],['id' => $data['order_detail_id']]);
                    //保存订单信息
                    $order->allowField(true)->save(['is_evaluation'=>1,'finish_time'=>time(),'order_status'=>60],['id'=>$data['order_id']]);
                    return_json('评价成功',0);
                }else{
                    return_json('评价失败',1);
                }
