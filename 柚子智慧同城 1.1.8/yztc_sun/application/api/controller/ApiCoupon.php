...
class ApiCoupon extends Api
{
    ...
    public function couponList(){
        ...
        if($type==1){
            $filed='c.id,c.pic,c.indexpic,c.name,s.name as storename,c.full,c.end_time,s.address,c.is_recommend,c.is_del,c.state,c.uniacid,s.end_time as store_end_time';
            $order['c.read_num_virtual']=$sort;
        }elseif ($type==2){
            $filed='c.id,c.pic,c.indexpic,c.name,s.name as storename,c.full,c.end_time,s.address,c.is_recommend,c.is_del,c.state,c.uniacid,s.end_time as store_end_time';
            $order['c.create_time']=$sort;
        }elseif($type==3){
            $filed="c.id,c.pic,c.indexpic,c.name,s.name as storename,c.full,c.end_time,s.address,c.is_recommend,c.is_del,c.state,c.uniacid,s.end_time as store_end_time,( 6371 * acos (    
                cos ( radians(" . $lat . ") )    
                * cos( radians( c.lat ) )    
