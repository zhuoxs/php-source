...
class Cdistributionorder extends Base
{
    ...
    public function get_list(){
        ...
        $query = function ($query){
            ...
            $ordertype=input('get.ordertype')?input('get.ordertype'):0;
            $ordertype+=1;
        ...
        foreach ($list as &$item) {
            if($item['type']==1||$item['type']==5){
                ...
                $order=Db::name('order')->find($item['order_id']);
                $store=Db::name('store')->find($order['store_id']);
                $item['store_name']=$store['name'];
                $item['order_no']=$order['order_no'];
                $order_detail=Db::name('orderdetail')->where(array('order_id'=>$item['order_id']))->select();
                $goods='';
                ...
                $item['type_z']='抢购订单';
                $order=Db::name('panicorder')->find($item['order_id']);
                $store=Db::name('store')->find($order['store_id']);
                $item['store_name']=$store['name'];
                $item['order_no']=$order['order_no'];
                $item['order_amount']=$order['order_amount'];
                $panic=Db::name('panic')->find($order['pid']);
                $item['goods']=$panic['name']." ".str_replace(',','',$order['attr_list'])." ".$order['num'];
            }else if($item['type']==3){
                $item['type_z']='拼团订单';
                $order=Db::name('pinorder')->find($item['order_id']);
                $store=Db::name('store')->find($order['store_id']);
                $item['store_name']=$store['name'];
                $item['order_no']=$order['order_no'];
                $item['order_amount']=$order['order_amount'];
                $pingoods=Db::name('pingoods')->find($order['goods_id']);
                $item['goods']=$pingoods['name']." ".str_replace(',','',$order['attr_list'])." ".$order['num'];
            }else if($item['type']==4){
                $item['type_z']='会员卡订单';
                $order=Db::name('openvip')->find($item['order_id']);
                $item['store_name']='平台';
                $item['order_no']=$order['out_trade_no'];
                $order['order_status']=60;
                $item['order_amount']=$order['money'];
                $item['goods']=$order['name']." 1";
            ...
            $item['distribution']="一级佣金:{$item['first_price']} 二级佣金:{$item['second_price']} 三级佣金:{$item['third_price']}";
    ...
    private function balanceRefund($order){
        ...
        if($order['order_status']==5&&$order['review_status']==1&&$order['refund_status']==0){
            ...
            $Userbalancerecord->addBalanceRecord($order['user_id'],$order['uniacid'],3,0,$order['order_amount'],$order['order_id'],$order['order_no'],$remark);
