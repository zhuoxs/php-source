...
class Cdistributionwithdraw extends Base
{
...
//   审核通过
    public function pass(){
        ...
        $id = intval($ids);
        $uniacid = $_W['uniacid'];
        $withdraw =Db::name('distributionwithdraw')->where(array('uniacid'=>$uniacid,'id'=>$id))->find();
        if($withdraw['is_state']==1&&$withdraw['state']==0){
            Db::name('distributionwithdraw')->where(array('id'=>$id,'uniacid'=>$uniacid))->update(array('state'=>1,'review_time'=>time()));
            $withdraw =Db::name('distributionwithdraw')->where(array('uniacid'=>$uniacid,'id'=>$id))->find();
            ...
            if($arr['return_code']=='SUCCESS'&&$arr['result_code']=='SUCCESS'){
                ...
                $distributionpromoterModel->where(['id'=>$withdraw['promoter_id']])->setDec('freezemoney',$withdraw['money']);
                //更新提现状态
                Db::name('distributionwithdraw')->where(array('uniacid'=>$_W['uniacid'],'id'=>$id))->update(array('return_status'=>1,'return_time'=>time()));
                //增加合伙人分销金额明细
                $distributionpromoter=Distributionpromoter::get($withdraw['promoter_id']);
                $mcd_memo="合伙人提现失败返还-提现总金额:￥{$withdraw['money']};支付手续费:￥{$withdraw['ratesmoney']};实际提现金额:￥{$withdraw['realmoney']}";
                $data=[
    ...
    public function nopass(){
        ...
        $withdraw =Db::name('distributionwithdraw')->where(array('uniacid'=>$uniacid,'id'=>$id))->find();
        if($withdraw['is_state']==1&&$withdraw['state']==0){
            $res=Db::name('distributionwithdraw')->where(array('id'=>$id,'uniacid'=>$uniacid))->update(array('state'=>2,'review_time'=>time()));
            //审核失败返还 减少商户冻结资金
            $distributionpromoterModel=new Distributionpromoter();
            $distributionpromoterModel->where(['id'=>$withdraw['promoter_id']])->setDec('freezemoney',$withdraw['money']);
            //增加商家资金明细记录
            ...
            $data=[
                'user_id'=>$withdraw['user_id'],
                'promoter_id'=>$withdraw['promoter_id'],
                'type'=>5,
                'mcd_type'=>3,
                'openid'=>$withdraw['openid'],
                'sign'=>1,
