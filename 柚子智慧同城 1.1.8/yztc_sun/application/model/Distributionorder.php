...
class Distributionorder extends Base
{
    ...
    public function CountOrdermoney($parent,$uid=0){
        ...
        $sql_ordinary = " select ".$field." from ims_yztc_sun_distributionorder as d left join ims_yztc_sun_order as o on o.id=d.order_id ".$where;
    ...
    public function getMyTeam($user_id){
        ...
        if($set['level']>1){
            $sql="select count(id) as num from ims_yztc_sun_user where parents_id=any(select id as user1_id from ims_yztc_sun_user where parents_id=$user_id) ";
        ...
        if($set['level']>2){
            $sql="select count(id) as num from ims_yztc_sun_user where parents_id=any(select id as usr1_id from ims_yztc_sun_user where parents_id=any(select id as user2_id from ims_yztc_sun_user where parents_id=$user_id)) ";
            $num=Db::query($sql)['num'];
            $allnum+=$num;
            //计算未结算佣金,三级佣金
