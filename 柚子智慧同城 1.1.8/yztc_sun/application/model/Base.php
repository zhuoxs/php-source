...
//
    ...
    //填充排序与分页规则
    public function fill_order_limit_length($order= true,$limit= true){
        if ($limit) {
            //分页
            $page = input('request.page')?input('request.page'):1;
            $limit = input('request.length')?input('request.length'):10;
            if($page){
                $this->limit($limit)->page($page);
            }
        }
     ...
     * @param string $field
     */
    public function getAllList($where=array(),$field='',$order='id asc'){
        if($field!='') {
