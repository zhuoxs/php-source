...
class Goods extends Base
{
    ...
    //获取预约商品列表信息
    public function getBookGoodsListByStoreId($store_id){
        $data=$this->where(['store_id'=>$store_id,'state'=>1,'check_status'=>2,'goods_type'=>2])->order('index asc,id desc')->select();
        return $data;
    }
    //获取商品列表信息
    public function getGoodsListByStoreId($store_id){
        $data=$this->where(['store_id'=>$store_id,'state'=>1,'check_status'=>2,'goods_type'=>1])->order('index asc,id desc')->select();
        return $data;
    }
    //获取商品详情信息
