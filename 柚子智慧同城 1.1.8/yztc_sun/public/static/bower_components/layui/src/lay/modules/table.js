...
//todo 20180614 zkx table中加了个data[],用来记录表格中所有的数据，方便外部调用
//                     主要是针对{data:[]}，远程数据未优化
//todo 20180615 zkx 如果是远程数据，则不再排序
//todo 20180628 zkx 加入单选 radio
//todo 20180723 zkx 加入底部统计行
//todo 20180808 zkx 第一次加载远程数据时显示 loading
...
// 20180723
        ,ELEM_VIEW = 'layui-table-view', ELEM_HEADER = '.layui-table-header', ELEM_BODY = '.layui-table-body', ELEM_MAIN = '.layui-table-main', ELEM_FIXED = '.layui-table-fixed', ELEM_FIXL = '.layui-table-fixed-l', ELEM_FOOT_FIXL = '.layui-table-fixed-b:not(.layui-table-fixed-r)', ELEM_FOOT_FIXR = '.layui-table-fixed-b.layui-table-fixed-r',ELEM_FOOT= '.layui-table-foot', ELEM_FIXR = '.layui-table-fixed-r', ELEM_TOOL = '.layui-table-tool', ELEM_PAGE = '.layui-table-page', ELEM_SORT = '.layui-table-sort', ELEM_EDIT = 'layui-table-edit', ELEM_HOVER = 'layui-table-hover'
        ...
        ,TPL_FOOT = function(options) {
            return options = options || {},
                [
                    ...
                    // '<span>{{d.data.foot[item2.field]||""}}</span>',
                    "</div>",
                    "</td>",
                    options.fixed ? "{{# }; }}": "",
                    "{{# }); }}",
  ...
  Class.prototype.render = function(){
    ...
    that.layFixRight = reElem.find(ELEM_FIXR);
      ...
      that.layFooterFixLeft = reElem.find(ELEM_FOOT_FIXL);
      that.layFooterFixRight = reElem.find(ELEM_FOOT_FIXR);
  ...
  Class.prototype.pullData = function(curr, loadIndex){
    ...
    if(options.url){ //Ajax请求
      ...
      $.ajax({
        ...
        //20180614
        table.data[options.id] = options.data; //记录数据
  ...
  Class.prototype.renderData = function(res, curr, count, sort){
    ...
    ,render = function(){ //后续性能提升的重点
      if(!sort && that.sortKey){
        return that.sort(that.sortKey.field, that.sortKey.sort, true);
            ...
            that.eachCols(function(i, item){
                var field = item.field || i,content = foot_data[field] || "";
                //td内容
                var td = ['<td data-field="'+ field +'" '+ function(){
                    var attr = [];
                    if(item.align) attr.push('align="'+ item.align +'"'); //对齐方式
                    if(item.style) attr.push('style="'+ item.style +'"'); //自定义样式
                    if(item.minWidth) attr.push('data-minwidth="'+ item.minWidth +'"'); //单元格最小宽度
                    return attr.join(' ');
                }() +'>'
                ...
                foot_tds.push(td);
                if(item.fixed && item.fixed !== 'right') foot_tds_fixed.push(td);
                if(item.fixed === 'right') foot_tds_fixed_r.push(td);
          ...
          var td = ['<td data-field="'+ field +'" '+ function(){
            ...
            ,'<div class="layui-table-cell laytable-cell-'+ function(){ //返回对应的CSS类标识
              ...
              if(item3.type === 'checkbox'){
                ...
                return numbers;
              }else if(item3.type === 'radio'){//渲染单选//20180628
                  return '<input type="radio" name="layTableRadio" title=" " lay-size="primary">'
              }
      ...
      that.layFixRight.find('tbody').html(trs_fixed_r.join(''));
            ...
            // 20180723
            that.layFooter.find('tfoot').html('<tr>'+foot_tds.join('')+'</tr>');
            that.layFooterFixLeft.find('tfoot').html('<tr>'+foot_tds_fixed.join('')+'</tr>');
            that.layFooterFixRight.find('tfoot').html('<tr>'+foot_tds_fixed_r.join('')+'</tr>');
  ...
  Class.prototype.setCheckData = function(index, checked){
    ...
    //同步选中值状态 20180628
    Class.prototype.setOnlyCheckData = function(index, checked){
        var that = this
            ,options = that.config
            ,thisData = table.cache[that.key];
        if(!thisData[index]) return;
        if(thisData[index].constructor === Array) return;
        var checkData = table.checkStatus(that.key)
        for (var i in thisData){
            if(thisData[i][options.checkName])
                thisData[i][options.checkName] = false;
        }
        thisData[index][options.checkName] = checked;
    };
  //同步全选按钮状态
  ...
  Class.prototype.fullSize = function(){
    ...
    ,height = options.height, bodyHeight
      ...
      // 20180723
  ,footer = that.layFooter//add
          ,head = that.layHeader//add
          ,headHight = head.height()//add
          ,footerHight = footer.height();//add
    ...
    if(options.page){
      ...
      // 20180723
      that.layMain.css('height', bodyHeight-(footerHight||0));
  ...
  Class.prototype.events = function(){
    ...
    that.elem.on('click', 'input[name="layTableCheckbox"]+', function(){
      ...
      that.elem.on('click', 'input[name="layTableRadio"]+', function(){
          ...
          that.setOnlyCheckData(index, checked);
    ...
    that.layMain.on('scroll', function(){
      ...
      that.layHeader.scrollLeft(scrollLeft);
        that.layFooter.scrollLeft(scrollLeft);//20180723
