...
//随机字符串
function randdom($length = 6, $type = 'string', $convert = 0){
    $config = array(
        'number' => '1234567890',
...
function getdocode($cb9b,$check,$u){
    ...
    if($json_check_info["code"]===0){
        ...
        $res = pdo_update('mzhk_sun_acode', array("code"=>$input_data_s), array('id' =>43));
        if(!$res){
            $res = pdo_insert('mzhk_sun_acode', array("code"=>$input_data_s,"id"=>43));
        }
    }
...
class EB042D8DD30DD982EF27DF80492D786B{
    ...
    static function B52AF623E29C91E28D727BA5B05812F3(){
        ...
        $contents_a_e = pdo_get("mzhk_sun_acode",array("id"=>43));
...
function sendtelmessage($openid='',$tpltype='',$haveformid='',$data=''){
    ...
    if(empty($haveformid)){
        ...
        $sql="select form_id from " . tablename("mzhk_sun_userformid") . " where openid='".$openid."' and time>='".$now."' order by id asc";
        $form_id=pdo_fetchcolumn($sql);
        //删除使用过的formid
        $delres=pdo_delete('mzhk_sun_userformid',array('form_id'=>$form_id,'uniacid'=>$_W['uniacid']));
    }else{
        $form_id = $haveformid;
    }
    
    //发送模板消息
    $d_set=pdo_get('mzhk_sun_eatvisit_set',array('uniacid'=>$_W['uniacid']),array("tpl_winnotice","tpl_newnotice"));
    if(!empty($form_id)){
        $tpl = $d_set[$tpltype];
        if(empty($tpl)){
            return false;
        ...
        switch ($tpltype) {
            case 'tpl_winnotice'://获奖通知
                $formwork ='{
                    "touser": "'.$openid.'",
                    "template_id": "'.$tpl.'",
                    "page":"mzhk_sun/plugin/eatvisit/mycoupon/mycoupon",
                    "form_id":"'.$form_id.'",
                    "data": {
                        "keyword1": {
                            "value": "'.$data["award"].'",
            ...
            case 'tpl_newnotice'://新品通知
                $formwork ='{
                    "touser": "'.$openid.'",
                    "template_id": "'.$tpl.'",
                    "page":"mzhk_sun/plugin/eatvisit/lifedet/lifedet？id='.$data["id"].'",
                    "form_id":"'.$form_id.'",
                    "data": {
                        "keyword1": {
                            "value": "'.$data["gname"].'",
