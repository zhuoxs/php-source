...
$where=" WHERE  g.lid=1 and g.uniacid=".$_W['uniacid'];
if($_GPC['keywords']){
    $op=$_GPC['keywords'];
    $where.=" and g.gname LIKE  '%$op%'";
    $data[':name']=$op;
}
if($_GPC['status']){
    $status=$_GPC['status'];
...
$total=pdo_fetchcolumn("select count(*) as wname from " . tablename("mzhk_sun_goods") . " as g left join ". tablename('mzhk_sun_goodscate') . " as c on c.id=g.cateid " .$where." order by g.gid desc ",$data);
