...
if(checksubmit('submit')){
    ...
    if(is_uploaded_file($_FILES['apiclient_cert']['tmp_name'])) {
        ...
        if(move_uploaded_file($uploaded_file,iconv("utf-8","gb2312",$move_to_file))) {
			...
			pdo_update('mzhk_sun_system', array('apiclient_cert' => ''), array('id' => $_GPC['id'],'uniacid'=>$_W['uniacid']));
        } else {
            echo "上传失败";
        }
    } else {
    ...
    if(is_uploaded_file($_FILES['apiclient_key']['tmp_name'])) {
        ...
        if(move_uploaded_file($uploaded_file,iconv("utf-8","gb2312",$move_to_file))) {
			...
			pdo_update('mzhk_sun_system', array('apiclient_key' => ''), array('id' => $_GPC['id'],'uniacid'=>$_W['uniacid']));
        } else {
            echo "上传失败";
        }
    } else {
