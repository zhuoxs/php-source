...
if($_GPC['op']=='delete'){
    if($_W['ispost']){
        $res=pdo_delete('mzhk_sun_bargain',array('id'=>$_GPC['id'],'uniacid'=>$_W['uniacid']));
        if($res){
...
if($_GPC['op']=='tg'){
    ...
    $res=pdo_update('mzhk_sun_bargain',array('status'=>2),array('id'=>$_GPC['id'],'uniacid'=>$_W['uniacid']));
    if($res){
...
if($_GPC['op']=='jj'){
    $res=pdo_update('mzhk_sun_bargain',array('status'=>3),array('id'=>$_GPC['id'],'uniacid'=>$_W['uniacid']));
    if($res){
...
//
$info=pdo_get('mzhk_sun_bargain',array('id'=>$_GPC['id'],'uniacid'=>$_W['uniacid']));
...
if(checksubmit('submit')){
    ...
    if(empty($_GPC['id'])){
        $res = pdo_insert('mzhk_sun_bargain', $data ,array('uniacid'=>$_W['uniacid']));
        ...
        $res = pdo_update('mzhk_sun_bargain', $data, array('id' => $_GPC['id'],'uniacid'=>$_W['uniacid']));
    }
    if($res){
