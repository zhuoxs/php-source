...
$item=pdo_get('mzhk_sun_withdrawset',array('uniacid'=>$_W['uniacid']));
...
if(checksubmit('submit')){
    ...
    if($_GPC['id']==''){
        ...
        $res=pdo_insert('mzhk_sun_withdrawset',$data);
        if($res){
        ...
        $res = pdo_update('mzhk_sun_withdrawset', $data, array('id' => $_GPC['id']));
        if($res){
