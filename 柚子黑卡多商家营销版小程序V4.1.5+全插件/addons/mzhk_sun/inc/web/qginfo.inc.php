...
if($_GPC['op']=='sendgoods'){
    ...
    $res=pdo_update('mzhk_sun_qgorder',$sg_data,array('oid'=>$id));
    if($res){
        message('发货成功！', $this->createWebUrl('qginfo'), 'success');
    }else{
        message('发货失败！','','error');
    }
}
...
if($_GPC['op']=='send'){
    ...
    $uniacid = $_W['uniacid'];
    //获取订单信息
    $order = pdo_get('mzhk_sun_qgorder',array('uniacid'=>$uniacid,'oid'=>$id));
    if($order["status"]==5){
        message('该订单已经是完成状态，不需要在完成订单！','','error');
    }
    $res=pdo_update('mzhk_sun_qgorder',array('status'=>5,"finishtime"=>time()),array('oid'=>$id));
    if($res){
		...
		if(pdo_tableexists("mzhk_sun_distribution_set")){
			...
			if($dset['dsource']==1 && $dset['commission_type']!=1){//商家出佣金
				$dmoney = pdo_get('mzhk_sun_distribution_order',array('uniacid'=>$_W['uniacid'],'order_id'=>$id,'ordertype'=>4),array("first_price","second_price","third_price"));
        ...
        $data["mcd_type"] = 1;
        $data["mcd_memo"] = "抢购订单".$mcd_memo_server."-订单id：".$id.";订单号：".$order["orderNum"]."-".$redexplain." ".$dexplain." ".$fxexplain." ".$clexplain." ";//备注
        $data["addtime"] = time();
        $data["money"] = $money;
        $data["order_id"] = $id;
		...
		if($system['rebate_open']==1){
			...
			$ordername = '抢购订单';
			$result = pdo_update('mzhk_sun_rebate',array('isdelete'=>1),array('uniacid'=>$_W['uniacid'],'oid'=>$id,'type'=>5));
			
			if($result){
				$rebates = pdo_get('mzhk_sun_rebate',array('uniacid'=>$_W['uniacid'],'oid'=>$id,'type'=>5));
				//修改商家金额
...
if($_GPC['op']=='refund'){
    ...
    if($isrefund==3){
        $ress = pdo_update('mzhk_sun_qgorder',array("isrefund ="=>3),array('oid'=>$id,'uniacid'=>$uniacid));
        if($ress){
            message('拒绝成功！', $this->createWebUrl('qginfo'), 'success');
        }else{
            message('拒绝失败！','','error');
        }
    }
    ...
    if ($result['result_code'] == 'SUCCESS') {//退款成功
        ...
        // pdo_update('mzhk_sun_goods',array("num +="=>intval($order["num"])),array('gid'=>$order['gid'],'uniacid'=>$uniacid));
        pdo_update('mzhk_sun_qgorder',array("isrefund ="=>2,"out_refund_no ="=>$out_refund_no),array('oid'=>$id,'uniacid'=>$uniacid));
        message('退款成功！', $this->createWebUrl('qginfo'), 'success');
    }else{
        pdo_update('mzhk_sun_qgorder',array("out_refund_no ="=>$out_refund_no),array('oid'=>$id,'uniacid'=>$uniacid));
        if($order["paytype"]==2){//余额支付
            message('退款失败！','error');
        }else{
...
$sql = "select o.*,(select u.name from ".tablename('mzhk_sun_user')." as u where u.openid=o.openid limit 1) as uname from ".tablename('mzhk_sun_qgorder')." as o ".$where;
...
$total=pdo_fetchcolumn("select count(*) as wname from ".tablename('mzhk_sun_qgorder')." as o ".$where,$data);
...
if($_GPC['op']=='delete'){
    $res=pdo_delete('mzhk_sun_qgorder',array('oid'=>$_GPC['id']));
    if($res){
        message('删除成功！', $this->createWebUrl('qginfo'), 'success');
    }else{
        message('删除失败！','','error');
    }
}
...
include $this->template('web/qginfo');