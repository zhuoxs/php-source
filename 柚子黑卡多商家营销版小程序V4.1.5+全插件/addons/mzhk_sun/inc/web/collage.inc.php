...
$where=" WHERE   g.lid=3 and  g.uniacid=".$_W['uniacid'];
if($_GPC['keywords']){
...
if($_GPC['op']=='delete'){
    ...
    if($res){
        message('删除成功！', $this->createWebUrl('collage'), 'success');
    }else{
        message('删除失败！','','error');
    }
}
if($_GPC['op']=='open'){
    ...
    if($res){
        message('成功！', $this->createWebUrl('collage'), 'success');
    }else{
        message('失败！','','error');
    }
}
if($_GPC['op']=='tg'){
    ...
    if($res){
        message('通过成功！', $this->createWebUrl('collage'), 'success');
    }else{
        message('通过失败！','','error');
    }
}
if($_GPC['op']=='jj'){
    ...
    if($res){
        message('拒绝成功！', $this->createWebUrl('collage'), 'success');
    }else{
        message('拒绝失败！','','error');
    }
}
...
include $this->template('web/collage');