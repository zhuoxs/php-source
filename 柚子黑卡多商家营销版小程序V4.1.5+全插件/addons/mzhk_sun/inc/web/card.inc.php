...
$where=" WHERE   g.lid=4 and  g.uniacid=".$_W['uniacid'];
...
if($list){
	//获取参与人数
	foreach($list as $k=>$v){
		$joinnums=pdo_fetchcolumn("select count(*) as wname from ".tablename("mzhk_sun_cardcollect")." where gid=".$v['gid']." ");
		$list[$k]['joinnums'] = intval($joinnums)>0?$joinnums:0;
	}
}
