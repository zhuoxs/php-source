...
// var_dump('<pre>');
// var_dump($goods_sku_data);
 $attr_ids = array();
 $attr_value_ids = array();
 if($goods_sku_data){
     foreach ($goods_sku_data as $k=>$v){
        // $properties_name_json  = json_decode(htmlspecialchars_decode($v['properties_name_json']),true);
        // if($properties_name_json){
        //   foreach ($properties_name_json as $kk=>$vv){
        //       if(!in_array($vv['kid'],$attr_ids)){
        //           $attr_ids[] =  $vv['kid'];
        //       }
        //   }
        // }
...
if(checksubmit('submit')){
 // p($_GPC);die;
	...
	$data['ptshopprice']=$_GPC['ptshopprice'];
...
//$sys=pdo_get('mzhk_sun_system',array('uniacid'=>$_W['uniacid']),array("openblackcard"));
