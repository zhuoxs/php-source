...
if($_GPC['op']=='add'){
    ...
    $data['position']=intval($_GPC['position']);//1弹窗，2首页轮播（默认主题），3砍价列表，4集卡列表，5抢购列表，6拼团列表，7免单列表，8营销图标，9底部导航，10广告1（主题2），10广告2（主题2）
    
    if($id==0){
        $data['uniacid']=$_W['uniacid'];
        $res=pdo_insert('mzhk_sun_popbanner',$data);
        if($res){
            message('添加成功',$this->createWebUrl('acbranner'),'success');
        }else{
            message('添加失败','','error');
        }
    }else{
        ...
        if($res){
            message('修改成功',$this->createWebUrl('acbranner'),'success');
        }else{
            message('修改失败','','error');
        }
    }
    ...
    if($res){
        message('删除成功！', $this->createWebUrl('acbranner'), 'success');
    }else{
        message('删除失败！','','error');
    }
}else{
    $where=" WHERE uniacid=".$_W['uniacid']." and position in(3,4,5,6,7,16)";
