...
if($_GPC['op']=='send'){
    ...
    $uniacid = $_W['uniacid'];
    //获取订单信息
    $order = pdo_get('mzhk_sun_kjorder',array('uniacid'=>$uniacid,'oid'=>$id));
    if($order["status"]==5){
    ...
    if($res){
		...
		if(pdo_tableexists("mzhk_sun_distribution_set")){
			...
			if($dset['dsource']==1 && $dset['commission_type']!=1){//商家出佣金
				$dmoney = pdo_get('mzhk_sun_distribution_order',array('uniacid'=>$_W['uniacid'],'order_id'=>$id,'ordertype'=>2),array("first_price","second_price","third_price"));
        ...
        $data["mcd_type"] = 1;
        $data["mcd_memo"] = "砍价订单".$mcd_memo_server."-订单id：".$id.";订单号：".$order["orderNum"]."-".$redexplain." ".$dexplain." ".$fxexplain." ".$clexplain." ";//备注
        $data["addtime"] = time();
        $data["money"] = $money;
        $data["order_id"] = $id;
...
$sql = "select o.*,(select u.name from ".tablename('mzhk_sun_user')." as u where u.openid=o.openid limit 1) as uname from ".tablename('mzhk_sun_kjorder')." as o ".$where;
...
$total=pdo_fetchcolumn("select count(*) as wname from ".tablename('mzhk_sun_kjorder')." as o ".$where,$data);
