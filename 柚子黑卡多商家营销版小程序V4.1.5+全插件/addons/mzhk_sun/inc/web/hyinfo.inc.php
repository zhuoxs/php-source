...
if(!empty($_GPC['lotterytype'])){
    ...
    if($lotterytype==9){
        $where.=" and o.islottery=0 ";
    }else{
        $where.=" and o.islottery = $lotterytype";
    }
}
...
if($islottery>0){
    $where.= " and o.islottery = $islottery ";
}
$sql = "select o.*,(select u.name from ".tablename('mzhk_sun_user')." as u where u.openid=o.openid limit 1) as uname from ".tablename('mzhk_sun_hyorder')." as o ".$where;
...
$total=pdo_fetchcolumn("select count(*) as wname from ".tablename('mzhk_sun_hyorder')." as o ".$where,$data);
...
if($_GPC['op']=='change'){
    ...
    //获取订单信息
    $order = pdo_get('mzhk_sun_hyorder',array('uniacid'=>$uniacid,'oid'=>$id),array("money","bid","gid","orderNum","status","oid","gname"));
    if($status==2){
        if($order["status"]==2){
