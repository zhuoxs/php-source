...
$info=pdo_get('mzhk_sun_plugin_lottery_sponsorship',array('sid'=>$_GPC['id'],'uniacid'=>$_W['uniacid']));

//查出已是商家用户
$sjuser=pdo_getall('mzhk_sun_plugin_lottery_sponsorship',array('uniacid'=>$_W['uniacid'],'status'=>'2'),'uid');
...
if(checksubmit('submit')){
    ...
    if($_GPC['sid']==''){
    	...
    	$data['endtime']=0;
    	$data['status']=1;
        $res = pdo_insert('mzhk_sun_plugin_lottery_sponsorship', $data);
        if($res){
        ...
        $store=pdo_get('mzhk_sun_plugin_lottery_sponsorship',array('uniacid'=>$_W['uniacid'],'sid'=>$_GPC['sid']));
        ...
        $res=pdo_update('mzhk_sun_plugin_lottery_sponsorship',$data,array('sid'=>$_GPC['id'],'uniacid'=>$_W['uniacid']));
        if($res){
