...
$item=pdo_get('mzhk_sun_plugin_lottery_system',array('uniacid'=>$_W['uniacid']));

$certfile = IA_ROOT . "/addons/mzhk_sun_plugin_lottery/cert/" . 'apiclient_cert_' .$_W['uniacid'] . '.pem';
$keyfile = IA_ROOT . "/addons/mzhk_sun_plugin_lottery/cert/" . 'apiclient_key_' . $_W['uniacid'] . '.pem';
    ...
    if(checksubmit('submit')){
        ...
        if(is_uploaded_file($_FILES['apiclient_cert']['tmp_name'])) {
            ...
            //我们给每个用户动态的创建一个文件夹
            $user_path=IA_ROOT."/addons/mzhk_sun_plugin_lottery/cert/";
        ...
        if(is_uploaded_file($_FILES['apiclient_key']['tmp_name'])) {
            ...
            if($_GPC['id']==''){                
                $res=pdo_insert('mzhk_sun_plugin_lottery_system',$data,array('uniacid'=>$_W['uniacid']));
                if($res){
