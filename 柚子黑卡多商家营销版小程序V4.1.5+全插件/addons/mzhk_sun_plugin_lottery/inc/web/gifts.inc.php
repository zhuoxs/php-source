...
$Giftstype=pdo_getall('mzhk_sun_plugin_lottery_type',array('uniacid'=>$_W['uniacid']));

$sql="SELECT a.*,b.`type`,c.`sname` FROM ".tablename('mzhk_sun_plugin_lottery_gifts'). " a"  . " left join " . tablename("mzhk_sun_plugin_lottery_type") . " b on b.id=a.type left join ". tablename("mzhk_sun_plugin_lottery_sponsorship") ." c on c.sid=a.sid ".$where. " ORDER BY a.status asc,a.id desc";
$total=pdo_fetchcolumn("SELECT count(*) FROM ".tablename('mzhk_sun_plugin_lottery_gifts'). " a" ." left join " . tablename("mzhk_sun_plugin_lottery_type") . " b on b.id=a.type ".$where,$data);
...
if($_GPC['op']=='delete'){
    ...
    // $status=$_GPC['status'];
    // $count=$_GPC['count'];
    $res=pdo_delete('mzhk_sun_plugin_lottery_gifts',array('id'=>$_GPC['id'],'uniacid'=>$_W['uniacid']));
    // $order=pdo_getall('mzhk_sun_plugin_lottery_order',array('id'=>$_GPC['id'],'uniacid'=>$_W['uniacid']));
    // if(!empty($order)){
    //     $result=pdo_delete('mzhk_sun_plugin_lottery_order',array('id'=>$_GPC['id'],'uniacid'=>$_W['uniacid']));
    // }
...
if($_GPC['op']=='tg'){
    // p($_GPC['id']);die;
    $res=pdo_update('mzhk_sun_plugin_lottery_gifts',array('status'=>2),array('id'=>$_GPC['id'],'uniacid'=>$_W['uniacid']));
    if($res){
...
if($_GPC['op']=='ty'){
    // p($_GPC['id']);die;
    $res=pdo_update('mzhk_sun_plugin_lottery_gifts',array('status'=>1),array('id'=>$_GPC['id'],'uniacid'=>$_W['uniacid']));
    if($res){
...
if($_GPC['op']=='jj'){
    $res=pdo_update('mzhk_sun_plugin_lottery_gifts',array('status'=>3),array('id'=>$_GPC['id'],'uniacid'=>$_W['uniacid']));
    if($res){
