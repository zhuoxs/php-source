...
$GLOBALS['frames'] = $this->getMainMenu();
 function tixian($withdraw){
    ...
    //保存报文信息
    pdo_update('mzhk_sun_withdraw',array('baowen'=>$xml),array('uniacid'=>$_W['uniacid'],'id'=>$withdraw['id']));
    //添加报文记录
    $baowen=array(
        'uniacid'=>$_W['uniacid'],
    ...
    pdo_insert('mzhk_sun_withdraw_baowen',$baowen);
    //更改提现状态 商家明显提现状态
    $arr=xml2array($xml);
    if($arr['return_code']=='SUCCESS'&&$arr['result_code']=='SUCCESS'){
        pdo_update('mzhk_sun_withdraw',array('status'=>1,'tx_time'=>time(),'request_time'=>time()),array('uniacid'=>$_W['uniacid'],'id'=>$withdraw['id']));
        pdo_update('mzhk_sun_mercapdetails',array('status'=>1),array('uniacid'=>$_W['uniacid'],'wd_id'=>$withdraw['id']));
    }else{
        pdo_update('mzhk_sun_withdraw',array('status'=>2,'request_time'=>time(),'err_code'=>$arr['err_code'],'err_code_des'=>$arr['err_code_des']),array('uniacid'=>$_W['uniacid'],'id'=>$withdraw['id']));
        pdo_update('mzhk_sun_mercapdetails',array('status'=>2),array('uniacid'=>$_W['uniacid'],'wd_id'=>$withdraw['id']));
    }
    return $arr;
...
function postXmlCurl($xml, $url, $second = 30)
{
    ...
    curl_setopt($ch, CURLOPT_SSLCERT, IA_ROOT . '/addons/mzhk_sun/cert/apiclient_cert_'.$_W['uniacid'].'.pem');
    curl_setopt($ch, CURLOPT_SSLKEYTYPE, 'PEM');
    curl_setopt($ch, CURLOPT_SSLKEY, IA_ROOT . '/addons/mzhk_sun/cert/apiclient_key_'.$_W['uniacid'].'.pem');
...
if($_GPC['op']=='delete'){
    $res=pdo_update('mzhk_sun_withdraw',array('del_status'=>1),array('id'=>$_GPC['id'],'uniacid'=>$_W['uniacid']));
    if($res){
    ...
    if($withdraw['is_state']==1&&$withdraw['state']==0){
        $res=pdo_update('mzhk_sun_withdraw',array('state'=>1,'review_time'=>time()),array('id'=>$id,'uniacid'=>$uniacid));
        $withdraw=pdo_get('mzhk_sun_withdraw',array('uniacid'=>$_W['uniacid'],'id'=>$withdraw['id']));
        $arr=tixian($withdraw);
        if($arr['return_code']=='SUCCESS'&&$arr['result_code']=='SUCCESS'){
            $msg='提现成功请在微信钱包查看。';
        }else{
    ...
    $withdraw = pdo_get('mzhk_sun_withdraw',array('uniacid'=>$uniacid,'id'=>$id));
    if($withdraw['is_state']==1&&$withdraw['state']==0){
        $res=pdo_update('mzhk_sun_withdraw',array('state'=>2,'review_time'=>time()),array('id'=>$id,'uniacid'=>$uniacid));
        //拒绝增加商户金额
        pdo_update('mzhk_sun_store',array('money +='=>$withdraw['money']),array('uniacid'=>$_W['uniacid'],'id'=>$withdraw['store_id']));
        //增加商家资金明细记录
        ...
        pdo_insert('mzhk_sun_mercapdetails',$mercapdetails);
        //拒绝扣除平台累计金额
        pdo_update('mzhk_sun_platform',array('total_money -='=>$withdraw['money'],'total_realmoney -='=>$withdraw['realmoney'],'total_paycommission -='=>$withdraw['paycommission'],'total_ratesmoney -='=>$withdraw['ratesmoney']),array('uniacid'=>$_W['uniacid']));
    ...
    //更新提现状态
    $res=pdo_update('mzhk_sun_withdraw',array('return_status'=>1,'return_time'=>time()),array('uniacid'=>$_W['uniacid'],'id'=>$id));
    //增加商户金额
    pdo_update('mzhk_sun_store',array('money +='=>$withdraw['money']),array('uniacid'=>$_W['uniacid'],'id'=>$withdraw['store_id']));
    //增加商户金额明细
    $mcd_memo="商家提现失败返还-提现总金额:￥{$withdraw['money']};支付佣金:￥{$withdraw['paycommission']};支付手续费:￥{$withdraw['ratesmoney']};实际提现金额:￥{$withdraw['realmoney']}";
    $store=pdo_get('mzhk_sun_store',array('id'=>$withdraw['store_id']));
    $mercapdetails=array(
        'uniacid'=>$_W['uniacid'],
        'store_id'=>$withdraw['store_id'],
