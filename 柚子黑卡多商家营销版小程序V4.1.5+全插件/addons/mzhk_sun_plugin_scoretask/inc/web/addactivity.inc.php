...
$info=pdo_get('mzhk_sun_activity',array('id'=>$_GPC['id']));
if($info['lb_pics']){
...
$category=pdo_getall('mzhk_sun_activity_category',array('uniacid'=>$_W['uniacid']));
...
if(checksubmit('submit')){
    ...
    if(empty($_GPC['id'])){
        $res = pdo_insert('mzhk_sun_activity', $data);
        if($res){
        ...
        unset($data['add_time']);
        $res = pdo_update('mzhk_sun_activity', $data, array('id' => $_GPC['id']));
        if($res){
