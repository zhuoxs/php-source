...
$info=pdo_get('mzhk_sun_plugin_scoretask_goods',array('id'=>$_GPC['id']));
if(checksubmit('submit')){
    ...
    $data['add_time']= time();
    if(empty($_GPC['id'])){
        $res = pdo_insert('mzhk_sun_plugin_scoretask_goods', $data);
    }else{
        $res = pdo_update('mzhk_sun_plugin_scoretask_goods', $data, array('id' => $_GPC['id']));
    }
    if($res){
