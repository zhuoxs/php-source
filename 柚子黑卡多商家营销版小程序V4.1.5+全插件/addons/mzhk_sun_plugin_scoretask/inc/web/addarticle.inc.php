...
$info=pdo_get('mzhk_sun_plugin_scoretask_article',array('id'=>$_GPC['id']));
if(checksubmit('submit')){
    ...
    $data['add_time']= time();
    $article=pdo_get('mzhk_sun_plugin_scoretask_article',array('uniacid'=>$_W['uniacid'],'type'=>$_GPC['type']));
    if(empty($_GPC['id'])){
        $res = pdo_insert('mzhk_sun_plugin_scoretask_article', $data);
        if($res){
        ...
        unset($data['add_time']);
        $res = pdo_update('mzhk_sun_plugin_scoretask_article', $data, array('id' => $_GPC['id']));
        if($res){
