...
$info=pdo_get('mzhk_sun_story_category',array('id'=>$_GPC['id']));
if(checksubmit('submit')){
    $data['uniacid']=$_W['uniacid'];
    $data['title']=$_GPC['title'];
    $data['add_time']= time();  
    if(empty($_GPC['id'])){
        $res = pdo_insert('mzhk_sun_story_category', $data);
        if($res){
        ...
        unset($data['add_time']);
        $res = pdo_update('mzhk_sun_story_category', $data, array('id' => $_GPC['id']));
        if($res){
