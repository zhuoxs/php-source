...
//查出已是商家用户
$info = pdo_get('mzhk_sun_store',array('uniacid'=>$_W['uniacid'],'id'=>$_GPC['id']));

//获取微信号
$userlist=pdo_get('mzhk_sun_user',array('uniacid'=>$_W['uniacid'],'openid'=>$info['openid']),array("name"));
$info["name"] = $userlist["name"];

$category=pdo_getall('mzhk_sun_store_category',array('uniacid'=>$_W['uniacid'],'state'=>1));
$district=pdo_getall('mzhk_sun_store_district',array('uniacid'=>$_W['uniacid'],'state'=>1));
if($_GPC['op']=='search'){
    $name=$_GPC['name'];
...
if(checksubmit('submit')){
            ...
            $data['rz_status']=$_GPC['rz_status'];
			 if($_GPC['id']==''){
				$res = pdo_insert('mzhk_sun_store', $data);
				if($res){
