...
$info=pdo_get('mzhk_sun_privilege_identifier',array('id'=>$_GPC['id']));
if(checksubmit('submit')){
    ...
    $data['add_time']= time();  
    if(empty($_GPC['id'])){
        $res = pdo_insert('mzhk_sun_privilege_identifier', $data);
        if($res){
        ...
        $res = pdo_update('mzhk_sun_privilege_identifier', $data, array('id' => $_GPC['id']));
        if($res){
