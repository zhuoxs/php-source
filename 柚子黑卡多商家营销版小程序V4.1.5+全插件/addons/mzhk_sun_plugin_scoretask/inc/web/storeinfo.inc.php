...
$info=pdo_get('mzhk_sun_store',array('id'=>$_GPC['id']));
$area=pdo_getall('mzhk_sun_area',array('uniacid'=>$_W['uniacid']));
$type=pdo_getall('mzhk_sun_storetype',array('uniacid'=>$_W['uniacid'],'state'=>1),array(),'','num asc');
$type2=pdo_get('mzhk_sun_storetype2',array('id'=>$info['storetype2_id']));
//查出已是商家用户
$sjuser=pdo_getall('mzhk_sun_store',array('uniacid'=>$_W['uniacid']),'user_id');
//二维数组转一维
...
//用户
$user=pdo_getall('mzhk_sun_user',array('uniacid'=>$_W['uniacid'],'id !='=>$yuser));
//入住类型
//$typein=pdo_getall('mzhk_sun_storein',array('uniacid'=>$_W['uniacid']));
   $time=24*60*60*7;//一周
...
if(checksubmit('submit')){
		...
		if($_GPC['user_name']){
			$rst=pdo_get('mzhk_sun_store',array('user_name'=>$_GPC['user_name'],'uniacid'=>$_W['uniacid'],'id !='=>$_GPC['id']));
			if($rst){
			...
			if($_GPC['type']){
				...
				$res = pdo_update('mzhk_sun_store', $data, array('id' => $_GPC['id']));
				if($res){
		 ...
		 function  getCoade($md_id){
          ...
          function set_msg($md_id){
           ...
           $data2=array(
            "scene"=>$md_id,
            "page"=>"mzhk_sun/pages/sellerinfo/sellerinfo",
            "width"=>100
            );
