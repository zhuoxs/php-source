...
class Corder extends Admin
{
    public function save(){
        ...
        foreach ($data['goodses'] as $goods) {
            $g[] = $goods['goods_name'].' * '.$goods['num'];
        }
        $order_data['detail'] = implode('，',$g);

        $order = new Deliveryorder($order_data);
        $order->allowField(true)->save();

        foreach ($data['goodses'] as &$goods) {
            $goods['deliveryorder_id'] = $order->id;
        }
        $goods_model = new Deliveryordergoods();
        $goods_model->saveAll($data['goodses']);
    }
    //    获取列表页数据
    public function get_list(){
        ...
        foreach ($list as &$item) {
            $item['address_z']=$item['province'].$item['city'].$item['zip'].$item['address'];
