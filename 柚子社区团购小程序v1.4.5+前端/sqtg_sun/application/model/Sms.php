...
class Sms extends Base
{
    ...
    public function onPinorderPay($order) {
        $store_ids = Pinorder::where('id', $order->id)
            ->field('store_id')
            ->find();
        $this->SendSms($store_ids->store_id, 0);
    }
    ...
    public function SendSms($store_id,$smstype){
        ...
        if($sms&&$sms['is_open']==1){
            if($sms["smstype"]==1){
                ...
                if($msg!=''){
                 return $this->SendYtxSms($msg,$sms,$phone);
                }
            }else if($sms["smstype"]==3){
                $sendid = $smstype==1?$sms["aly_orderrefund"]:$sms["aly_order"];
                return $this->SendAldy($sms,$phone,$sendid);
            }
        }
    }
