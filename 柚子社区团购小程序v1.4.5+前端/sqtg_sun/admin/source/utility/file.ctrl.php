...
if (!in_array($do, array('upload', 'fetch', 'browser', 'delete', 'image' ,'module' ,'video', 'voice', 'news', 'keyword',
	'networktowechat', 'networktolocal', 'towechat', 'tolocal','wechat_upload','group_list','add_group','change_group','del_group'))) {
	exit('Access Denied');
}
$result = array(
	'error' => 1,
...
if ($do == 'image') {
	...
	if ($islocal) {
		$page = $_GPC['page'];
		$page = max(1, $page);
		$condition = ' WHERE uniacid = :uniacid AND type = :type AND module_upload_dir = :module_upload_dir AND group_id=:group_id';
		$params = array(':uniacid' => $uniacid, ':type' => 1, ':module_upload_dir' => $module_upload_dir,':group_id'=>$_GPC['groupid']);
