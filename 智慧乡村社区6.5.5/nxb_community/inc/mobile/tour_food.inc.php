...
if($_GPC['act'] == 'list_ajax'){
    ...
    $psize = 10;
    $start = ($page-1)*$psize;
    $list = pdo_fetchall("SELECT * FROM ".tablename('bc_community_tour_food')." WHERE weid=$weid AND town_id = $id ORDER BY dateline DESC LIMIT $start,$psize");

    foreach ($list as $key => $value) {
        echo '<div style="margin-bottom: 10px;" class="link" data-url="'.$this->createMobileUrl('tour_food_view',array('food_id'=>$value['food_id'])).'">
            <div>
...
include $this -> template('tour_food');
