...
if($_GPC['act'] == 'list_ajax'){
    ...
    $postlist = pdo_fetchall("SELECT * FROM " . tablename('bc_community_news') . " WHERE weid=:uniacid AND danyuan=$id $sql ORDER BY nid DESC LIMIT $start,$psize", array(':uniacid' => $_W['uniacid']));
...
//获取本镇所有的村列表
$townlist=pdo_fetchall("SELECT * FROM ".tablename('bc_community_town')." WHERE weid=:uniacid AND lev=3 AND pid=:pid  ORDER BY paixu ASC,id DESC LIMIT 50",array(':uniacid'=>$_W['uniacid'],':pid'=>$id));
...
$menuscolist=pdo_fetchall("SELECT * FROM".tablename('bc_community_menu')."WHERE weid=:uniacid AND mtype=2 AND townid=:townid  ORDER BY displayorder DESC,meid DESC",array(':uniacid'=>$_W['uniacid'],':townid'=>$id));
//判断菜单长度
$menusco_width = 30+50;
...
//精选
$hotpost = pdo_fetchall("SELECT * FROM " . tablename('bc_community_news') . " WHERE weid=:uniacid AND danyuan=" . $id . " AND nimg<>'' ORDER BY browser DESC LIMIT 0,4", array(':uniacid' => $_W['uniacid']));
