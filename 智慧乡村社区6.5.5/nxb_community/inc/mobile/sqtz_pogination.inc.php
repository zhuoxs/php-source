...
if(!empty($res5)){
	...
	if(!empty($meid) && $meid!=0){
		...
		$cxtj=' AND nmenu IN('.$cwgkid.','.$cwgkid.','.$zwgkid.','.$dwgkid.','.$zcfgid.')';
	}
	
	
	
	
	$res = pdo_fetchall("SELECT a.*,b.avatar,b.nickname,b.realname,b.grade FROM " . tablename('bc_community_news') . " as a left join ".tablename('bc_community_member')." as b on a.mid=b.mid WHERE a.weid=" . $_W['uniacid'] . $cxtj." AND status=0 ORDER BY top DESC,nid DESC LIMIT  ". $num . ",{$psize}");	
					...
					.'<div class="mui-col-xs-10 pt1 pb1">'
						.'<div class="mui-row">'
								.'<div class="mui-col-xs-6 text_overflow1">'
									...
									$ht.='</span><span class="ulev-1 t-gra pl05">(';
									if ($item['grade']==2){
										$ht.='认证村民';
									}
									if ($item['grade']==3){
								...
								if (getmenuname($item['weid'],$item['nmenu'])=='志愿活动'){
								...
								$ht.='<div class="mui-col-xs-12 mt05">';
									if ($images[0]!=''){
										if($imgnum==1){
								...
								$ht.='</div></a>'
								.'<div class="mui-col-xs-5"><span class="pl05 ulev-1 t-gra">'.getgaptime($item['nctime']).'</span></div>'
								.'<div class="mui-col-xs-7 tx-r t-gra ulev-1">'
									.'<span class="am-icon-eye"> '.$item['browser'].'</span>&nbsp;&nbsp;'
									.'<a href="'.$this->createMobileUrl('newsinfo',array('id'=>$item['nid'])).'" class="t-gra"><span class="am-icon-commenting-o"> '.getcommentnum($item['weid'],$item['nid']).'</span>&nbsp;&nbsp;</a>'
