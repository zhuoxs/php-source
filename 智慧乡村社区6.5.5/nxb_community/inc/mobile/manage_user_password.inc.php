...
if ($webtoken == '') {
    ...
    if ($_W['ispost']) {
        if (checksubmit('submit')) {
            if ($_GPC['upsd'] == '') {
                message('密码必填的哦~', $this->createMobileUrl('manage_user_password', array('nav' => 6)), 'error');
            }
            $newdata = array(
                'upsd'   => md5($_GPC['upsd'])
            );
            $res = pdo_update('bc_community_jurisdiction', $newdata, array('id' => $manageid));
            if (!empty($res)) {
                message('提交成功', $this->createMobileUrl('manage_user_password', array('nav' => 6)), 'success');
            } else {
                message('提交失败！', $this->createMobileUrl('manage_user_password', array('nav' => 6)), 'error');
            }
        }
    }
    include $this->template('manage_user_password');
}
