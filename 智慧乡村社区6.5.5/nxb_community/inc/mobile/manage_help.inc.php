...
//获取所有救助扶贫记录
//$list=pdo_fetchall("SELECT * FROM ".tablename('bc_community_help')." WHERE weid=:uniacid",array(':uniacid'=>$_W['uniacid']));
	
	
	$count=0;
	$pindex = max(1, intval($_GPC['page']));
	$psize = 10;	
	$total = pdo_fetchcolumn("SELECT count(id) FROM " . tablename('bc_community_help') . " WHERE weid=:uniacid ".$cx." ORDER BY id DESC", array(':uniacid' => $_W['uniacid']));
	$count = ceil($total / $psize);
		
	
	
	include $this->template('manage_help');
}
