...
if($_GPC['act'] == 'assess'){
    ...
    $count = pdo_fetchcolumn("SELECT count(*) FROM ".tablename('bc_community_assess_log')." WHERE link_id=$id AND a_type=3 AND mid=$mid");
    if($count < 1){
        ...
        $items = pdo_fetch("SELECT * FROM ".tablename('bc_community_tour_hotel')." WHERE hotel_id = $id");
        if($assess==1){
            pdo_update('bc_community_tour_hotel',array('good'=>$items['good']+1),array('hotel_id'=>$id));
        }
        if($assess==2){
            pdo_update('bc_community_tour_hotel',array('general'=>$items['general']+1),array('hotel_id'=>$id));
        }
        if($assess==3){
            pdo_update('bc_community_tour_hotel',array('poor'=>$items['poor']+1),array('hotel_id'=>$id));
        }
        $result = array();
        $result['full'] = 1;
        $result['message'] = '评价成功！';
...
if($_GPC['act'] == 'message_list_ajax') {
    ...
    $psize = 10;
    $start = ($page-1)*$psize;
    $list = pdo_fetchall("SELECT A.*,B.nickname,B.avatar FROM ".tablename('bc_community_tour_comment')." A LEFT JOIN ".tablename('bc_community_member')." B ON A.mid=B.mid WHERE A.weid=$weid AND A.ctype=3 AND A.link_id = $id ORDER BY A.dateline DESC LIMIT $start,$psize");
...
include $this -> template('tour_hotel_view');
