...
$res=pdo_fetch("SELECT * FROM ".tablename('nx_information_record')." WHERE weid=:weid AND recid=:recid",array(':weid'=>$_W['uniacid'],':recid'=>$recid));
//提交帮扶信息增加帮扶项目记录表单
if ($_W['ispost']) {
	if (checksubmit('submit')) {
		...
		if (!empty($res)) {
			...
			message('抱歉，提交失败！', $this -> createMobileUrl('record_info',array('recid'=>$recid)), 'error');
		}
...
include $this -> template('record_info');
