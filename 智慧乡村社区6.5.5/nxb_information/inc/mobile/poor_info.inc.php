...
$pid=intval($_GPC['pid']);
$res=pdo_fetch("SELECT * FROM ".tablename('nx_information_pinkuns')." WHERE weid=:weid AND pid=:pid",array(':weid'=>$_W['uniacid'],':pid'=>$pid));
//提交贫困户表单
if ($_W['ispost']) {
	if (checksubmit('submit')) {
		...
		$pid=intval($_GPC['pid']);
		$poor=pdo_fetch("SELECT * FROM ".tablename('nx_information_pinkuns')." WHERE pid=:pid ",array(':pid'=>$pid));
		$hid=intval($_GPC['hid']);
		...
		if (!empty($res)) {
			...
			message('抱歉，提交失败！', $this -> createMobileUrl('poor_info',array('pid'=>$pid)), 'error');
		}
...
include $this -> template('poor_info');
