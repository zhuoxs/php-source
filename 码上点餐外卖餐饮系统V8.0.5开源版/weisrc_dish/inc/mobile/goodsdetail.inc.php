...
$goodsdetail = pdo_fetch("SELECT * FROM " . tablename($this->table_goods) . " WHERE weid = '{$weid}' AND
storeid={$storeid} AND status = '1'
AND deleted=0 AND id=:id AND find_in_set(".$week.",week) ORDER BY displayorder DESC, subcount DESC, id DESC ", array(':id' => $id));
...
include $this->template($this->cur_tpl . '/goodsdetail');