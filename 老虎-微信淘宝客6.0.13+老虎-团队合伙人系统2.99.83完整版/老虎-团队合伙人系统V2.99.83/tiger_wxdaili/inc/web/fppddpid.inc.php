<?php
 global $_W,$_GPC;
		$pidlist=pdo_fetch("select * from ".tablename($this->modulename."_pddpid")." where weid='{$_W['uniacid']}' and type<>1 order by id desc ");
		if(empty($pidlist)){
			message('暂无可分配的PID！', $this -> createWebUrl('dlmember', array('op' => 'display')), 'success');
		}else{
		   $sharelist=pdo_fetchall("select * from ".tablename("tiger_newhu_share")." where weid='{$_W['uniacid']}' and  (dltype=1 || dltype=2)  and pddpid='' order by id desc ");
		   foreach($sharelist as $k=>$v){
		   	   $pidres=pdo_fetch("select * from ".tablename($this->modulename."_pddpid")." where weid='{$_W['uniacid']}' and type=0 order by id desc ");
					 if(empty($pidres)){
					 	message('可分配的PID不足！,请在生成一些PID，重新分配！', $this -> createWebUrl('pddpidlist', array('op' => 'display')), 'error');
					 }
