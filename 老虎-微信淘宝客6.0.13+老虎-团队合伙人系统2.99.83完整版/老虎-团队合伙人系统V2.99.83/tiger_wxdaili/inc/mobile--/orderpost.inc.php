...
global $_W, $_GPC;
       ...
       //授权验证
        $tbuid=$cfg['tbuid'];
        $tkurl=urlencode($_W['setting']['site']['url']);
        $tkip=urlencode($_SERVER["SERVER_ADDR"]);
        $sqaaa=$this->curl_request("http://api1.tigertaoke.com/sq.php?tbuid=".$tbuid."&tkurl=".$tkurl."&tkip=".$tkip."");
        $sq = @json_decode($sqaaa, true);
        if($sq['error']==2){
           echo $sq['msg'];
           exit;
        }
