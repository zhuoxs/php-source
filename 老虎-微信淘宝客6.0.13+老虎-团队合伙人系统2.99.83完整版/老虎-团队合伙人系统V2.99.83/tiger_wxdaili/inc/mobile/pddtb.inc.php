...
//	exit;	
	...
	foreach($orderlist as $k=>$v){
		$row = pdo_fetch("SELECT * FROM " . tablename('tiger_newhu_pddorder') . " WHERE weid='{$_W['uniacid']}' and order_sn='{$v['order_sn']}'");
		$data=array(
			"weid"=>$_W['uniacid'],
			"order_sn" =>$v['order_sn'],
            "goods_id" => $v['goods_id'],
		...
		if (!empty($row)){
            pdo_update("tiger_newhu_pddorder", $data, array('order_sn' => $v['order_sn'],'weid'=>$_W['uniacid']));
        }else{
            pdo_insert("tiger_newhu_pddorder", $data);
        }
	}
     ?>