...
//       }
         ...
         //今日付款笔数
         $dyaordercount= pdo_fetchcolumn("SELECT count(id) FROM " . tablename('tiger_newhu_tkorder')." where weid='{$_W['uniacid']}' and tgwid='{$share['tgwid']}' and addtime>'{$daytime}'"); 
         //昨日付款笔数
         $zraordercount= pdo_fetchcolumn("SELECT count(id) FROM " . tablename('tiger_newhu_tkorder')." where weid='{$_W['uniacid']}' and tgwid='{$share['tgwid']}' and addtime<'{$daytime}' and addtime>'{$zttime}'");
...
//       exit;
         ...
         if($cfg['newdlxjtype']==1){
         	include $this->template ( 'dlshouyi1' ); 
         }else{
         	include $this->template ( 'dlshouyi' );  
         }
         ?> 