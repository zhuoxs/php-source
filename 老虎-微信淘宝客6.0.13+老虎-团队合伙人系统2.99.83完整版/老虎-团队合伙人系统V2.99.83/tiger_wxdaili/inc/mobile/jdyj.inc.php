 <?php global $_W, $_GPC;
         ...
         $fs=jcbl($share,$bl);//粉丝比例和名称
         $fsbl=jdtqbl($share,$bl,$cfg,$day);
//       echo "<pre>";
//       print_r($fsbl);
//       exit;
						...
						function jdtqbl($share,$bl,$cfg,$day){//代理抽成模式  16.已付  17.已完成,18.已结算  $day=1 今天  2昨天  3本月  4上月
									global $_W;
						...
						if($day==1){//今天
						//今日付款笔数
						$dyaordercount= pdo_fetchcolumn("SELECT count(id) FROM " . tablename('tiger_newhu_jdorder')." where weid='{$_W['uniacid']}' and validCode=16  and spId='{$share['jdpid']}' and orderTime>'{$daytime}'"); 
						if(empty($dyaordercount)){
						$dyaordercount=0;
						}
							$data=array(
									'jd1'=>$dyaordercount,//本人-今天已付款订单数
									'jd2'=>$fsbl['j2'],//本人-今天已付款佣预估金
									'jd12'=>$fsbl['j3'],//二级-今天已付款佣预估金数
									'jd22'=>$fsbl['j4'],//三级-今天已付款佣预估金数
							);
							$dayname="今天数据,订单数：".$dyaordercount." 付款预估佣金：".$fsbl['j2']."二级付款预估佣金：".$fsbl['j3']."三级付款预估佣金：".$fsbl['j4'];
						}elseif($day==2){//昨天
						//昨日付款笔数
						$zraordercount= pdo_fetchcolumn("SELECT count(id) FROM " . tablename('tiger_newhu_jdorder')." where weid='{$_W['uniacid']}' and validCode=16  and spId='{$share['jdpid']}' and orderTime<'{$daytime}' and orderTime>'{$zttime}'");
						if(empty($zraordercount)){
							$zraordercount=0;
						}
							$data=array(
									'jd3'=>$zraordercount,//本人-昨天已付款订单
									'jd4'=>$fsbl['z2'],//本人-昨天已付款预估佣金数
									'jd14'=>$fsbl['z3'],//二级-昨天已付款预估佣金数
									'jd24'=>$fsbl['z4'],//三级-昨天已付款预估佣									
							);
							$dayname="昨天数据,订单数：".$zraordercount." 付款预估佣金：".$fsbl['z2']."二级付款预估佣金：".$fsbl['z3']."三级付款预估佣金：".$fsbl['z4'];
						}elseif($day==3){//本月
						//本月付款笔数
						$byaordercount= pdo_fetchcolumn("SELECT count(id) FROM " . tablename('tiger_newhu_jdorder')." where weid='{$_W['uniacid']}' and validCode=16 and spId='{$share['jdpid']}' and orderTime>'{$b_time}'");
						if(empty($byaordercount)){
							$byaordercount=0;
						}
							$data=array(
// 									'jd5'=>$byaordercount,//本人-本月已付款订单数
// 									'jd6'=>$fsbl['d2']+$fsbl['b2'],//本人-本月已付款预估佣金数
// 									'jd16'=>$fsbl['d3']+$fsbl['b3'],//二级-本月已付款预估佣金数
// 									'jd26'=>$fsbl['d4']+$fsbl['b4'],//三级-本月已付款预估佣金数		
								'jd5'=>$byaordercount,//本人-本月已付款订单数
								'jd6'=>$fsbl['d2'],//本人-本月已付款预估佣金数
								'jd16'=>$fsbl['d3'],//二级-本月已付款预估佣金数
								'jd26'=>$fsbl['d4'],//三级-本月已付款预估佣金数	
								'jd32'=>$fsbl['b2'],//本人-本月已付款预估佣金数
								'jd34'=>$fsbl['b3'],//二级-本月已付款预估佣金数
								'jd36'=>$fsbl['b4'],//三级-本月已付款预估佣金数
							);
							$dayname="本月数据,订单数：".$byaordercount." 付款预估佣金：".$fsbl['d2']."二级付款预估佣金：".$fsbl['d3']."三级付款预估佣金：".$fsbl['d4'];
						}elseif($day==4){//上月
						//上月付款笔数
						$syaordercount= pdo_fetchcolumn("SELECT count(id) FROM " . tablename('tiger_newhu_jdorder')." where weid='{$_W['uniacid']}' and spId='{$share['jdpid']}' and orderTime>'{$sy_time}' and orderTime<'{$b_time}' and (validCode=17 || validCode=18)");
						...
						if(empty($syaordercount)){
							...
							$data=array(
									'jd7'=>$syaordercount,//本人-上月已结算订单数
									'jd8'=>$fsbl['s2'],//本人-上月已结算预估佣金数
									'jd18'=>$fsbl['s3'],//二级-上月已结算预估佣金数
									'jd28'=>$fsbl['s4'],//三级-上月已结算预估佣金数
									'jd10'=>$fsbl['s6'],//本人-上月已付款预估佣金数
									'jd20'=>$fsbl['s7'],//二级-上月已付款预估佣金数
									'jd30'=>$fsbl['s8'],//三级-上月已付款预估佣金数
							);
						...
						exit(json_encode(array("【京东】 UID:".$uid.$dayname.$res,$data)));
