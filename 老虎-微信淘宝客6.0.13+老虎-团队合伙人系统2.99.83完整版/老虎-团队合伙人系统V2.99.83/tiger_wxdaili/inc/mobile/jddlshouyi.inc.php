...
//      		exit;
        ...
        $fans['tkuid']=$share['id'];
	     ...
	     include IA_ROOT . "/addons/tiger_wxdaili/inc/sdk/jdjs.php"; 
         $fs=jcbl($share,$bl);//粉丝比例和名称
         $fsbl=jdtqbl($share,$bl,$cfg);
//       echo "<pre>";
//       print_r($fsbl);
//       exit;
...
//       }
         ...
         //今日付款笔数
         $dyaordercount= pdo_fetchcolumn("SELECT count(id) FROM " . tablename('tiger_newhu_jdorder')." where weid='{$_W['uniacid']}' and p_id='{$share['jdpid']}' and orderTime>'{$daytime}'"); 
         if(empty($dyaordercount)){
         	$dyaordercount=0;
         }
         //昨日付款笔数
         $zraordercount= pdo_fetchcolumn("SELECT count(id) FROM " . tablename('tiger_newhu_jdorder')." where weid='{$_W['uniacid']}' and p_id='{$share['jdpid']}' and orderTime<'{$daytime}' and addtime>'{$zttime}'");
        if(empty($zraordercount)){
         	$zraordercount=0;
         }
        //今日付款佣金
        $jrygsum=$this->jdbryj($share,$daytime,'',3,$bl,$cfg);
        $jrygsum=number_format($jrygsum,2,".","");
        if(empty($jrygsum)){
        	$jrygsum="0.00";
        }
        //昨天付款佣金
        $zrygsum=$this->jdbryj($share,$zttime,$daytime,3,$bl,$cfg);
...
//       exit;
         ...
         if($cfg['newdlxjtype']==1){
         	include $this->template ( 'jddlshouyi1' );   
         }else{
         	include $this->template ( 'jddlshouyi' );   
         }
//        include $this->template ( 'jddlshouyi' );   
         ?> 