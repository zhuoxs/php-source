...
// 				 
				 ...
				 $dyaordercount= pdo_fetchcolumn("SELECT count(id) FROM " . tablename('tiger_newhu_pddorder')." where weid='{$_W['uniacid']}' and p_id='{$share['pddpid']}' and order_pay_time>'{$daytime}' and (order_status_desc='已成团' || order_status_desc='审核通过 ')"); 
				 ...
				 $zraordercount= pdo_fetchcolumn("SELECT count(id) FROM " . tablename('tiger_newhu_pddorder')." where weid='{$_W['uniacid']}' and p_id='{$share['pddpid']}' and order_pay_time<'{$daytime}' and order_pay_time>'{$zttime}' and (order_status_desc='已成团' || order_status_desc='审核通过')");
				 ...
				 $byaordercount= pdo_fetchcolumn("SELECT count(id) FROM " . tablename('tiger_newhu_pddorder')." where weid='{$_W['uniacid']}' and p_id='{$share['pddpid']}' and order_pay_time>'{$b_time}' and (order_status_desc='已成团' || order_status_desc='审核通过')");
				 if(empty($byaordercount)){
					$byaordercount=0;
				 	...
				 	$data=array(
				 			...
				 			'pdd16'=>$fsbl['d3']+$fsbl['b'],//二级-本月已付款预估佣金数
				 ...
				 $syaordercount= pdo_fetchcolumn("SELECT count(id) FROM " . tablename('tiger_newhu_pddorder')." where weid='{$_W['uniacid']}' and p_id='{$share['pddpid']}' and order_pay_time>'{$sy_time}' and order_pay_time<'{$b_time}' and (order_status_desc='确认收货' || order_status_desc='已结算')");
