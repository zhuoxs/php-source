 <?php     
	...
	if(empty($uid)){
 	...
 	$member=pdo_fetchall("select id,nickname,jdpid,tztime,helpid from ".tablename('tiger_newhu_share')." where weid='{$_W['uniacid']}' and dltype=1");
 	//echo "<pre>";
 		//echo 111;
 		//echo $dllist;
 	//print_r($member);
				...
				foreach ($mids as $valueone) {
					foreach ($members as $key => $valuetwo) {
						if($valuetwo['helpid']==$valueone){
							...
							$shareteam['jdpid']=$valuetwo['jdpid'];
...
// 		exit;
		...
		$tzyjdata=array(
			...
			'jdbyfkprice'=>$byyjsum+$xjyjs['byfkyj'],
			'jdsyjsprice'=>$syyjsum+$xjyjs['syjsyj'],
			'jdjrfkprice'=>$jrfkyj+$xjyjs['jrfkyj'],
			'jdzrfkprice'=>$zrfkyj+$xjyjs['zrfkyj'],
			'createtime'=>time()
		);
		//echo "<pre>";
		//print_r($tzyjdata);
		...
		function yjsun($shareid,$sharetztime,$appset,$weid,$qdtype=0){
		    ...
		    //本月付款
			$byyj = pdo_fetchcolumn("select SUM(estimateFee) from ".tablename("tiger_newhu_jdorder")."  where spId in (select jdpid from ".tablename("tiger_newhu_share")." where weid='{$weid}' and dltype=1 and helpid='{$shareid}') and orderTime>'{$b_time}'   and (validCode=17 || validCode=18) and weid='{$weid}'");//and addtime>='{$sharetztime}'
			 ...
			 //上月结算佣金
			 $syyj=pdo_fetchcolumn("select SUM(estimateFee) from ".tablename("tiger_newhu_jdorder")." where spId in (select jdpid from ".tablename("tiger_newhu_share")." where weid='{$weid}' and dltype=1 and helpid='{$shareid}') and orderTime<='{$b_time}' and orderTime>='{$sy_time}'  and (validCode=17 || validCode=18) and weid='{$weid}'");
			 ...
			 //今日付款佣金
			 $jrfkyj = pdo_fetchcolumn("select SUM(estimateFee) from ".tablename("tiger_newhu_jdorder")."  where spId in (select jdpid from ".tablename("tiger_newhu_share")." where weid='{$weid}' and dltype=1 and helpid='{$shareid}') and orderTime>='{$daytime}'  and (validCode=17 || validCode=18) and weid='{$weid}' ");//and addtime>='{$sharetztime}'
			...
			//昨日付款
			$zrfkyj = pdo_fetchcolumn("select SUM(estimateFee) from ".tablename("tiger_newhu_jdorder")."  where spId in (select jdpid from ".tablename("tiger_newhu_share")." where weid='{$weid}' and dltype=1 and helpid='{$shareid}') and orderTime>='{$daytime}' and  orderTime<='{$zttime}' and (validCode=17 || validCode=18)  and weid='{$weid}'");// and addtime>'{$sharetztime}'
