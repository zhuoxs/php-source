...
$data = array('i'=>$weid,'c'=>'entry','do'=>'appqudaosq','m'=>'tiger_wxdaili','code'=>$_GPC['code'],'state'=>$_GPC['state']);
...
$fh=$output;
if($fh=="OK"){
	$url=$url."?i=".$weid."&c=entry&do=msg&m=tiger_wxdaili&error=0";
  header("location:".$url);
}else{
	$url=$url."?i=".$weid."&c=entry&do=msg&m=tiger_wxdaili&error=1";
  header("location:".$url);
}
