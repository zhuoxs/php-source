...
//     exit;
       ...
       //print_r($bl);
		...
		if($_GPC['rj']==1){
			...
			if(empty($zxshare['yaoqingma'])){
				$yaoqingma=$share['yaoqingma'];
			}else{
				$yaoqingma=$zxshare['yaoqingma'];
			}
          ...
          if($lm==1){//联盟产品
             $views['itemid']=$_GPC['itemid'];
						 if($_GPC['price']){
							 ...
							 $views['itemtitle']=$_GPC['itemtitle'];
...
//             exit;
             ...
             $res=hqyongjin($turl,$ck,$cfg,$this->modulename,'','',$tksign['sign'],$tksign['tbuid'],$_W,1,$views['itemid'],$qdid);
...
//            exit;
      ...
      //二维码图片链接
        ...
        if(empty($cfg['itemewm'])){
	    	...
	    	$ewmurl=$_W['siteroot'].str_replace('./','app/',$this->createMobileurl('tklview',array('itemid'=>$views['itemid'],'itemendprice'=>$views['itemendprice'],'couponmoney'=>$views['couponmoney'],'itempic'=>urlencode($views['itempic']),'tkl'=>$views['taokouling'],'itemprice'=>$views['itemprice'])))."&rhyurl=".$rhyurl."&yaoqingma=".$yaoqingma."&yj=".$flyj."&itemtitle=".urlencode($views['itemtitle']);
	    	//$url=urlencode($url);
        ...
        }elseif($cfg['itemewm']==6){//微信防封中间页
			$taokou=str_replace("￥","",$views['taokouling']);
			$taokou1="(".$taokou.")";
			$content=urlencode("点击右上角...【复制链接】或长按复制这段文字，打开『手淘APP』即可购买").$taokou1;
			
        	$ewmurl="http://rd.wechat.com/qrcode/confirm?block_type=101&content=".$content."&lang=zh_CN&scene=4&weixin=".$taokou1;   
        }
      ...
      $scview= pdo_fetch("SELECT * FROM " . tablename($this->modulename."_shoucang") . " WHERE weid = '{$_W['uniacid']}' and uid='{$zxshare['id']}' and goodsid='{$views['itemid']}'");//有没有收藏
      ...
      if($_GPC['fromid']==1){
      	...
      	exit(json_encode(array('data' =>$views)));
			...
			if($views['couponendtime']){
				$views['day']=date(d,$views['couponendtime']);
				$views['month']=date(m,$views['couponendtime']);
				$views['year']=date(Y,$views['couponendtime']);
			}else{
				$views['couponendtime']=time()+172800;
				$views['day']=date(d,$views['couponendtime']);
				$views['month']=date(m,$views['couponendtime']);
				$views['year']=date(Y,$views['couponendtime']);
			}
			if(empty($views['couponmoney']) || $views['couponmoney']=='undefined'){
				 $views['couponmoney']=0;
			}
// 			
// 		
...
// 			exit;
        ...
        if($cfg['qdtype']==1){
			if($zxshare['dltype']==1){
				if($zxshare['qdid']==0){
					...
					//$tksign = pdo_fetchall("SELECT * FROM " . tablename("tiger_newhu_tksign") . " WHERE  tbuid='{$cfg['tbuid']}'");
					if(!empty($_SESSION["tksign"])){
						$arr=getqudaolist($_SESSION["tksign"],$_W['uniacid']);	
					}					
					$rtag="tiger".$zxshare['id'];
					$rid = pdo_fetch("select * from ".tablename("tiger_newhu_qudaolist")." where weid='{$_W['uniacid']}' and rtag='{$rtag}'");
					if(!empty($rid['relation_id'])){
						if(empty($zxshare['tgwid'])){
							$b=pdo_update("tiger_newhu_share",array('qdid'=>$rid['relation_id'],'dlptpid'=>$rid['root_pid'],'tgwid'=>11111), array ('id' =>$zxshare['id']));
						}else{
							$b=pdo_update("tiger_newhu_share",array('qdid'=>$rid['relation_id'],'dlptpid'=>$rid['root_pid']), array ('id' =>$zxshare['id']));
						}
						$zxshare['qdid']=$rid['relation_id'];
					}else{
						...
						$qudaourl=$tksign['qdtgurl']."&rtag=".$rtag;
						//echo $qudaourl;
        ...
        include $this->template ( 'tbgoods/style88/view' );
?>