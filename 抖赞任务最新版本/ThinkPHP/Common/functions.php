...
function U($url = '', $vars = '', $suffix = true, $domain = false)
{
    ...
    if (C('URL_MODEL') == 0) {
        ...
        if (isset($route)) {
            ...
            //by dav 显示默认模块名称
            $url    = __APP__ . '/' . ($module ? $module . MODULE_PATHINFO_DEPR : C('DEFAULT_MODULE') . MODULE_PATHINFO_DEPR) . implode($depr, array_reverse($var));
        }
        if ($urlCase) {
            $url = strtolower($url);
        }
    ...
    if(C('URL_ROUTER_ON')){
        foreach (C('URL_ROUTE_RULES') as $rule=>$real) {
            if(strpos($url, $real)!==false){
                ...
                $url = str_replace("/".C('DEFAULT_MODULE'), '', $url);
