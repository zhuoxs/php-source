...
$info=pdo_getall('ymmf_sun_information',array('id'=>$_GPC['id']));
$type=pdo_getall('ymmf_sun_type',array('uniacid'=>$_W['uniacid']));
if($info['img']){
			if(strpos($info['img'],',')){
...
if(checksubmit('submit')){
		if($_GPC['img']){
			...
			$data['sh_time']=time();
				...
				$res = pdo_insert('ymmf_sun_information', $data);
				if($res){
					message('新增成功',$this->createWebUrl('ininformation',array()),'success');
				}else{
					message('新增失败','','error');
				}
		}
include $this->template('web/inaddinformation');