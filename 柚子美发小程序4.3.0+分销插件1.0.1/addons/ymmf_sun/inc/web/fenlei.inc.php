...
//$list = pdo_getall('ymmf_sun_type',array('uniacid' => $_W['uniacid']),array(),'','num ASC');
    $type=pdo_getall('ymmf_sun_type',array('uniacid'=>$_W['uniacid']),array(),'','id asc');
    $type2=pdo_getall('ymmf_sun_type2',array('uniacid'=>$_W['uniacid']),array(),'','id asc');
    $sql=" select * from".tablename('ymmf_sun_label')." where type2_id in (select id from ".tablename('ymmf_sun_type2')." where uniacid={$_W['uniacid']})";
...
if($_GPC['op']=='change'){
	 $res=pdo_update('ymmf_sun_type',array('state'=>$_GPC['state']),array('id'=>$_GPC['id']));
    if($res){
...
if($_GPC['op']=='change2'){
     $res=pdo_update('ymmf_sun_type2',array('state'=>$_GPC['state']),array('id'=>$_GPC['id']));
    if($res){
        message('操作成功',$this->createWebUrl('fenlei',array()),'success');
    }else{
        message('操作失败','','error');
