...
$type2=pdo_get('ymmf_sun_storetype2',array('id'=>$info['storetype2_id']));
//查出已是商家用户
...
if(checksubmit('submit')){
		...
		if($_GPC['user_name']){
			$rst=pdo_get('ymmf_sun_store',array('user_name'=>$_GPC['user_name'],'uniacid'=>$_W['uniacid'],'id !='=>$_GPC['id']));
			if($rst){
		 ...
		 function  getCoade($md_id){
          ...
          function set_msg($md_id){
           ...
           $data2=array(
            "scene"=>$md_id,
            "page"=>"ymmf_sun/pages/sellerinfo/sellerinfo",
            "width"=>100
            );
