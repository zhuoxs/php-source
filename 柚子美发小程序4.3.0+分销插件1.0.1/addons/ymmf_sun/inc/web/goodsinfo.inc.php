...
//$id = 1;
//$sql1 = ' SELECT * FROM ' . tablename('ymmf_sun_gallery') . ' g ' . ' JOIN ' . tablename('ymmf_sun_hairers') .' h ' . ' ON ' . ' g.hair_id=h.id'. ' WHERE ' . ' h.id='.$id;
//$hairData = pdo_fetch($sql1);
//p($hairData);die;


//
$info=pdo_get('ymmf_sun_goods',array('id'=>$_GPC['id']));

$spec=pdo_getall('ymmf_sun_goods_spec',array('uniacid'=>$_W['uniacid']));
$type=pdo_getall('ymmf_sun_type',array('uniacid'=>$_W['uniacid'],'state'=>1));
$hair = pdo_getall('ymmf_sun_hairers',array('uniacid'=>$_W['uniacid']));
if($info['imgs']){
    $imgs = $info['imgs'];
}
if($info['zs_imgs']){
...
//            p($data);die;
			if(empty($_GPC['id'])){
                ...
                $res = pdo_insert('ymmf_sun_goods', $data);
            }else{

                $res = pdo_update('ymmf_sun_goods', $data, array('id' => $_GPC['id']));
            }
				if($res){
			        pdo_update('ymmf_sun_service',array('option'=>$_GPC['goods_name'],'price'=>$_GPC['goods_price']),array('uniacid'=>$_W['uniacid'],'goods_id'=>$_GPC['id']));
