...
$info=pdo_get('ymmf_sun_yellowstore',array('id'=>$_GPC['id']));
...
//入住类型
$typein=pdo_getall('ymmf_sun_yellowset',array('uniacid'=>$_W['uniacid']));	
if($info['imgs']){
			if(strpos($info['imgs'],',')){
        ...
        if(checksubmit('submit')){
            ...
            //查找入驻天数
            $days=pdo_get('ymmf_sun_yellowset',array('id'=>$_GPC['rz_type']));
            $data['dq_time']=time()+$days['days']*60*60*24;
        	$res = pdo_insert('ymmf_sun_yellowstore', $data);
        	if($res){
