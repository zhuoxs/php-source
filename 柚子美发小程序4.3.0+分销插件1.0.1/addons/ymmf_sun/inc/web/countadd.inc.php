...
//$city = pdo_fetchall("SELECT * FROM " . tablename('ymmf_sun_hotcity') . " WHERE uniacid= :weid ORDER BY id DESC", array(':weid' =>$_W['uniacid']), 'id');
$city = pdo_fetchall("SELECT DISTINCT cityname FROM " . tablename('ymmf_sun_hotcity') . " WHERE uniacid= :weid ORDER BY id DESC", array(':weid' =>$_W['uniacid']), 'id');
    $id = intval($_GPC['id']);
    //echo $id;die;
    $storeid = intval($_GPC['storeid']);
    if (!empty($id)) {
        $account = pdo_fetch("SELECT * FROM " . tablename('ymmf_sun_account') . " WHERE weid = :weid AND id=:id ORDER BY id DESC", array(':weid' =>$_W['uniacid'], ':id' => $id));
        ...
        if (empty($users)) {
            ...
            if ($uid > 0) {
                ...
                pdo_insert('users_permission', array(
                    ...
                    'url' => '',
                    'type' => 'ymmf_sun',
                    'permission' => 'ymmf_sun_menu_stores2'
                ));



                pdo_insert('ymmf_sun_account', array(
                    'uid' => $uid,
                    'weid' =>$_W['uniacid'],
                    'storeid' => intval($_GPC['storeid']),
            ...
            user_update($user);
            pdo_update('ymmf_sun_account', array(
                'weid' =>$_W['uniacid'],
                'storeid' => intval($_GPC['storeid']),
                'from_user' => trim($_GPC['from_user']),
                'email' => trim($_GPC['email']),
