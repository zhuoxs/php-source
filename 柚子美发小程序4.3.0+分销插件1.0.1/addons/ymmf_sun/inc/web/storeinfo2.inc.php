...
$info=pdo_get('ymmf_sun_store',array('id'=>$_GPC['id']));
$area=pdo_getall('ymmf_sun_area',array('uniacid'=>$_W['uniacid']));
//查出已是商家用户
...
//行业
$type=pdo_getall('ymmf_sun_storetype',array('uniacid'=>$_W['uniacid'],'state'=>1),array(),'','num asc');
//入住类型
//$typein=pdo_getall('ymmf_sun_in',array('uniacid'=>$_W['uniacid']));
...
//		}
		...
		//查询钱
		$storetype_id=pdo_get('ymmf_sun_storetype',array('id'=>$_GPC['storetype_id']));
		$typemoney=pdo_get('ymmf_sun_in',array('id'=>$_GPC['type']));
			...
			$data['cityname']=$system['cityname'];
				$res = pdo_insert('ymmf_sun_store', $data);
				if($res){
				...
				$res = pdo_update('ymmf_sun_store', $data,array('id'=>$_GPC['id']));
				if($res){
