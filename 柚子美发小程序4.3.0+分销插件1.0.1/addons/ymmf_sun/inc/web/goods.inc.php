...
$type=isset($_GPC['type'])?$_GPC['type']:'all';
 $data[':uniacid']=$_W['uniacid'];
  $sql="select a.*,b.store_name from " . tablename("ymmf_sun_goods") . " a"  . " left join " . tablename("ymmf_sun_store") . " b on a.store_id=b.id" .$where."  order by a.time desc ";
  $total=pdo_fetchcolumn("select count(*) as wname from " . tablename("ymmf_sun_goods") . " a"  . " left join " . tablename("ymmf_sun_store") . " b on a.store_id=b.id".$where."  order by a.time desc ",$data);
...
if($_GPC['op']=='delete'){
    ...
    if($res){
        pdo_delete('ymmf_sun_service',array('goods_id'=>$_GPC['id']));
