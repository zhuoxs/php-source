...
$GLOBALS['frames'] = $this->getMainMenu();
    ...
    $id = $_GPC['id'];
	$info = pdo_get('ymmf_sun_gallery',array('id'=>$id));
//	p($info);die;
if($info['imgs']){
	...
	$type = pdo_getall('ymmf_sun_hairers',array('id' => $hair_id));
//	p($type);die;
...
//		    p($_GPC);die;
			...
			if($_GPC['id']==''){
				$res=pdo_insert('ymmf_sun_gallery',$data);
				...
				$res = pdo_update('ymmf_sun_gallery', $data, array('id' => $_GPC['id']));
				if($res){
