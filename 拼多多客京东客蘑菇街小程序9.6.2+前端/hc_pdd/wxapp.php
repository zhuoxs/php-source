...
//红包树注册
    ...
    public function doPageGoodslist(){
        ...
        $signold = $client_secret.'cat_id'.$category_id.'client_id'.$client_id.'data_type'.$data_type.'keyword'.$keyword.'page'.$page.'page_size10sort_type'.$sort_type.'timestamp'.$timestamp.'type'.$type.'with_couponfalse'.$client_secret;
        $sign = md5($signold);
    ...
    public function Goodslist($user_id,$pageNum,$sort_type){
        ...
        $timestamp = time();
        $type = 'pdd.ddk.goods.search';
        $signold = $client_secret.'client_id'.$client_id.'data_type'.$data_type.'page'.$page.'page_size10sort_type'.$sort_type.'timestamp'.$timestamp.'type'.$type.'with_couponfalse'.$client_secret;
        //推荐置顶商品
        $goodtop = $info['goodtop'];
        if($sort_type == -1){
            if (!empty($goodtop)){
                $goods_id_list = '['.$goodtop.']';
                $signold = $client_secret.'client_id'.$client_id.'data_type'.$data_type.'goods_id_list'.$goods_id_list.'page'.$page.'page_size10sort_type'.$sort_type.'timestamp'.$timestamp.'type'.$type.'with_couponfalse'.$client_secret;
                $sign = md5($signold);
                $sign = strtoupper($sign);
                $data = array (
                    'type' => urlencode('pdd.ddk.goods.search'),
        ...
        if($sort_type == 4){
            ...
            $signold = $client_secret.'client_id'.$client_id.'data_type'.$data_type.'page'.$page.'page_size10range_list'.$range_list.'sort_type'.$sort_type.'timestamp'.$timestamp.'type'.$type.'with_couponfalse'.$client_secret;
        }
        if($sort_type == 28){
    ...
    public function doPageTuijianlist(){
        ...
        if($_GPC['jump'] == 0){
            ...
            $promInfoQuery = json_encode(array('hasCoupon'=>'true','pageSize'=>5,'pageNo'=>$page,'sortType'=>0));
    ...
    public function doPageMogugoodslist(){
        ...
        $AppSecret = $info['app_secret'];
        //$promInfoQuery = '{"hasCoupon":true,"pageSize":20}';
        $promInfoQuery = json_encode(array('hasCoupon'=>'true','pageSize'=>5,'pageNo'=>$page,'sortType'=>$sortType,'keyword'=>$_GPC['inputValue']));
    ...
    public function doPageThemedetail(){
        ...
        $this->result(0, '主题商品列表',array('list'=>$list,'img'=>array('img'=>$img),'tname'=>array('tname'=>$tname)));
    ...
    //官方主题列表
    public function doPageThemelistOne(){
        ...
        if($_GPC['parameter'] == 1){
            ...
            if($order_status == 0){
                ...
                $result = pdo_getslice('hcpdd_moguorders',$where,array($pageindex, $pagesize),$total,array(),'','orderTime DESC');
                 ...
                 $result = $query->from('hcpdd_moguorders')->where($where)->where('paymentStatus !=', '20000')->where('paymentStatus !=', '40000')->where('paymentStatus !=', '45000')->page($pageindex, $pagesize)->getall();
            }
        ...
        if($_GPC['parameter'] == 2){
            ...
            if($order_status == 0){
                $result = pdo_getslice('hcpdd_jdorders',array('uniacid' =>$_GPC['i'],'positionId'=>$arr['positionId']),array($pageindex, $pagesize),$total,array(),'','orderTime DESC');
                    ...
                    $query = load()->object('query');
                    $result = $query->from('hcpdd_jdorders')->where($where)->where('validCode !=', '16')->where('validCode !=', '17')->where('validCode !=', '18')->page($pageindex, $pagesize)->getall();
                    //                    $result = pdo_getslice('hcpdd_jdorders',$where,array($pageindex, $pagesize),$total,array(),'','orderTime DESC');
                }
...
//        $this->result(0, '推广订单列表',array('list'=>$result,'order_status'=>$order_status,'parameter'=>$_GPC['parameter'],'pageindex'=>$pageindex));
     }
    ...
    public function doPageHeadcolor(){
        ...
        //公告状态
        $notice_list = pdo_getall('hcpdd_notice', array('uniacid'=>$_GPC['i']),'id');
        $noticeuser_list = pdo_getall('hcpdd_noticeuser', array('uniacid'=>$_GPC['i'],'user_id'=>$user_id),'notice_id');
        foreach ($noticeuser_list as $k =>$no){
            $arr_no[] = $no['notice_id'];
        }
        $is_read = 0;
        if (empty($notice_list)){
            ...
            if (empty($noticeuser_list)){
                $is_read = 1;
            }else{
                foreach ($notice_list as $key =>$val){
                    if (!in_array($val['id'],$arr_no)){
                        $is_read = 1;
                        break;
                    }
                }
        ...
        return $this->result(0, '各种颜色',array('config'=>$config,'yesno'=>$yesno,'is_daili'=>$arr['is_daili'],'info'=>$info,'theme'=>$theme,'is_mobile'=>$is_mobile,'hongbao'=>$hongbao,'hb'=>$hb,'user'=>$arr,'banner'=>$banner,'nav'=>$nav,'show'=>$show,'icon'=>$icon,'is_read'=>$is_read));
    }

    //修改公告已读未读
    public function doPageIsread(){
        global $_GPC, $_W;
        $user_id = $_GPC['user_id'];

        $notice_list = pdo_getall('hcpdd_notice', array('uniacid'=>$_GPC['i']),'id');
        foreach ($notice_list as $key=>$val){
            $notice_one = pdo_get('hcpdd_noticeuser', array('uniacid'=>$_GPC['i'],'user_id'=>$user_id,'notice_id'=>$val['id']));
            if (empty($notice_one)){
                $data = array('uniacid'=>$_GPC['i'], 'notice_id'=>$val['id'],'user_id'=>$user_id,'createtime'=>time());
                pdo_insert('hcpdd_noticeuser', $data);
            }
        }
        $this->result(0, '修改成功',$_GPC);
    }
    ...
    public function doPageWithdraw(){
        ...
        $moguzisheng = pdo_getcolumn('hcpdd_moguorders',array('groupId'=>$user['gid'],'orderStatus !='=>80000,'paymentStatus !='=>10000,'paymentStatus !='=>90000,'fafang'=>0),array('sum(expense)'));//蘑菇街预估收入
        ...
        if($user['positionId']>0){
            $jdzisheng = pdo_getcolumn('hcpdd_jdorders',array('positionId'=>$user['positionId'],'validCode !='=>18,'validCode >'=>15,'fafang'=>0),array('sum(commission)'));//
        ...
        if($set['is_mogu'] == 1 and $set['is_jd'] ==1){
            ...
            $all = round($all,2);
            $tongji = array();
            $tongji[0]['zisheng'] = round($pddzisheng,2);
            $tongji[0]['fenxiao'] = round($pddfenxiao,2);
            $tongji[0]['he'] = round($pddzong,2);
            $tongji[1]['zisheng'] = round($jdzisheng,2);
            $tongji[1]['fenxiao'] = round($jdfenxiao,2);
            $tongji[1]['he'] = round($jdzong,2);
            $tongji[2]['zisheng'] = round($moguzisheng,2);
            $tongji[2]['fenxiao'] = round($mogufenxiao,2);
            $tongji[2]['he'] = round($moguzong,2);
    ...
    public function doPageNotice(){
        ...
        global $_GPC, $_W;
        $notice = pdo_getall('hcpdd_notice', array('enabled' => 1,'uniacid'=>$_GPC['i']),array(),'','createtime desc');
...
/*     foreach ($list as $k => $v) {
    ...
    public function doPageTreelist(){
        ...
        $info = pdo_get('hcpdd_set',array('uniacid'=>$_GPC['i']));
        $data['treeadultid'] = $info['treeadultid'];
    ...
    public function doPageCollect(){
        ...
        $pageindex = max(1, intval($_GPC['pageNum']));
        $pagesize = 20;
        $list = pdo_getslice('hcpdd_collect',array('uniacid'=>$_GPC['i'],'user_id'=>$user_id),array($pageindex, $pagesize),$total,array(),'','createtime DESC');

        foreach ($list as $key => &$value){
            $numbers = pdo_getall('hcpdd_collect',array('uniacid'=>$_GPC['i'],'goods_id'=>$value['goods_id']));
            $collect_num = count($numbers);
            $value['collect_num'] = $collect_num;
        }
        $this->result(0, '收藏列表',$list);
    }

    //商品收藏
    public function doPageCollectpost(){
        global $_W,$_GPC;
        //添加收藏
        if($_GPC['act']=='add'){
            ...
            $data['now_price'] = $_GPC['now_price'];
            $data['coupon_discount'] = $_GPC['coupon_discount'];
            $data['goods_thumbnail_url'] = $_GPC['goods_thumbnail_url'];
            $data['itemUrl'] = $_GPC['itemUrl'];
            $data['skuId'] = $_GPC['skuId'];
            $data['parameter'] = $_GPC['parameter'];
            $data['sold_quantity'] = $_GPC['sold_quantity'];
            $data['couponUrl'] = $_GPC['couponUrl'];
            $data['materialUrl'] = $_GPC['materialUrl'];
            $data['createtime'] = time();
            $result = pdo_insert('hcpdd_collect', $data);
            if (!empty($result)) {
                $this->result(0, '收藏成功');
            }
        }
        //删除
        if($_GPC['act']=='del'){
            $goods_id = $_GPC['goods_id']; //拼多多商品
            $user_id = $_GPC['user_id'];
            $parameter = $_GPC['parameter'];
            $itemUrl = $_GPC['itemUrl']; //蘑菇街商品
            $skuId = $_GPC['skuId']; //京东商品
            if ($parameter == 0){
                $result = pdo_delete('hcpdd_collect', array('uniacid'=>$_GPC['i'],'goods_id'=>$goods_id,'user_id'=>$user_id,'parameter'=>$parameter));
            }elseif ($parameter == 1){
                $result = pdo_delete('hcpdd_collect', array('uniacid'=>$_GPC['i'],'itemUrl'=>$itemUrl,'user_id'=>$user_id,'parameter'=>$parameter));
            }else{
                $result = pdo_delete('hcpdd_collect', array('uniacid'=>$_GPC['i'],'skuId'=>$skuId,'user_id'=>$user_id,'parameter'=>$parameter));
            }
            if (!empty($result)) {
                $this->result(0, '删除成功');
            }
    ...
    //是否有收藏
    public function doPageIscollect(){
        global $_GPC,$_W;
        $goods_id = $_GPC['goods_id']; //拼多多商品
        $user_id = $_GPC['user_id'];
        $parameter = $_GPC['parameter'];
        ...
        $skuId = $_GPC['skuId']; //京东商品
        if ($parameter == 0){
            $res = pdo_get('hcpdd_collect', array('uniacid'=>$_GPC['i'],'user_id'=>$user_id,'goods_id'=>$goods_id,'parameter'=>$parameter));
        }elseif ($parameter == 1){
            $res = pdo_get('hcpdd_collect', array('uniacid'=>$_GPC['i'],'itemUrl'=>$itemUrl,'user_id'=>$user_id,'parameter'=>$parameter));
        }else{
            $res = pdo_get('hcpdd_collect', array('uniacid'=>$_GPC['i'],'skuId'=>$skuId,'user_id'=>$user_id,'parameter'=>$parameter));
        }
        $is_collect = 1;
        if (empty($res)){
            $is_collect = 0;
        }
        $this->result(0, '是否收藏',$is_collect);
    }

    //红包树提现方式
    public function doPageTreeway(){
        global $_GPC,$_W;
        $set = pdo_get('hcpdd_set',array('uniacid'=>$_GPC['i']));
        if (empty($set['tree_way'])){
            $tree_way = 0;
        }else{
            $tree_way = 1;
        }
        $user_id=$_GPC['user_id'];
        $user = pdo_get('hcpdd_users', array('user_id' => $user_id));
        $this->result(0, '红包树提现方式',array('tree_way'=>$tree_way,'totalmoney'=>$user['treemoney'],'user'=>$user['$user']));
    }
    //红包树支付宝提现
    public function doPageTreetixianzfb()
    {
        ob_end_clean();
        global $_GPC, $_W;
        $user_id=$_GPC['user_id'];
        ...
        $data['tel']=$_GPC['tel'];
        $data['zhifubao']=$_GPC['weixin'];
        $data['money']=$_GPC['tmoney'];
        $data['truename']=$_GPC['name'];
        $data['createtime']=time();
        $data['status']=0;
        $res = $this->isMobile($_GPC['tel']);
        if ($res == false){
            return $this->result(1, '请输入正确的手机号',$data);
        }
        if($data['money'] <= 0){
            return $this->result(1, '请输入正确金额',$data);
        }
        if($user['treemoney'] < $data['money']){
            return $this->result(1, '可提现金额不足',$data);
        }
        elseif($data['money'] < $set['min_treetxmoney']){
            ...
            $result = pdo_insert('hcpdd_treewithzfb', $data);
            $nowmoney     = $user['treemoney'] - $data['money'];
            /*$update =array(
                    'waitmoney' => $nowwaitmoney,
                    'money'     => $nowmoney
