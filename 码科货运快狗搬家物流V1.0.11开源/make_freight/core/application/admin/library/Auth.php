...
class Auth extends \fast\Auth
{
    ...
    {
        return Session::get('freight_admin.' . $name);
    }
    ...
    public function login($username, $password, $keeptime = 0)
    {
        ...
        $admin->save();
        Session::set("freight_admin", $admin->toArray());
    ...
    public function getUserInfo($uid = null)
    {
        ...
        return $uid != $this->id ? Admin::get(intval($uid)) : Session::get('freight_admin');
    }
...
//            $v['pinyin'] = $pinyin->permalink($v['title'], '');
            ...
            $referer = tp_url($v['url']) == $refererUrl ? $v : $referer;
        }
        if ($selected == $referer) {
            $referer = [];
        }
        $selected && $selected['url'] = tp_url($selected['url']);
        $referer && $referer['url'] = tp_url($referer['url']);
        ...
        if (Config::get('fastadmin.multiplenav')) {
            ...
            foreach ($topList as $index => $item) {
                ...
                $url = $childList ? 'javascript:;' : tp_url($item['url']);
