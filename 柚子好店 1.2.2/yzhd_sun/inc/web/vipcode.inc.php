...
if($_GPC['op']=='create'){
    //获取VIP列表
    $viplist=pdo_getall('yzhd_sun_vip',array('uniacid'=>$_W['uniacid'],'status'=>1));

    $template = "web/vipcodeadd";
}elseif($_GPC['op']=='delete'){
    if($_W['ispost']){
        $res=pdo_delete('yzhd_sun_vipcode',array('id'=>$_GPC['id'],'uniacid'=>$_W['uniacid']));
        if($res){
    ...
    $vipid = $_GPC["vipid"];
    $viplist=pdo_get('yzhd_sun_vip',array('uniacid'=>$_W['uniacid'],'id'=>$_GPC['vipid']),array("prefix"));
    ...
    if($lavenum>0){
        ...
        for($i=0;$i<$thatnum;$i++){
            ...
            $vcarr=pdo_get('yzhd_sun_vipcode',array('uniacid'=>$_W['uniacid'],'vc_code'=>$data["vc_code"]),array("id"));
            if(!$vcarr){
                $res=pdo_insert('yzhd_sun_vipcode',$data);
            }
            unset($data);
        }
        if($thatnum<10){
    ...
    $pagesize=10;
    $data[':uniacid']=$_W['uniacid'];
    $sql="select c.id,v.title,v.price,c.vc_starttime,c.vc_endtime,c.vc_code,c.vc_isuse,c.uid,u.name from " . tablename("yzhd_sun_vipcode") ." as c left join " . tablename("yzhd_sun_vip") ." as v on c.vipid=v.id left join " . tablename("yzhd_sun_user") ." as u on u.openid=c.uid {$where} order by c.id desc ";
    $total=pdo_fetchcolumn("select count(c.id) as wname from " . tablename("yzhd_sun_vipcode") . " as c left join " . tablename("yzhd_sun_vip") ." as v on c.vipid=v.id {$where} order by c.id desc ",$data);
