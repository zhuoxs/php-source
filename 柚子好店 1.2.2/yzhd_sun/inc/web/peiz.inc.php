...
$item=pdo_get('yzhd_sun_system',array('uniacid'=>$_W['uniacid']));
    if(checksubmit('submit')){
        ...
        if(is_uploaded_file($_FILES['apiclient_cert']['tmp_name'])) {
            ...
            //我们给每个用户动态的创建一个文件夹
            $user_path=IA_ROOT."/addons/yzhd_sun/cert/";
...
//            $data['apiclient_cert']=trim($_GPC['apiclient_cert']);
            $data['apiclient_cert']=$apiclient_cert['name'] . rand(1,9);
...
//            }
            ...
            if($_GPC['id']==''){                
                ...
                $res = pdo_update('yzhd_sun_system', $data, array('id' => $_GPC['id'],'uniacid'=>$_W['uniacid']));
                if($res){
