...
$info=pdo_get('yzhd_sun_information',array('id'=>$_GPC['id']));
$type=pdo_getall('yzhd_sun_type',array('uniacid'=>$_W['uniacid'],'state'=>1),array(),'','num asc');
//$type=pdo_get('yzhd_sun_type',array('id'=>$info['type_id']));
$type2=pdo_get('yzhd_sun_type2',array('id'=>$info['type2_id']));
if($info['img']){
	if(strpos($info['img'],',')){
...
if(checksubmit('submit')){
	...
	$res = pdo_update('yzhd_sun_information', $data, array('id' => $_GPC['id']));
	if($res){
