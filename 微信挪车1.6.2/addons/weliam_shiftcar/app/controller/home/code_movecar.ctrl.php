...
if($_W['ispost']){
	...
	if(empty($movecode)){
		die(json_encode(array("result" => 2,"msg" => '挪车用户码为空，请重新输入！')));
	}
	...
	$carmember = wl_mem_single(array('uniacid' => $_W['uniacid'],'ncnumber' => $movecode));
	$carcard = $carmember['plate1'].$carmember['plate2'].$carmember['plate_number'];
	
	if(empty($carmember)){
		die(json_encode(array("result" => 2,"msg" => '未找到车主，请检查用户码是否正确！')));
	}
