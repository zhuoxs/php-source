...
$info=pdo_get('byjs_sun_active',array('uniacid'=>$_W['uniacid'],'id'=>$_GPC['id']));
$id = $_GPC['id'];
if (!empty($id)) {
    $item = pdo_fetch("SELECT *  FROM " . tablename('byjs_sun_active') . " WHERE id = :id", array(':id' => $id));
...
if(checksubmit('submit')){
        ...
        if (!empty($id)) {
            ...
            pdo_update('byjs_sun_active', $data, array('id' => $id,'uniacid'=>$_W['uniacid']));
        } else {
            pdo_insert('byjs_sun_active', $data);
            $id = pdo_insertid();
        }
