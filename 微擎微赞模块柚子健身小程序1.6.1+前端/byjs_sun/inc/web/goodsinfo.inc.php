...
$info=pdo_get('byjs_sun_goods',array('id'=>$_GPC['id']));

$spec=pdo_getall('byjs_sun_goods_spec',array('uniacid'=>$_W['uniacid']));
$type=pdo_getall('byjs_sun_type',array('uniacid'=>$_W['uniacid'],'state'=>1));
    ...
    if(empty($_GPC['goods_name'])){
		...
		if($_GPC['lb_imgs']){
			...
			if(empty($_GPC['id'])){
                ...
                $res = pdo_insert('byjs_sun_goods', $data);
            }else{

                $res = pdo_update('byjs_sun_goods', $data, array('id' => $_GPC['id'],'uniacid'=>$_W['uniacid']));
            }
				if($res){
