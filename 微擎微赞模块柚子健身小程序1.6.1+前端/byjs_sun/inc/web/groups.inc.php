...
$info=pdo_get('byjs_sun_groups',array('uniacid' => $_W['uniacid']));
...
$sql = 'SELECT COUNT(*) FROM ' . tablename('byjs_sun_groups') .$condition ;

$total = pdo_fetchcolumn($sql, $params);

if (!empty($total)) {
    $sql = 'SELECT * FROM  ' . tablename('byjs_sun_groups') .$condition.' ORDER BY  `selftime`  DESC LIMIT ' . ($pindex - 1) * $psize . ',' . $psize;
...
if($operation == 'done'){
    $id = intval($_GPC['id']);
    $row = pdo_fetch("SELECT id FROM " . tablename('byjs_sun_groups') . " WHERE id = :id", array(':id' => $id));
    if (empty($row)) {
    ...
    if($status == 1)
    {
        ...
        $active = pdo_fetch("SELECT id FROM " . tablename('byjs_sun_groups') .$condition , $params);

        pdo_update('byjs_sun_groups', array('status'=>$status), array('id' => $id));
...
if($operation == 'delete'){
    ...
    $res = pdo_delete('byjs_sun_groups',array('id'=>$id));
    if($res){
