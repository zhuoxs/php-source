...
$sql="select a.* ,b.title from " . tablename("byjs_sun_zx_assess") . " a"  . " inner join " . tablename("byjs_sun_zx") . " b on a.zx_id=b.id ".$where;
$total=pdo_fetchcolumn("select count(*) from " . tablename("byjs_sun_zx_assess") . " a"  . " inner join " . tablename("byjs_sun_zx") . " b on a.zx_id=b.id".$where,$data);
	$select_sql =$sql." LIMIT " .($pageindex - 1) * $pagesize.",".$pagesize;
	$list=pdo_fetchall($select_sql,$data);
	$pager = pagination($total, $pageindex, $pagesize);

	if($operation=='info'){
		$sql="select a.* ,b.name from " . tablename("byjs_sun_zx_assess") ." a" . " left join " . tablename("zhjd_user") . " b on b.id=a.user_id where a.uniacid=:uniacid and a.id=:id";
		$list=pdo_fetch($sql, array(':uniacid'=>$_W['uniacid'],':id'=>$_GPC['id']));
		include $this->template('web/pingluninfo');
	}
	if(checksubmit('submit2')){
		//保存回复
		$result = pdo_update('byjs_sun_zx_assess', array('reply' => $_GPC['reply'],'status'=>2,'reply_time'=>date("Y-m-d H:i:s")), array('id' =>  $_GPC['id']));
		if($result){
	...
	if($operation=='delete'){
		$res=pdo_delete('byjs_sun_zx_assess',array('id'=>$_GPC['id']));
		if($res){
