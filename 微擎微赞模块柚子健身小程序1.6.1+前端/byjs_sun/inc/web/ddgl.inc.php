...
$pagesize=10;
$where=" WHERE  a.uniacid=:uniacid and a.good_name!='课程预约' and a.good_name!='教练课程预约'  and a.good_name!='会员卡' ";
...
if($operation=='delete'){
	$res=pdo_delete('byjs_sun_order',array('id'=>$_GPC['id'],'uniacid'=>$_W['uniacid']));
...
/////////////////分销/////////////////
        ...
        $set=pdo_get('byjs_sun_fxset',array('uniacid'=>$_W['uniacid']));
        $order=pdo_get('byjs_sun_order',array('id'=>$_GPC['id'],'uniacid'=>$_W['uniacid']));
        if($set['is_open']==1){
            if($set['is_ej']==2){//不开启二级分销
       $user=pdo_get('byjs_sun_fxuser',array('fx_user'=>$order['user_id'],'uniacid'=>$_W['uniacid']));
       if($user){
            $userid=$user['user_id'];//上线id
            $money=$order['money']*($set['commission']/100);//一级佣金
            pdo_update('byjs_sun_user',array('commission +='=>$money),array('id'=>$userid,'uniacid'=>$_W['uniacid']));
            $data6['user_id']=$userid;//上线id
            $data6['son_id']=$order['user_id'];//下线id
            $data6['money']=$money;//金额
            $data6['time']=time();//时间
            $data6['uniacid']=$_W['uniacid'];
            pdo_insert('byjs_sun_earnings',$data6);
          }
      }else{//开启二级
       $user=pdo_get('byjs_sun_fxuser',array('fx_user'=>$order['user_id'],'uniacid'=>$_W['uniacid']));
          $user2=pdo_get('byjs_sun_fxuser',array('fx_user'=>$user['user_id'],'uniacid'=>$_W['uniacid']));//上线的上线
          if($user){
            $userid=$user['user_id'];//上线id
            $money=$order['money']*($set['commission']/100);//一级佣金
            pdo_update('byjs_sun_user',array('commission +='=>$money),array('id'=>$userid,'uniacid'=>$_W['uniacid']));
          ...
          if($user2){
            ...
            pdo_update('byjs_sun_user',array('commission +='=>$money),array('id'=>$userid2,'uniacid'=>$_W['uniacid']));
            $data7['user_id']=$userid2;//上线id
            $data7['son_id']=$order['user_id'];//下线id
            $data7['money']=$money;//金额
            $data7['time']=time();//时间
            $data7['uniacid']=$_W['uniacid'];
            pdo_insert('byjs_sun_earnings',$data7);
          }
        }
        }
...
/*if(checksubmit('export_submit', true)) {
    ...
    $time="'%$time%'";
        $count = pdo_fetchcolumn("SELECT COUNT(*) FROM". tablename("byjs_sun_order")." WHERE time LIKE ".$time);
        ...
        for ($j = 1; $j <= $pagesize; $j++) {
            $sql = "select a.*,b.store_name as seller_name from " . tablename("byjs_sun_order")."  a"  . " inner join " . tablename("byjs_sun_store")." b on a.store_id=b.id  WHERE a.time LIKE ".$time."  limit " . ($j - 1) * 5000 . ",5000 ";
