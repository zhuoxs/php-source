...
$info=pdo_get('byjs_sun_yellowstore',array('id'=>$_GPC['id']));
...
//入住类型
$typein=pdo_getall('byjs_sun_yellowset',array('uniacid'=>$_W['uniacid']));
if($info['imgs']){
			if(strpos($info['imgs'],',')){
        ...
        if(checksubmit('submit')){
        	...
        	if($_GPC['rz_type']){
        		...
        		$data['time_over']=2;
                $days=pdo_get('byjs_sun_yellowset',array('id'=>$_GPC['rz_type']));
            $data['dq_time']=$info['sh_time']+$days['days']*60*60*24;
        	}
            $data['cityname']=$_GPC['cityname'];
        	$res = pdo_update('byjs_sun_yellowstore', $data, array('id' => $_GPC['id']));
        	if($res){
