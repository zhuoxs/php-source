// yzmdwsc_sun/pages/index/limitDet/limitDet.js
const app = getApp();
...
Page({
  ...
  onLoad: function (options) {
    ...
    var cdInterval = setInterval(function () {
      for (var i = 0; i < countDown.length; i++) {
        var time = tool.countDown(that, countDown[0].endtime);/***第二个参数 结束时间 */
        if (time) {
          clock = time[2] + " : " + time[3] + " : " + time[4];
        } else {
          clock = '00 : 00 : 00';
    ...
    /*价格数组*/
    var arrPrice = (countDown[0].price).split('.')
    that.setData({
      arrPrice: arrPrice 
    })
  },
  ...
  onShareAppMessage: function (res) {
    ...
    return {
      title: title,
      path: 'yzmdwsc_sun/pages/index/limitDet/limitDet?gid=' + that.data.gid,
      success: function (res) {
        // 转发成功
      },
      fail: function (res) {
  ...
  addNum(e) {
    ...
    var limit=that.data.goods[0].limit;
