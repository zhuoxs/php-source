...
$sql = 'SELECT * FROM ' . tablename('yzmdwsc_sun_active') . ' WHERE `uniacid` = :uniacid ';
...
$id = $_GPC['id'];
if (!empty($id)) {
    $item = pdo_fetch("SELECT *  FROM " . tablename('yzmdwsc_sun_gift') . " WHERE id = :id", array(':id' => $id));
}
if(checksubmit('submit')){
    ...
    if (!empty($id)) {
        ...
        pdo_update('yzmdwsc_sun_gift', $data, array('id' => $id));
    } else {
