...
$info=pdo_get('yzmdwsc_sun_active',array('uniacid'=>$_W['uniacid'],'id'=>$_GPC['id']));
$id = $_GPC['id'];
if (!empty($id)) {
    $item = pdo_fetch("SELECT *  FROM " . tablename('yzmdwsc_sun_active') . " WHERE id = :id", array(':id' => $id));
...
if(checksubmit('submit')){
        ...
        if (!empty($id)) {
            ...
            pdo_update('yzmdwsc_sun_active', $data, array('id' => $id));
        } else {
            pdo_insert('yzmdwsc_sun_active', $data);
            $id = pdo_insertid();
        }
