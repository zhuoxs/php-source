...
//$sql="SELECT a.*,b.store_name as seller_name FROM ".tablename('yzmdwsc_sun_order') .  " a"  . " left join " . tablename("yzmdwsc_sun_store") . " b on a.store_id=b.id".$where." ORDER BY a.time DESC";

$sql="select * from ".tablename('yzmdwsc_sun_order') .$where." order by id desc";

$data[':uniacid']=$_W['uniacid'];
//$total=pdo_fetchcolumn("SELECT count(*) FROM ".tablename('yzmdwsc_sun_order') .  " a"  . " left join " . tablename("yzmdwsc_sun_store") . " b on a.store_id=b.id".$where." ORDER BY a.time DESC",$data);
$total=pdo_fetchcolumn("SELECT count(*) FROM ".tablename('yzmdwsc_sun_order').$where." ORDER BY id DESC",$data);
...
if($operation=='delete'){
   ...
   //$res=pdo_update('yzmdwsc_sun_order',array('is_delete'=>1),array('id'=>$_GPC['id']));
	if($res){
		message('删除成功',$this->createWebUrl('ddmdinfo',array()),'success');
	}else{
		message('删除失败','','error'); 
	}
}
if($operation=='delivery'){
   ...
   $res=pdo_update('yzmdwsc_sun_order',array('order_status'=>2,'fahuo_time'=>time()),array('id'=>$_GPC['id']));
	if($res){
		message('操作成功',$this->createWebUrl('ddmdinfo',array()),'success');
	}else{
		message('操作失败','','error');
	}
}
