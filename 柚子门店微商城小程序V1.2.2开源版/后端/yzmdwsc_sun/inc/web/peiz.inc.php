...
$item=pdo_get('yzmdwsc_sun_system',array('uniacid'=>$_W['uniacid']));
    if(checksubmit('submit')){
        ...
        if($apiclient_cert['name'] ){
            if($apiclient_cert['type']!='application/octet-stream'){
                message('文件类型只能为pem格式');
            }
        }
       if( $apiclient_key['name']){
            if($apiclient_key['type']!='application/octet-stream'){
                message('文件类型只能为pem格式');
            }
        }
        ...
        if(is_uploaded_file($_FILES['apiclient_cert']['tmp_name'])) {
            ...
            //我们给每个用户动态的创建一个文件夹
            $user_path=IA_ROOT."/addons/yzmdwsc_sun/cert/";
        ...
        if(is_uploaded_file($_FILES['apiclient_key']['tmp_name'])) {
            ...
            if($apiclient_cert['name']){
         	   $data['apiclient_cert']=$apiclient_cert['name'].'_'.time();  
            }
    		if($apiclient_key['name']){
           	   $data['apiclient_key']=$apiclient_key['name'].'_'.time();
            }
