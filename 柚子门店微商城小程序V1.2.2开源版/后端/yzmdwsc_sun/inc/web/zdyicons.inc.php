...
if($_GPC['op']=='show'){
  $show_status=$_GPC['show_status'];
  if($show_status==1){
    $msg='显示';
  }else{
    $msg='隐藏';
  }
    $res=pdo_update('yzmdwsc_sun_customize',array('show_status'=>$show_status),array('id'=>$_GPC['id']));
    if($res){
        message("{$msg}成功", $this->createWebUrl('zdyicons'), 'success');
    }else{
        message("{$msg}拒绝失败！",'','error');
    }
    exit;
