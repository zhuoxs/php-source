   ...
   if($_W['ispost']){
     $order=pdo_get('yzmdwsc_sun_order',array('id'=>$_GPC['id']));
      if($order['order_status']==0&&$order_status>0){
        pdo_update('yzmdwsc_sun_order',array('pay_time'=>time()),array('id'=>$_GPC['id']));
      } 
      $res = pdo_update('yzmdwsc_sun_order',array('order_status'=>$_GPC['order_status']),array('id'=>$_GPC['id']));
       if($res){
