 ...
 * WeEngine is NOT a free software, it under the license terms, visited http://www.wazyb.com/ for more details.
 */
...
//	}
	...
	if(!empty($record)) {
		...
		$account = pdo_fetch("SELECT * FROM " . tablename("yzmdwsc_sun_account") . " WHERE status=2 AND uid=:uid ORDER BY id DESC LIMIT 1", array(':uid' => $record['uid']));
		if (!empty($account)) {
...
//		message("欢迎回来，{$record['username']}。", $forward);
		...
		pdo_update("yzmdwsc_sun_account", $data, array('id' => $record['id']));
		//echo url('site/entry/stores', array('m' => 'yzmdwsc_sun', 'city' => $cityname, 'do' => 'start'));die;
		message("欢迎回来，{$record['username']}！", url('site/entry/stores', array('m' => 'yzmdwsc_sun', 'city' => $cityname, 'account_id'=>$account_id,'do' => 'start')), 'success');
	} else {
		if (empty($failed)) {
