...
$list = pdo_getall('wnjz_sun_category',array('uniacid'=>$_W['uniacid']),'','','cid ASC');
$list = getTree($list);



if($_GPC['op']=='delete'){
    $data = pdo_getall('wnjz_sun_category',array('uniacid'=>$_W['uniacid'],'pid'=>$_GPC['cid']));
    if($data){
        message('存在子分类，请先删除子类！','','error');
    }else{
        $res=pdo_delete('wnjz_sun_category',array('cid'=>$_GPC['cid']));
        if($res){
