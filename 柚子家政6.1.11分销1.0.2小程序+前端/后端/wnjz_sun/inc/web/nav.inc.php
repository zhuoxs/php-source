...
if($op=="add" || $op=="edit"){
    $id = intval($_GPC['id']);
    if($id>0){
        $info=pdo_get('wnjz_sun_nav',array('id'=>$id));
    }
...
//        echo $id;exit;
        ...
        if($id==0){
            ...
            $res=pdo_insert('wnjz_sun_nav',$data);
//            pdo_debug();
            if($res){
            ...
            $res=pdo_update('wnjz_sun_nav',$data,array('id'=>$id));
            if($res){
    ...
    $res=pdo_delete('wnjz_sun_nav',array('id'=>intval($_GPC['id'])));
    if($res){
    ...
    $pagesize=10;
    $sql="select * from".tablename('wnjz_sun_nav')." where uniacid={$_W['uniacid']} and position=".$position." ORDER BY id desc";
    $total=pdo_fetchcolumn("select count(*) from".tablename('wnjz_sun_nav')." where uniacid={$_W['uniacid']} and position=".$position." ");
