...
$GLOBALS['frames'] = $this->getMainMenu();
    ...
    $id = $_GPC['id'];
	$info = pdo_get('wnjz_sun_gallery',array('id'=>$id));
//	p($info);die;
if($info['imgs']){
	...
	$type = pdo_getall('wnjz_sun_hairers',array('id' => $hair_id));
//	p($type);die;
...
//		    p($_GPC);die;
			...
			if($_GPC['id']==''){
				$res=pdo_insert('wnjz_sun_gallery',$data);
				...
				$res = pdo_update('wnjz_sun_gallery', $data, array('id' => $_GPC['id']));
				if($res){
