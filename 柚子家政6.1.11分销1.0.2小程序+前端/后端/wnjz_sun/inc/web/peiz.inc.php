...
$item=pdo_get('wnjz_sun_system',array('uniacid'=>$_W['uniacid']));
    if(checksubmit('submit')){
			...
			//我们给每个用户动态的创建一个文件夹
			$user_path=IA_ROOT."/addons/wnjz_sun/cert/";
			...
			if(is_uploaded_file($_FILES['apiclient_cert']['tmp_name'])) {
				...
				if(move_uploaded_file($uploaded_file,iconv("utf-8","gb2312",$move_to_file))) {
					...
					pdo_update('wnjz_sun_system', array('apiclient_cert' => ''), array('id' => $_GPC['id'],'uniacid'=>$_W['uniacid']));
				} else {
					echo "上传失败";
				}
			} else {
			...
			if(is_uploaded_file($_FILES['apiclient_key']['tmp_name'])) {
				...
				if(move_uploaded_file($uploaded_file,iconv("utf-8","gb2312",$move_to_file))) {
					...
					pdo_update('wnjz_sun_system', array('apiclient_key' => ''), array('id' => $_GPC['id'],'uniacid'=>$_W['uniacid']));
				} else {
					echo "上传失败";
				}
			} else {
...
//            }
            ...
            if($_GPC['id']==''){                
                $res=pdo_insert('wnjz_sun_system',$data,array('uniacid'=>$_W['uniacid']));
                if($res){
                ...
                $res = pdo_update('wnjz_sun_system', $data, array('id' => $_GPC['id'],'uniacid'=>$_W['uniacid']));
                if($res){
