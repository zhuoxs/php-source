...
$info = pdo_get('wnjz_sun_coupon', array('id' => $_GPC['id']));
...
if (checksubmit('submit')) {
   ...
   $data['uniacid'] = $_W['uniacid'];
    ...
    if (empty($_GPC['id'])) {
        $res = pdo_insert('wnjz_sun_coupon', $data,array('uniacid'=>$_W['uniacid']));
        if($res){
        ...
        $res = pdo_update('wnjz_sun_coupon', $data,array('id'=>$_GPC['id'],'uniacid'=>$_W['uniacid']));
    }
    if($res){
