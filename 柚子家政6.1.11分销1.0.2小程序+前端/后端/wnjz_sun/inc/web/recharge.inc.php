...
//    p($status);die;
//    $sql = "select * from ".tablename('wnjz_sun_kjorderlist')."a left join ".tablename('wnjz_sun_kjorder')."b on a.order_id=b.id"." WHERE  a.uniacid=".$_W['uniacid'] . ' and b.type=2' . ' and b.status=' . $status;
...
$servies = pdo_getall('wnjz_sun_recharges',array('uniacid'=>$_W['uniacid']));

if($_GPC['op'] == 'delete'){
    $res = pdo_delete('wnjz_sun_recharges',array('rid'=>$_GPC['rid']));
    if($res){
