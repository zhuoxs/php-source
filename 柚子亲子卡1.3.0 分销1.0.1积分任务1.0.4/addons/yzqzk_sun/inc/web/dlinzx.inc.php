...
//$list=pdo_getall('yzqzk_sun_zx',array('uniacid'=>$_W['uniacid']),array(),'','time ASC');
...
$select_sql =$sql." LIMIT " .($pageindex - 1) * $pagesize.",".$pagesize;
	...
	$total=pdo_fetchcolumn("select count(*) from " . tablename("yzqzk_sun_zx") . " a"  . " left join " . tablename("yzqzk_sun_zx_type") . " b on a.type_id=b.id" . " left join " . tablename("yzqzk_sun_user") . " c on a.user_id=c.id".$where,$data);
...
if($_GPC['state']){
	...
	$res=pdo_update('yzqzk_sun_zx',$data,array('id'=>$_GPC['id']));
	if($res){
