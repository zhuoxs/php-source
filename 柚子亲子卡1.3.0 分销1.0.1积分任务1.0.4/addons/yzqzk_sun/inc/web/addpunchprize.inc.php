...
$result=$result[$num];
$coupon=pdo_getall('yzqzk_sun_coupon',array('uniacid'=>$_W['uniacid'],'is_punch'=>1));
$info=pdo_get('yzqzk_sun_punch_prize',array('id'=>$_GPC['id']));
if(checksubmit('submit')){
    ...
    $coupon=pdo_get('yzqzk_sun_coupon',array('uniacid'=>$_W['uniacid'],'id'=>$_GPC['coupon_id']));
    if(empty($coupon)){
    ...
    if(empty($_GPC['id'])){
        $res = pdo_insert('yzqzk_sun_punch_prize', $data);
        if($res){
        ...
        unset($data['add_time']);
        $res = pdo_update('yzqzk_sun_punch_prize', $data, array('id' => $_GPC['id']));
        if($res){
