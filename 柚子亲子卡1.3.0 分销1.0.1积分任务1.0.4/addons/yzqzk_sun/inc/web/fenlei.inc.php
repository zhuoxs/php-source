...
//$list = pdo_getall('yzqzk_sun_type',array('uniacid' => $_W['uniacid']),array(),'','num ASC');
 ...
 global $_W, $_GPC;
    $type=pdo_getall('yzqzk_sun_type',array('uniacid'=>$_W['uniacid']),array(),'','num asc');
    $type2=pdo_getall('yzqzk_sun_type2',array('uniacid'=>$_W['uniacid']),array(),'','num asc');
    $sql=" select * from".tablename('yzqzk_sun_label')." where type2_id in (select id from ".tablename('yzqzk_sun_type2')." where uniacid={$_W['uniacid']})";
  //  $type3=pdo_fetchall( $sql);
    $type3=pdo_getall('yzqzk_sun_label');
...
if($_GPC['op']=='change2'){
     $res=pdo_update('yzqzk_sun_type2',array('state'=>$_GPC['state']),array('id'=>$_GPC['id']));
    if($res){
        message('操作成功',$this->createWebUrl('fenlei',array()),'success');
    }else{
        message('操作失败','','error');
