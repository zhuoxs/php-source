...
//查出已是商家用户
$info = pdo_get('yzqzk_sun_store',array('uniacid'=>$_W['uniacid'],'id'=>$_GPC['id']));
$system=pdo_get('yzqzk_sun_system',array('uniacid'=>$_W['uniacid']));

//获取微信号
$userlist=pdo_get('yzqzk_sun_user',array('uniacid'=>$_W['uniacid'],'openid'=>$info['openid']),array("name"));
$info["name"] = $userlist["name"];

$category=pdo_getall('yzqzk_sun_store_category',array('uniacid'=>$_W['uniacid'],'state'=>1));
$district=pdo_getall('yzqzk_sun_store_district',array('uniacid'=>$_W['uniacid'],'state'=>1));
if($_GPC['op']=='search'){
    $name=$_GPC['name'];
    $where=" WHERE uniacid=".$_W['uniacid'];
    $sql="select openid,name as uname from " . tablename("yzqzk_sun_user") ." ".$where." and name like'%".$name."%' ";
...
if(checksubmit('submit')){
            ...
            $data['rz_status']=$_GPC['rz_status'];
			 if($_GPC['id']==''){
				$res = pdo_insert('yzqzk_sun_store', $data);
				if($res){
