...
$sql="select a.*,b.store_name from " . tablename("yzqzk_sun_goods") . " a"  . " left join " . tablename("yzqzk_sun_store") . " b on a.store_id=b.id" .$where."  order by a.time desc ";
$total=pdo_fetchcolumn("select count(*) as wname from " . tablename("yzqzk_sun_goods") . " a"  . " left join " . tablename("yzqzk_sun_store") . " b on a.store_id=b.id".$where."  order by a.time desc ",$data);
$select_sql =$sql." LIMIT " .($pageindex - 1) * $pagesize.",".$pagesize;
$list=pdo_fetchall($select_sql,$data);
foreach($list as &$val){
  if($val['related_gid']){
      $val['goods_name']=pdo_getcolumn('yzqzk_sun_goods', array('id' =>$val['related_gid'],'uniacid' => $_W['uniacid']), 'goods_name',1);
  }
}
...
if($_GPC['op']=='tg'){
    $res=pdo_update('yzqzk_sun_goods',array('status'=>2),array('id'=>$_GPC['id']));
    if($res){
