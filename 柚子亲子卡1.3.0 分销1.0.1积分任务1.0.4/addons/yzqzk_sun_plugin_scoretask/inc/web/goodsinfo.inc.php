...
$info=pdo_get('yzqzk_sun_plugin_scoretask_goods',array('id'=>$_GPC['id']));
if(checksubmit('submit')){
    ...
    $data['add_time']= time();
    if(empty($_GPC['id'])){
        $res = pdo_insert('yzqzk_sun_plugin_scoretask_goods', $data);
    }else{
        $res = pdo_update('yzqzk_sun_plugin_scoretask_goods', $data, array('id' => $_GPC['id']));
    }
    if($res){
