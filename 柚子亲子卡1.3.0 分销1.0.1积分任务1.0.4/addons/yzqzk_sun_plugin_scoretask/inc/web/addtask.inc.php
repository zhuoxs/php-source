...
$info=pdo_get('yzqzk_sun_plugin_scoretask_task',array('id'=>$_GPC['id']));
$category=array(
    array('id'=>1,'title'=>'签到'),
...
if(checksubmit('submit')){
    ...
    $data['add_time']= time();
    $task=pdo_get('yzqzk_sun_plugin_scoretask_task',array('uniacid'=>$_W['uniacid'],'type'=>$_GPC['type']));
    if(empty($_GPC['id'])){
        if($task){
            message('添加失败,每种任务类型只能添加一种','','error');
        }
        $res = pdo_insert('yzqzk_sun_plugin_scoretask_task', $data);
        if($res){
        ...
        $task_first=pdo_get(yzqzk_sun_plugin_scoretask_task,array('id'=>$_GPC['id']));
        if($task&&$task_first['type']!=$_GPC['type']){
            message('添加失败,每种任务类型只能添加一种','','error');
        }
        unset($data['add_time']);
        $res = pdo_update('yzqzk_sun_plugin_scoretask_task', $data, array('id' => $_GPC['id']));
        if($res){
