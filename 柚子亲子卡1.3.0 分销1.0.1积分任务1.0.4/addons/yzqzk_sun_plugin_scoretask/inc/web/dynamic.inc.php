...
$sql="select * from ".tablename("yzqzk_sun_dynamic").$where." order by id desc";
$total=pdo_fetchcolumn("select count(*) from ".tablename("yzqzk_sun_dynamic").$where,$data);
$select_sql =$sql." LIMIT " .($pageindex - 1) * $pagesize.",".$pagesize;
$list=pdo_fetchall($select_sql,$data);
foreach($list as &$val){
    $goods=pdo_get('yzqzk_sun_goods',array('id'=>$val['gid']));
    if(!empty($goods)){
...
if($_GPC['op']=='tg'){
    $res=pdo_update('yzqzk_sun_dynamic',array('state'=>2),array('id'=>$_GPC['id']));
    if($res){
...
if($_GPC['op']=='jj'){
    $res=pdo_update('yzqzk_sun_dynamic',array('state'=>3),array('id'=>$_GPC['id']));
    if($res){
