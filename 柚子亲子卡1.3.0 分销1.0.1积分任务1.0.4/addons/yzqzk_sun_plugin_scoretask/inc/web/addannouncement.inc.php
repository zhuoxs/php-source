...
$info=pdo_get('yzqzk_sun_announcement',array('id'=>$_GPC['id']));
if(checksubmit('submit')){
    ...
    $data['add_time']= time();  
    if(empty($_GPC['id'])){
        $res = pdo_insert('yzqzk_sun_announcement', $data);
        if($res){
        ...
        $res = pdo_update('yzqzk_sun_announcement', $data, array('id' => $_GPC['id']));
        if($res){
