...
class Distribution{
    ...
    private function getdistributionset(){
        ...
        $d_set = pdo_get('ymktv_sun_distribution_set', array('uniacid' => $uniacid));
        if($d_set){
    ...
    private function getleader($distributionset){
        ...
        if($distributionset["is_buyself"]==1){
            $d_promoter = pdo_get('ymktv_sun_distribution_promoter', $wdata);
            if($d_promoter){//自己为一级
                $this->rebate = 1;
        ...
        $d_user = pdo_get('ymktv_sun_user', $wdata);
        if($d_user){
    ...
    private function getnextleader($uid){
        $uniacid = $this->uniacid;
        $data_promoter = pdo_get('ymktv_sun_distribution_promoter', array("uniacid"=>$uniacid,"uid"=>$uid));
    ...
    private function computleadermoney($dataset){
        ...
        if($dataset["status"]>0){
            ...
            $res = pdo_insert('ymktv_sun_distribution_order', $data);
            if($res){
                return true;
            }else{
                return false;
    ...
    public function settlecommission(){
        ...
        //先判断表是否存在，不存在就断开
        if(!pdo_tableexists('ymktv_sun_distribution_order')) {
            return false;
        }
        //查找相关订单的数据
        $d_order = pdo_get('ymktv_sun_distribution_order', array('uniacid' => $uniacid,'order_id' => $order_id,'ordertype' => $ordertype,'is_delete' => 0));
        if($d_order){
            //一级
            if($d_order["parent_id_1"]>0 && $d_order["first_price"]>0){
                $res1 = pdo_update('ymktv_sun_distribution_promoter', array("allcommission +="=>$d_order["first_price"],"canwithdraw +="=>$d_order["first_price"]), array('uid' => $d_order["parent_id_1"], 'uniacid' => $uniacid));
            }
            //二级
            if($d_order["parent_id_2"]>0 && $d_order["second_price"]>0){
                $res2 = pdo_update('ymktv_sun_distribution_promoter', array("allcommission +="=>$d_order["second_price"],"canwithdraw +="=>$d_order["second_price"]), array('uid' => $d_order["parent_id_2"], 'uniacid' => $uniacid));
            }
            //三级
            if($d_order["parent_id_3"]>0 && $d_order["third_price"]>0){
                $res3 = pdo_update('ymktv_sun_distribution_promoter', array("allcommission +="=>$d_order["third_price"],"canwithdraw +="=>$d_order["third_price"]), array('uid' => $d_order["parent_id_3"], 'uniacid' => $uniacid));
            }

            //更新分销订单信息
            $res = pdo_update('ymktv_sun_distribution_order', array("is_delete"=>1), array('id' => $d_order["id"], 'uniacid' => $uniacid));
            if($res){
                return true;
            }else{
                return false;
    ...
    public function updateorder(){
        ...
        switch ($ordertype) {
            case '1'://普通
                $orderinfo = pdo_update('ymktv_sun_order', $datas, array('id' => $order_id, 'uniacid' => $uniacid));
                break;
            case '2'://砍价
                $orderinfo = pdo_update('ymktv_sun_kjorder', $datas, array('id' => $order_id, 'uniacid' => $uniacid));
                break;
            default:
