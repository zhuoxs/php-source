...
if($_GPC['state']){
 ...
 $data[':cityname']= $_COOKIE['cityname'];
  $sql="select a.*,b.store_name from " . tablename("ymktv_sun_goods") . " a"  . " left join " . tablename("ymktv_sun_store") . " b on a.store_id=b.id" .$where."  order by a.time desc ";
  $total=pdo_fetchcolumn("select count(*) as wname from " . tablename("ymktv_sun_goods") . " a"  . " left join " . tablename("ymktv_sun_store") . " b on a.store_id=b.id".$where."  order by a.time desc ",$data);
