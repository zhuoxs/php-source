...
$info=pdo_get('ymmf_sun_adminstore',array('id'=>$_GPC['id']));
// 获取门店数据
...
if(checksubmit('submit')){
    ...
    $data['uniacid']=$_W['uniacid'];
     if($_GPC['id']==''){
         $re = pdo_get('ymmf_sun_adminstore',array('uniacid'=>$_W['uniacid'],'account'=>$_GPC['account']));
         if($re){
             message('该账户已存在!');
         }else{
             $res=pdo_insert('ymmf_sun_adminstore',$data);
         }
        if($res){
         ...
         $be = pdo_getall('ymmf_sun_adminstore',array('uniaicd'=>$_W['uniacid'],'id !='=>$_GPC['id'],'account'=>$_GPC['account']));
         if($be){
             message('该账户已存在!');
         }else{
