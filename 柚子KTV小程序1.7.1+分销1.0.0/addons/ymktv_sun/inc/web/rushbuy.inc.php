...
$pagesize=10;
$where=' WHERE  a.uniacid=:uniacid  and a.is_qiang=1';
if($_GPC['keywords']){
    $op=$_GPC['keywords'];
...
if($operation=='server'){
    ...
    if($res){
        message('成功',$this->createWebUrl('rushbuy',array()),'success');
    }else{
        message('失败','','error');
    }
}
...
if($operation=='delete'){
    ...
    if($res){
        message('删除成功',$this->createWebUrl('rushbuy',array()),'success');
    }else{
        message('删除失败','','error');
    }
}
if($operation=='delivery'){
    ...
    if($res){
        message('操作成功',$this->createWebUrl('rushbuy',array()),'success');
    }else{
        message('操作失败','','error');
    }
}
...
if($operation=='refund'){
    ...
    if ($result['result_code'] == 'SUCCESS') {//退款成功
        ...
        message('退款成功',$this->createWebUrl('rushbuy',array()),'success');
...
include $this->template('web/rushbuy');