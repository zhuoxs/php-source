$(function(){
    ...
    if($('.poster-con').length>0){
        ...
        var widthImg = $(".poster-con").find('[data-target]').children('.user_img').outerWidth()*750/300;
    ...
    if($('.activity-con').length>0){
        ...
        if(productSel==''){
            ...
            $.each(productSel,function(i,items){
                ...
                var trHtml =
                    ...
                    '</td>'+
                    '<td>件</td>'+
                    '<td class="line_price">￥'+items.priceEx+'</td>'+
        ...
        $('.layout-bottom .control-submit').click(function(){
            ...
            $('.activity-pro tbody>tr').each(function(){
                ...
                var obj = {};
                obj['priceActivity'] = $(this).children('.activity_price').children().val();
                obj['limit'] = $(this).children('.limit').children().val();
                obj['inventory'] = $(this).children('.inventory').children().val();
                obj['sort'] = $(this).children('.sort').children().val();
                $.each(proSelTable.rules,function(i,item){
                    if(thisId == item.id){
                        item=$.extend(item,obj);
                        return false;
                    }
                })
            })
    ...
    switch(modal){
        case 'lunbotu':{
            ...
            $.each(imgGroup,function(i,item){
                ...
                if(item.img=='images/dz_bg.png'){
                    $('.edit-box .lay_lbt-modal ul').append('<li class="lay_img-box"><div class="imgbox"><img src="images/default_add.png"ondragstart="return false"alt=""title=""></div><div class="input-box"><div class="input-group"><input type="text"class="control-input form-control lay_imgsrc"name="magic_img0"readonly onclick="inputFile(this)"><div class="input-group-btn"><button type="button"class="btn btn-default"onclick="inputFile(this)">选择图片</button></div><input type="file"name=""class="inputfile"></div><div class="input-group"style="margin-top: 8px;"><input type="text"class=" control-input form-control valid lay_imglink"name="magic_img"value="'+item.link+'"readonly><div class="input-group-btn"><button type="button"class="btn btn-default btn-select-pic">选择链接</button></div></div></div><a href="javascript:void(0);"class="lay_img-del"onclick="lay_imgDel(this)"><i class="glyphicon glyphicon-remove"></i></a></li>');
                }else{
                    $('.edit-box .lay_lbt-modal ul').append('<li class="lay_img-box"><div class="imgbox"><img src="'+item.img+'"ondragstart="return false"alt=""title=""></div><div class="input-box"><div class="input-group"><input type="text"class="control-input form-control lay_imgsrc"name="magic_img0"readonly onclick="inputFile(this)"value="'+item.img+'"><div class="input-group-btn"><button type="button"class="btn btn-default"onclick="inputFile(this)">选择图片</button></div><input type="file"name=""class="inputfile"></div><div class="input-group"style="margin-top: 8px;"><input type="text"class=" control-input form-control valid lay_imglink"name="magic_img"value="'+item.link+'"readonly><div class="input-group-btn"><button type="button"class="btn btn-default btn-select-pic">选择链接</button></div></div></div><a href="javascript:void(0);"class="lay_img-del"onclick="lay_imgDel(this)"><i class="glyphicon glyphicon-remove"></i></a></li>');
                }
            });
            ...
            var layImgEditmodal = Sortable.create(lay_img_editmodal,{
                ...
                onUpdate: function (evt) {
                    ...
                    $('.edit-box .lay_lbt-modal ul>li').each(function(){
                        var obj={};
                        obj['img'] = $(this).find('.lay_imgsrc').val()==''?'images/dz_bg.png':$(this).find('.lay_imgsrc').val();
        ...
        case 'cenNav':{
            ...
            $.each(ftnLoad.findJson(layoutModalSelect,dataId,'imgGroup'),function(i,item){
                ...
                var nowImg = item.img=='images/dz_bg.png'?'images/default_add.png':item.img;
            ...
            var layImgEditmodal = Sortable.create(lay_img_editmodal,{
                ...
                onUpdate: function (evt) {
                    ...
                    $('.edit-box .lay_lbt-modal ul>li').each(function(){
                        ...
                        obj['img'] = $(this).find('.lay_imgsrc').attr('src')=='images/default_add.png'?'images/dz_bg.png':$(this).find('.lay_imgsrc').attr('src');
                        obj['link'] = $(this).find('.lay_imglink').val();
                        Oarr.push(obj);
                    });
                    console.log(Oarr)
                    ...
                    $.each(Oarr,function(i,item){
                        var nowColor = item.newCol==''?item.original:item.newCol;
                        var nowImg = item.img=='images/default_add.png'?'images/dz_bg.png':item.img;
        ...
        case 'consulting':{
            ...
            if(parseInt(perPage)){
                ...
                $('.edit-box .lay_cg-modal').hide();
                $('.edit-box .lay_img-add').hide();
                $('#layout_show').find('li[data-id="'+dataId+'"]').find('.lay_consulting').html('<div class="clum"><div class="msg"><span class="title"data-editid="title"data-state="true">文章标题-文章标题-文章标题</span><div class="msg-bottom"><span class="time"data-editid="time"data-state="'+elementSel.time+'"><i class="glyphicon glyphicon-time"></i>2018-07-30</span><span class="reading"data-editid="reading"data-state="'+elementSel.reading+'"><i class="glyphicon glyphicon-eye-open"></i>2010</span><span class="like"data-editid="like"data-state="'+elementSel.like+'"><i class="glyphicon glyphicon-heart-empty"></i>200</span></div></div><div class="imgboxs"><div class="imgbox middle_center"><img src="images/lb_bg.png"ondragstart="return false"alt=""title=""style="height: 100%; width: auto;"></div></div></div><div class="clum"><div class="msg"><span class="title"data-editid="title"data-state="true">文章标题-文章标题-文章标题</span><div class="msg-bottom"><span class="time"data-editid="time"data-state="'+elementSel.time+'"><i class="glyphicon glyphicon-time"></i>2018-07-30</span><span class="reading"data-editid="reading"data-state="'+elementSel.reading+'"><i class="glyphicon glyphicon-eye-open"></i>2010</span><span class="like"data-editid="like"data-state="'+elementSel.like+'"><i class="glyphicon glyphicon-heart-empty"></i>200</span></div></div><div class="imgboxs"><div class="imgbox middle_center"><img src="images/lb_bg.png"ondragstart="return false"alt=""title=""style="height: 100%; width: auto;"></div></div></div><div class="clum"><div class="msg"><span class="title"data-editid="title"data-state="true">文章标题-文章标题-文章标题</span><div class="msg-bottom"><span class="time"data-editid="time"data-state="'+elementSel.time+'"><i class="glyphicon glyphicon-time"></i>2018-07-30</span><span class="reading"data-editid="reading"data-state="'+elementSel.reading+'"><i class="glyphicon glyphicon-eye-open"></i>2010</span><span class="like"data-editid="like"data-state="'+elementSel.like+'"><i class="glyphicon glyphicon-heart-empty"></i>200</span></div></div><div class="imgboxs"><div class="imgbox middle_center"><img src="images/lb_bg.png"ondragstart="return false"alt=""title=""style="height: 100%; width: auto;"></div></div></div>');
...
LMData.prototype = {
    ...
    layLbtAdd:function(idx){
        ...
        $('#lay_imgAdd').click(function(){
            ...
            $(this).parents('.lay_img-add').siblings('.lay_lbt-modal').children().append('<li class="lay_img-box"><div class="imgbox"><img src="images/default_add.png"ondragstart="return false"alt=""title=""></div><div class="input-box"><div class="input-group"><input type="text"class="control-input form-control lay_imgsrc"name="magic_img0"readonly onclick="inputFile(this)"><div class="input-group-btn"><button type="button"class="btn btn-default"onclick="inputFile(this)">选择图片</button></div><input type="file"name=""class="inputfile"></div><div class="input-group"style="margin-top: 8px;"><input type="text"class=" control-input form-control valid lay_imglink"name="magic_img"readonly><div class="input-group-btn"><button type="button"class="btn btn-default btn-select-pic">选择链接</button></div></div></div><a href="javascript:void(0);"class="lay_img-del"onclick="lay_imgDel(this)"><i class="glyphicon glyphicon-remove"></i></a></li>');
            var obj ={};
            obj['img'] = 'images/dz_bg.png';
            obj['link'] = '';
    ...
    layCenNavAdd:function(idx){
        ...
        $('#lay_imgAdd').click(function(){
            ...
            $(this).parents('.lay_img-add').siblings('.lay_lbt-modal').children().append('<li class="lay_img-box lay_fileimg"><div class="imgbox"><img src="images/default_add.png"class="lay_imgsrc"ondragstart="return false"alt=""title=""><input type="file"name=""class="inputfile"><div class="imgNavUp"><span class=""title="点击重新选择"onclick="iconLibrary(this)"data-toggle="modal"data-target=".icon-lib">重新选择</span></div></div><div class="input-box"><div class="input-group input-text-color"><span class="input-group-addon">文字</span><input type="text"class="control-input form-control text-input"name="navname"data-editid="cennav_linkname"onkeyup="inputChange(this,&quot;textName&quot;)"value="'+obj['name']+'"><div class="color-box"data-newcol="'+obj['original']+'"style="background-color:'+obj['original']+'"data-original="'+obj['original']+'"></div><div class="input-group-btn reset-btn"><button type="button"class="btn btn-default"onclick="inputColorReset(this)">重置</button></div></div><div class="input-group"style="margin-top: 8px;"><input type="text"class=" control-input form-control valid lay_imglink"name="magic_img"readonly value="'+obj['link']+'"><div class="input-group-btn"><button type="button"class="btn btn-default btn-select-pic">选择链接</button></div></div></div><a href="javascript:void(0);"class="lay_img-del"onclick="lay_imgDel(this)"><i class="glyphicon glyphicon-remove"></i></a></li>');
    ...
    layBottomNavAdd:function(idx){
        ...
        $('#lay_imgAdd').click(function(){
            ...
            var original = LMData.prototype.findJson(layoutModalSelect,dataId,'original')
            obj['name'] = '按钮文字';
            obj['img'] = 'images/default_img.png';
            obj['img_sel'] = 'images/default_img.png';
            var newImg = 'images/default_add.png';
            obj['link'] = '';
        ...
        $(".edit-box[data-id="+dataId+"] .radio-box .selct-checkbox").click(function(){
            ...
            if(picLen-imgEditLen>0){
                for(var i=0;i<(picLen-imgEditLen);i++){
                    $(this).parents('.edit-con').find('#lay_img_editmodal').append('<li class="lay_img-box"><div class="imgbox"><img src="images/default_add.png"ondragstart="return false"alt=""title=""></div><div class="input-box"><div class="input-group"><input type="text"class="control-input form-control lay_imgsrc"name="magic_img0"readonly onclick="inputFile(this)"><div class="input-group-btn"><button type="button"class="btn btn-default"onclick="inputFile(this)">选择图片</button></div><input type="file"name=""class="inputfile"></div><div class="input-group"style="margin-top: 8px;"><input type="text"class=" control-input form-control valid lay_imglink"name="magic_img"readonly><div class="input-group-btn"><button type="button"class="btn btn-default btn-select-pic">选择链接</button></div></div></div></li>');
                    $layMoFang.append('<div class="clum"><div class="imgbox middle_center"data-editid="lunbotu_height"><img src="images/dz_bg.png"ondragstart="return false"alt=""title=""></div></div>');
                    var obj ={};
                    obj['img'] = 'images/dz_bg.png';
                    obj['link'] = '';
                    imgGroup.push(obj);
...
function proPageShow(parent,pageJson,clickPage,pageDisplay){
    ...
    $('.service-state').click(function(){
        ...
        if($(this).hasClass('action')){
            ...
            //添加
            serviceState(this);
            $('.service-select-modal .main-con .service-select').append('<li><span class="title">'+$(this).parents('tr').find('.title').text()+'</span><a href="javascript:void(0)"class="service-select-remove" data-selectnum="'+selectNum+'" onclick="serviceSelectRemove(this)"><i class="glyphicon glyphicon-remove"></i></a></li>');
        }
    });
    ...
    switch(parentStyle){
        ...
        case 'comActSel':{
            if(!$(ev).hasClass('disabled')){
                if($(ev).parents('tbody').children('tr').length<1){
                    $.Toast("提示", "至少保留1个商品！", "notice", {});
                }else{
...
function batchSet(ev){
    ...
    $(".pro-more-table tbody tr").find('td.'+address).children().val(name);
}
...
function LinkToChoose(ev){
    ...
    $(".modal .btn-confirm").click(function(){
        ...
        if($('.linktochoose_li.active').hasClass('getmessage')){
            ...
            $(linkTraget).val($.trim(listname)+":"+$.trim(name)+"("+msg+")");
            ...
            var listname = $('.link-lib-control li.active').children().text();
            var name = $('.linktochoose_li.active').children('.title').text();
            $(linkTraget).val($.trim(listname)+":"+$.trim(name));
...
function choosePart(ev){
    ...
    $(".service-select-modal .btn-primary").click(function(){
        ...
        var listname = $('.link-lib-control li.active').children().text();
        var name = $('.linktochoose_li.active').children('.title').text();
        $(linkTraget).val($.trim(listname)+":"+$.trim(name)+"("+itemName+")");
        $(linkTraget).attr('data-type',type).attr('data-link',selectID);
...
function order_delivery(ev){
    ...
    switch(orderModal){
        case 'physical_commodity':{
            ...
            titleMsg = '订单发货'
            cfMsg = '发货';
            tipsMsg = '*请仔细填写物流公司及快递单号，发货后24小时内仅支持做一次更正，逾期不可修改';
            $.each(logisticsCompany,function(i,item){
                option+='<option>'+item+'</option>';
            })

            msg='配送方式：'+specExArrTable.default.distribution+'<br>'+'收货人：'+specExArrTable.default.consignee+'&nbsp;&nbsp;'+specExArrTable.default.phone+'<br>收货地址：'+specExArrTable.default.address;

            theadContent = '<tr><th class="order-tab-10"><label><input class="selectAll" type="checkbox" name="products"></label></th><th class="order-tab-30">商品</th><th class="order-tab-10">数量</th><th class="order-tab-20">状态</th><th class="order-tab-30">运单号</th></tr>';
        ...
        case 'city_distribution':{
            ...
            msg='配送方式：'+specExArrTable.default.distribution+'<br>'+'送达时间：'+specExArrTable.default.deliveryTime+'<br>'+'收货人：'+specExArrTable.default.consignee+'&nbsp;&nbsp;'+specExArrTable.default.phone+'<br>收货地址：'+specExArrTable.default.address;
    ...
    $('body').append('<div class="modal fade bs-example-modal-md order_delivery-modal"tabindex="-1"role="dialog"aria-labelledby="mySmallModalLabel"data-parent="proBox"><div class="modal-dialog modal-md"role="document"><div class="modal-content"><div class="modal-header"><button type="button"class="close"data-dismiss="modal"aria-label="Close"><span aria-hidden="true">&times;</span></button><h4 class="modal-title"id="myModalLabel">'+titleMsg+'</h4></div><div class="modal-body clearit"data-parent><div class="control_part"><div class="form-group col-xs-12 row order_delivery-state"><label class="control-label col-xs-6 col-sm-4"><span class="text-title"style="font-weight:bold">选择商品</span></label><div class="col-xs-6 col-sm-8 row send_num">待发货<span class="sending">'+sending+'</span>&nbsp;&nbsp;已发货<span class="sended">'+sended+'</span></div></div><div class="form-group col-xs-12"><div class="main-table table-responsive row"><table class="table table-hover table-condensed"><thead>'+theadContent+'</thead><tbody>'+tbodyContent+'</tbody></table></div></div><div class="form-group col-xs-12 row"><label class="control-label col-xs-6 col-sm-4"><span class="text-title">配送信息</span><span class="text-danger2">:</span></label><div class="col-xs-6 col-sm-8 row order_delivery-msg">'+msg+'</div></div><div class="form-group col-xs-12 row order_delivery-way"><label class="control-label col-xs-6 col-sm-4"><span class="text-title">发货方式</span><span class="text-danger2">:</span></label><div class="col-xs-6 col-sm-8 row order_delivery-radio"><div class="radio-box"><label class="radio-checkbox-label"><input class="selct-checkbox"type="radio"name="order_delivery_radio"value="1">选择物流</label><label class="radio-checkbox-label"><input class="selct-checkbox"type="radio"name="order_delivery_radio"value="0">无需物流</label><label class="radio-checkbox-label order_delivery-citylabel"><input class="selct-checkbox"type="radio"name="order_delivery_radio"value="0">商家自行配送</label></div><div class="order_delivery-show clearit"data-stylediy=""><div class="form-group col-xs-12 row"><label class="control-label col-xs-6 col-sm-4"><span class="text-title">物流公司</span><span class="text-danger2">:</span></label><div class="col-xs-6 col-sm-8 row "><select class="order_delivery-chosen control-input"data-placeholder="请选择物流公司"><option></option></select></div></div><div class="form-group col-xs-12 row"><label class="control-label col-xs-6 col-sm-4"><span class="text-title">运输单号</span><span class="text-danger2">:</span></label><div class="col-xs-6 col-sm-8 row"><input class="control-input tracking_number"type="text"autocomplete="off"required></div></div></div></div></div><div class="clearit"></div><span class="control-tips" style="text-align:center">'+tipsMsg+'</span></div></div><div class="modal-footer"data-target="proBox"data-prostyle="oneProSel"><button type="button"class="btn btn-primary btn-confirm">'+cfMsg+'</button><button type="button"class="btn btn-default"data-dismiss="modal">取消</button></div></div></div></div>');
    ...
    $(".modal .btn-confirm").click(function(){
        ...
        var logistics = '';
        var waybill = '';
        if($('.order_delivery-radio .selct-checkbox:checked').val()=='1'){
...
function order_modifyPrice(ev){
    ...
    msg='收货地址：'+specExArrTable.default.address;
...
function integralNew(ev,id,money,integral){
    ...
    $('body').append('<div class="modal fade bs-example-modal-md integral-modal"tabindex="-1"role="dialog"aria-labelledby="mySmallModalLabel"data-parent="proBox"><div class="modal-dialog modal-md"role="document"><div class="modal-content"><div class="modal-header"><button type="button"class="close"data-dismiss="modal"aria-label="Close"><span aria-hidden="true">&times;</span></button><h4 class="modal-title"id="myModalLabel">新建积分规则</h4></div><div class="modal-body clearit"data-parent><div class="form-group col-xs-12 row sanji_pro-way"><label class="control-label col-xs-6 col-sm-4"><span class="text-title">奖励条件</span><span class="text-danger2">:</span></label><div class="col-xs-6 col-sm-8 row"><div class="input-daterange input-group"id="datepicker"><span class="input-group-addon">每购买金额达</span><input type="text"class="control-input integral_money"style="margin:0" value="'+ money +'"><span class="input-group-addon">元</span></div><span class="control-tips">全部商品参加；发生部分退款时,按照实际支付金额扣除应退积分。</span></div></div><div class="form-group col-xs-12 row sanji_pro-way"><label class="control-label col-xs-6 col-sm-4"><span class="text-title">奖励分值</span><span class="text-danger2">:</span></label><div class="col-xs-6 col-sm-8 row"><div class="min-input"><input type="number" value="'+ integral +'" class="control-input  integral_num"placeholder="请输入整数"onkeyup="positiveInteger(this)"></div></div></div></div><div class="modal-footer"data-target="proBox"data-prostyle="oneProSel"><button type="button"class="btn btn-primary btn-confirm">确定</button><button type="button"class="btn btn-default"data-dismiss="modal">取消</button></div></div></div></div>');
