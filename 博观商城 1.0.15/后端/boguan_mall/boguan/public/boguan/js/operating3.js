...
function order_modifyPrice(ev){
  ...
  $(".modal .btn-confirm").click(function(){
    ...
    saveOrderInfo(thisId,specExArrTable);
...
function integralNew(ev){


  $('body').append('<div class="modal fade bs-example-modal-md integral-modal"tabindex="-1"role="dialog"aria-labelledby="mySmallModalLabel"data-parent="proBox"><div class="modal-dialog modal-md"role="document"><div class="modal-content"><div class="modal-header"><button type="button"class="close"data-dismiss="modal"aria-label="Close"><span aria-hidden="true">&times;</span></button><h4 class="modal-title"id="myModalLabel">新建积分规则</h4></div><div class="modal-body clearit"data-parent><div class="form-group col-xs-12 row sanji_pro-way"><label class="control-label col-xs-6 col-sm-4"><span class="text-title">奖励条件</span><span class="text-danger2">:</span></label><div class="col-xs-6 col-sm-8 row"><div class="input-daterange input-group"id="datepicker"><span class="input-group-addon">每购买金额达</span><input type="text"class="control-input"style="margin:0"><span class="input-group-addon">元</span></div><span class="control-tips">全部商品参加；发生部分退款时,按照实际支付金额扣除应退积分。</span></div></div><div class="form-group col-xs-12 row sanji_pro-way"><label class="control-label col-xs-6 col-sm-4"><span class="text-title">奖励分值</span><span class="text-danger2">:</span></label><div class="col-xs-6 col-sm-8 row"><div class="min-input"><input type="number"class="control-input"placeholder="请输入整数"onkeyup="positiveInteger(this)"></div></div></div></div><div class="modal-footer"data-target="proBox"data-prostyle="oneProSel"><button type="button"class="btn btn-primary btn-confirm">确定</button><button type="button"class="btn btn-default"data-dismiss="modal">取消</button></div></div></div></div>');
...
function actPageShow(parent,pageJson,clickPage,pageDisplay){
  ...
  $.each(pageJson,function(i,items){
    if(((clickPage-1)*pageDisplay)<=i&&(clickPage*pageDisplay)-1>=i){
      ...
      var lastState = false;
      if(actLength==1){
        if(endTime<items.startTime||startTime>items.endTime){
        }else{
          obj.push(items.id);
          $.Toast("提示", "商品: "+items.id+" ,在其他活动优惠中，请重新选择", "notice", {});
        }
      }else{
        // $.each(items.activity,function(n,itm){
          switch(true){
            case i==0:{
