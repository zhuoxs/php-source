                  ...
                  <div class="toggle-tipsbox">
                    ...
                    <form action="">
                        <ul class="nav-contral clearit nav nav-tabs" role="tablist">
                            <li role="presentation" class="nav-tabs-li <?php if(input('status')== ''): ?>active<?php endif; ?>"><a href="<?php echo url('boguan/platform.coupon/index'); ?>">全部</a></li>
                            <li role="presentation" class="nav-tabs-li <?php if(input('status') == '1'): ?>active<?php endif; ?>"><a href="<?php echo url('boguan/platform.coupon/index',['status'=> 1]); ?>">已生效</a></li>
                            <li role="presentation" class="nav-tabs-li <?php if(input('status') == '0'): ?>active<?php endif; ?>"><a href="<?php echo url('boguan/platform.coupon/index',['status'=> 0]); ?>">已失效</a></li>
                        </ul>

                        <ul class="nav-contral clearit">
                            <li><a href="<?php echo url('boguan/platform.coupon/add'); ?>" class="btn"><i class="iconfont icon-jia"></i>新增优惠券</a></li>
                        </ul>
                        <div class="main-table table-responsive">
                            <table class="table table-hover table-condensed">
                                ...
                                <tbody><?php if(is_array($coupon) || $coupon instanceof \think\Collection || $coupon instanceof \think\Paginator): $i = 0; $__LIST__ = $coupon;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="del-<?php echo $vo['id']; ?>">
                                    <td><?php echo $vo['name']; ?></td>
                                    <td><?php if($vo['expire_type'] == 1): ?><?php echo date('Y-m-d',$vo['start_time']); ?> 至 <?php echo date('Y-m-d',$vo['end_time']); else: ?>领取<?php echo $vo['days']; ?>天内有效<?php endif; ?></td>
                                    <td><?php if($vo['min_price'] > 0): ?>满<?php echo $vo['min_price']; ?>元可使用<?php else: ?>不限制<?php endif; ?></td>
                                    <td><?php if($vo['type'] == 1): ?>优惠<?php echo $vo['sub_price']; ?>元<?php else: ?>打<?php echo $vo['discount']; ?>折<?php endif; ?></td>
                                    <td><?php if($vo['apply'] == 1): ?>指定商品分类<?php elseif($vo['apply'] == 2): ?>指定商品<?php else: ?>全场通用<?php endif; ?></td>
                                    <td><?php echo $vo['send_num']; ?>/<?php echo $vo['use_num']; ?>/<?php if($vo['sum'] <= '0'): ?>不限制<?php else: ?><?php echo $vo['sum']; endif; ?></td>
                                    <td>
                                    ...
                                    <td>
                                        <a href="<?php echo url('boguan/platform.coupon/edit',['id'=> $vo['id']]); ?>">编辑</a>
    ...
    function del(id,key){
        ...
        layui.use(['layer','form'], function(){
            ...
            layer.confirm('确认删除吗？', {btn:['确认'], yes:function(index){
                    ...
                    $.ajax({
                        type: 'POST',
                        url: '<?php echo url("boguan/platform.coupon/del"); ?>',
                        data:{'id':id},
                        success: function(data) {
                            console.log(data);
                            if (data.errorCode == 1) {
