                  ...
                  <div class="toggle-tipsbox">
                    ...
                    <form action="" method="get" enctype="multipart/form-data">
                        <ul class="nav-contral clearit">
                            <li><a href="<?php echo url('boguan/platform.product/add'); ?>" class="btn"><i class="iconfont icon-jia"></i> 添加商品</a></li>
                            <li>
                                <select class="control-chosen control-input" name="cate_id" onchange='window.location="index.html?type=<?php echo input('type'); ?>&cate_id="+this.value;'>
                                    <option value="0" <?php if(input('cate_id') == '0'): ?>selected<?php endif; ?>>全部分类</option>
                                    <?php if(is_array($category) || $category instanceof \think\Collection || $category instanceof \think\Paginator): $i = 0; $__LIST__ = $category;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                                        <option value="<?php echo $vo['id']; ?>" <?php if(input('cate_id') == $vo['id']): ?>selected<?php endif; ?>><?php echo $vo['name']; ?></option>
                                        ...
                                        <option value="<?php echo $v['id']; ?>" <?php if(input('cate_id') == $v['id']): ?>selected<?php endif; ?>>&nbsp;&nbsp;&nbsp;<?php echo $v['name']; ?></option>
                                            ...
                                            <option value="<?php echo $vv['id']; ?>" <?php if(input('cate_id') == $vv['id']): ?>selected<?php endif; ?>>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php echo $vv['name']; ?></option>
                        ...
                        <div class="main-table table-responsive">
                            <form action="" method="post" enctype="multipart/form-data">
                                <table class="table table-hover table-condensed sort">
                                    ...
                                    <tbody><?php if(is_array($products) || $products instanceof \think\Collection || $products instanceof \think\Paginator): $i = 0; $__LIST__ = $products;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="del-<?php echo $vo['id']; ?>" data-id="<?php echo $vo['id']; ?>">
                                        <td class="id"><label><input class="selct-checkbox" type="checkbox" name="id" value="<?php echo $vo['id']; ?>"><?php echo $vo['id']; ?></label></td>
                                      	<td class="sanji-pro">
                                          <ul>
                                            <li data-toggle="tooltip" data-placement="top" title="" data-original-title="<?php echo $vo['name']; ?>">
                                              <div class="min-img"><img src="<?php echo $vo['thumb']; ?>" ondragstart="return false" alt="" title=""></div>
                                              <div class="min-title">
                                                <div class="name"><span><?php echo $vo['name']; ?></span></div>
                                              </div>
                                            </li>
                                          </ul>
                                        </td>
                                        <!--<td class="min-img"><img src="<?php echo $vo['thumb']; ?>" ondragstart="return false" alt="" title=""></td>
                                        <td class="title"><?php echo $vo['name']; ?></td>-->
                                        <td><?php echo $vo['category']['name']; ?></td>
                                        <td>￥<?php echo $vo['price']; ?></td>
                                        <td><?php echo $vo['total_stock']; ?></td>
                                        ...
                                        <td>
                                            <button type="button" class="state-pro" onclick="status('<?php echo $vo['id']; ?>','<?php echo $vo['status']; ?>')">
                                                <?php if($vo['status'] == 0): ?>
                                        ...
                                        <td>
                                            <a href="javascript:;" class="pro_vip" onclick="vipPrice(this,'<?php echo $vo['id']; ?>')" data-vipLink="<?php echo url('boguan/platform.vip/index'); ?>" data-toggle="modal" data-target=".vip-price">会员价</a>
                                            <span class="split">|</span>
                                            <a href="<?php echo url('boguan/Platform.product/edit',['id'=> $vo['id']]); ?>">编辑</a>
                                            <span class="split">|</span>
...
<script>
  function batch(type) {
     ...
     var text= '';
      ...
      layui.use(['layer','form'], function(){
           ...
           layer.confirm('确认'+ text +'吗？', {btn:['确认'], yes:function(index){
                   ...
                   $.ajax({
                       type: 'POST',
                       url: '<?php echo url("boguan/platform.product/batch"); ?>',
                       data:{
                           'ids': str,
                           'type':type
                       },
    ...
    $(function(){
            ...
            $(this).popModal({
                ...
                onOkBut : function(ev){
                    ...
                    $.ajax({
                        type: "post",
                        async:false,
                        url: "<?php echo url('boguan/platform.product/sort'); ?>",
        ...
        $('tr .erweima').click(function(){
            ...
            $.ajax({
                type: "post",
                async:false,
                url: "<?php echo url('boguan/platform.product/qrcode'); ?>",
                ...
                success: function(data) {
                    if(data.url == ''){
                        var imageUrl= '//www.lg0578.com/addons/boguan_mall/boguan/public/boguan//images/default_image.png';
                    }else {
    ...
    function setVipPrice(id,key){
        ...
        $.ajax({
            type: 'POST',
            url: '<?php echo url("boguan/platform.product/setVipPrice"); ?>',
            data:{'id':id},
            async: false, //设置为同步请求
    ...
    function saveVipPrice(id,data){
        $.ajax({
            type: 'POST',
            url: '<?php echo url("boguan/platform.product/saveVipPrice"); ?>',
            data:{
                'id':id,
    ...
    function del(id,key){
        ...
        layui.use(['layer','form'], function(){
            ...
            layer.confirm('确认删除吗？', {btn:['确认'], yes:function(index){
                    ...
                    $.ajax({
                        type: 'POST',
                        url: '<?php echo url("boguan/platform.product/del"); ?>',
                        data:{'id':id},
                        success: function(data) {
                            console.log(data);
                            if (data.errorCode == 1) {
    ...
    function status(id,status) {
        ...
        layui.use(['layer','form'], function(){
            ...
            layer.confirm('确认'+ text +'吗？', {btn:['确认'], yes:function(index){
                    ...
                    $.ajax({
                        type: 'POST',
                        url: '<?php echo url("boguan/platform.product/updateBannerStatus"); ?>',
                        data:{
                            'id':id,
                            'status': updateStatus,
                        },
