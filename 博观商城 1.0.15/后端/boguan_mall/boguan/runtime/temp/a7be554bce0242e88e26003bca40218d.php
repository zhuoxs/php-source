                ...
                <div class="main-con">
                    <form action="">
                        <ul class="nav-contral clearit">
                            <li><a href="<?php echo url('boguan/platform.printer/add'); ?>" class="btn"><i class="iconfont icon-jia"></i>添加打印机</a></li>
                        </ul>
                        <div class="main-table table-responsive">
                            <table class="table table-hover table-condensed">
                                ...
                                <tbody><?php if(is_array($printer) || $printer instanceof \think\Collection || $printer instanceof \think\Paginator): $i = 0; $__LIST__ = $printer;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="del-<?php echo $vo['id']; ?>">
                                    <td>
                                        <?php echo $vo['name']; ?>
                                    </td>
                                    <td>
                                        <?php if($vo['type'] == 'kdt2'): ?>365-kdt2云打印<?php elseif($vo['type'] == 'yilianyun-k4'): ?>易联云-k4<?php else: ?>飞鹅打印机<?php endif; ?>
                                    </td>
                                    <td><?php echo $vo['page']; ?>张</td>
                                    <td><?php echo $vo['printer_status']; ?></td>
                                    <td>
                                    ...
                                    <td>
                                        <a href="<?php echo url('boguan/platform.printer/edit',['id'=> $vo['id']]); ?>">修改</a>
    ...
    function status(id,status) {
        ...
        layui.use(['layer','form'], function(){
            ...
            layer.confirm('确认'+ text +'吗？', {btn:['确认'], yes:function(index){
                    ...
                    $.ajax({
                        type: 'POST',
                        url: '<?php echo url("boguan/platform.printer/updateStatus"); ?>',
                        data:{
                            'id':id,
                            'status': updateStatus,
                        },
    ...
    function del(id,key){
        ...
        layui.use(['layer','form'], function(){
            ...
            layer.confirm('确认删除吗？', {btn:['确认'], yes:function(index){
                    ...
                    $.ajax({
                        type: 'POST',
                        url: '<?php echo url("boguan/platform.printer/del"); ?>',
                        data:{'id':id},
                        success: function(data) {
                            console.log(data);
                            if (data.errorCode == 1) {
