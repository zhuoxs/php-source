                  ...
                  <div class="toggle-tipsbox">
                    ...
                    <form action="">
                        <ul class="nav-contral clearit nav nav-tabs">
                            <li role="presentation" class="nav-tabs-li <?php if(input('status')== ''): ?>active<?php endif; ?>"><a href="<?php echo url('boguan/platform.share/index'); ?>">全部</a></li>
                            <li role="presentation" class="nav-tabs-li <?php if(input('status') == '0'): ?>active<?php endif; ?>"><a href="<?php echo url('boguan/platform.share/index',['status'=> 0]); ?>">待审核</a></li>
                            <li role="presentation" class="nav-tabs-li <?php if(input('status') == '1'): ?>active<?php endif; ?>"><a href="<?php echo url('boguan/platform.share/index',['status'=> 1]); ?>">审核通过</a></li>
                            <li role="presentation" class="nav-tabs-li <?php if(input('status') == '2'): ?>active<?php endif; ?>"><a href="<?php echo url('boguan/platform.share/index',['status'=> 2]); ?>">审核不通过</a></li>
                            <li role="presentation" class="nav-tabs-li <?php if(input('status') == '3'): ?>active<?php endif; ?>"><a href="<?php echo url('boguan/platform.share/index',['status'=> 3]); ?>">已清退</a></li>
                            <li class="pull-right">
                                <input type="submit" class="right-input btn" value="搜索">
                            </li>
                            <li class="pull-right">
                                <input type="text"  class="right-input" placeholder="姓名" name="keyword" value="<?php echo input('keyword'); ?>">
                            </li>
                        </ul>
                        <div class="main-table table-responsive">
                            <table class="table table-hover table-condensed">
                                ...
                                <tbody><?php if(is_array($share) || $share instanceof \think\Collection || $share instanceof \think\Paginator): $i = 0; $__LIST__ = $share;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="del-<?php echo $vo['id']; ?>">
                                    <td class="min-img"><div><img src="<?php echo $vo['user']['avatar']; ?>" ondragstart="return false" alt="" title=""></div></td>
                                    <td class="title"><?php echo $vo['user']['nickname']; ?></td>
                                    <td><?php echo $vo['name']; ?>/<?php echo $vo['phone']; ?></td>
                                    <td>
                                        <?php if($vo['parent']): ?><?php echo $vo['parent']['nickname']; ?><br><?php echo $vo['parent']['phone']; else: ?>总店<?php endif; ?>
                                    </td>
                                    <?php if($vo['user']['is_distributor'] == '1'): ?>
                                    <td><?php echo $vo['total_sub']; ?></td>
                                    <td><?php echo $vo['user']['total_share_money']; ?></td>
                                    <td><?php echo $vo['not_settle_money']; ?></td>
                                    <td><?php echo $vo['user']['share_money']; ?></td>
                                    <td><?php echo $vo['user']['distributor_time']; ?></td>
                                    <?php else: ?>
                                        <td>-</td>
                                        <td>-</td>
                                        <td>-</td>
                                    ...
                                    <td>
                                        <?php if($vo['status'] == '0'): ?>
                                        <a href="javascript:;" onclick="status('<?php echo $vo['id']; ?>','1')">审核通过</a>
                                        <span class="split">|</span>
                                        <a href="javascript:;" onclick="status('<?php echo $vo['id']; ?>','2')">审核不通过</a>
                                        <span class="split">|</span>
                                        <?php elseif($vo['status'] == 1): ?>
                                        <a href="javascript:;" onclick="status('<?php echo $vo['id']; ?>','3')">清退</a>
                                        <span class="split">|</span>
                                        <?php endif; ?>
                                        <a href="javascript:;" onclick="del('<?php echo $vo['id']; ?>','del-<?php echo $vo['id']; ?>','<?php echo $vo['user_id']; ?>')">删除</a>
                                    </td>
...
function status(id,status) {
    ...
    layui.use(['layer','form'], function(){
        ...
        layer.confirm('确认'+ text +'吗？', {btn:['确认'], yes:function(index){
                ...
                $.ajax({
                    type: 'POST',
                    url: '<?php echo url("boguan/platform.share/updateStatus"); ?>',
                    data:{
                        'id':id,
                        'status': status,
                    },
    ...
    function del(id,key,user){
        ...
        layui.use(['layer','form'], function(){
            ...
            layer.confirm('确认删除吗？', {btn:['确认'], yes:function(index){
                    ...
                    $.ajax({
                        type: 'POST',
                        url: '<?php echo url("boguan/platform.share/del"); ?>',
