                ...
                <div class="main-con">
                    <form action="">
                        <ul class="nav-contral clearit">
                            <li><a href="<?php echo url('boguan/platform.home/add'); ?>" class="btn"><i class="iconfont icon-jia"></i>新增页面</a></li>
                        </ul>
                        <div class="main-table table-responsive">
                            <table class="table table-hover table-condensed">
                                ...
                                <tbody><?php if(is_array($home) || $home instanceof \think\Collection || $home instanceof \think\Paginator): $i = 0; $__LIST__ = $home;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="del-<?php echo $vo['id']; ?>" data-id="<?php echo $vo['id']; ?>">
                                    <td class="id">
                                        <?php echo $vo['id']; ?>
                                    </td>
                                    <td class="sanji-pro min-pro"><?php echo $vo['name']; ?>
                                        &nbsp;<?php if($vo['is_home'] == 1): ?><span class="label label-primary">首页</span><?php endif; ?></td>
                                    <td><?php echo checkData($vo['update_time'],'time'); ?></td>
                                    <td><?php echo $vo['create_time']; ?></td>
                                    <td>
                                        <a href="javascript:;" onclick="setDefault('<?php echo $vo['id']; ?>','<?php echo $vo['name']; ?>')">设为首页</a>
                                        <span class="split">|</span>
                                        <a href="<?php echo url('boguan/platform.home/edit',['id'=> $vo['id']]); ?>">编辑</a>
                                        <span class="split">|</span>
    ...
    function setDefault(id,name){
        ...
        layui.use(['layer','form'], function(){
            ...
            layer.confirm('确认将（'+ name +'）设为主页吗？', {btn:['确认'], yes:function(index){
                    ...
                    $.ajax({
                        type: 'POST',
                        url: '<?php echo url("boguan/platform.home/setDefault"); ?>',
                        data:{'id':id},
                        success: function(data) {
                            console.log(data);
                            if (data.errorCode == 1) {
    ...
    function del(id,key){
        ...
        layui.use(['layer','form'], function(){
            ...
            layer.confirm('确认删除吗？', {btn:['确认'], yes:function(index){
                    ...
                    $.ajax({
                        type: 'POST',
                        url: '<?php echo url("boguan/platform.home/del"); ?>',
                        data:{'id':id},
                        success: function(data) {
                            console.log(data);
                            if (data.errorCode == 1) {
