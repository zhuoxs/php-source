                ...
                <div class="main-con clerk-list member-list user-list">
                    ...
                    <form action="" method="get" enctype="multipart/form-data">
                        <ul class="nav-contral clearit">
                            <li><a href="<?php echo url('boguan/platform.admin/add'); ?>" class="btn"><i class="iconfont icon-jia"></i>新增员工</a></li>

                            <li class="pull-right">
                                <input type="submit" class="right-input btn" value="搜索">
                            </li>
                            <li class="pull-right">
                                <input type="text" name="keyword"  class="right-input" placeholder="用户名" value="<?php echo input('keyword'); ?>">
                            </li>
                        </ul>
                    </form>
                    <div class="main-table table-responsive">
                        <table class="table table-hover table-condensed">
                            ...
                            <tbody><?php if(is_array($clerk) || $clerk instanceof \think\Collection || $clerk instanceof \think\Paginator): $i = 0; $__LIST__ = $clerk;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="del-<?php echo $vo['id']; ?>">
                                ...
                                <td><?php echo date('Y-m-d H:i:s',$vo['clerk_time']); ?></td>
                                <td><?php echo $vo['clerk_remark']; ?></td>
                                <td><?php if($vo['is_clerk'] == 1): ?>核销员<?php elseif($vo['is_mobile'] == 1): ?>手机管理员<?php else: ?>其他<?php endif; ?></td>
                                <td><?php echo $vo['clerk_order']; ?></td>
                                <td><?php echo $vo['clerk_amount']; ?></td>
                                <td>
                                    <a href="<?php echo url('boguan/platform.admin/edit',['id'=> $vo['id']]); ?>">编辑</a>
                                    <span class="split">|</span>
                                    <a href="javascript:;" onclick="cancel('<?php echo $vo['id']; ?>','del-<?php echo $vo['id']; ?>','<?php if($vo['is_mobile'] == 1): ?>mobile<?php else: ?>clerk<?php endif; ?>')">解除</a>
                                </td>
    ...
    function cancel(id,key,type){
        ...
        layui.use(['layer','form'], function(){
            ...
            layer.confirm('确认解除'+ text +'吗？', {btn:['确认'], yes:function(index){
                    ...
                    $.ajax({
                        type: 'POST',
                        url: '<?php echo url("boguan/platform.admin/cancelClerk"); ?>',
    ...
    function del(id,key){
        ...
        layui.use(['layer','form'], function(){
            ...
            layer.confirm('确认删除吗？', {btn:['确认'], yes:function(index){
                    ...
                    $.ajax({
                        type: 'POST',
                        url: '<?php echo url("boguan/platform.admin/del"); ?>',
                        data:{'id':id},
                        success: function(data) {
                            console.log(data);
                            if (data.errorCode == 1) {
