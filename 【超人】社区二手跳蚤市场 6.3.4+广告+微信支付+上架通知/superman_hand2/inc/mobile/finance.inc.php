...
if ($act == 'display') {
    ...
    if ($result['getcash']['getcash_type']) {
        foreach ($result['getcash']['getcash_type'] as $k => &$type) {
            switch ($k) {
                case '1':
                    $type = array(
                        'type' => 'wechat',
    ...
    $result['member'] = pdo_get('superman_hand2_member', array(
        'uniacid' => $_W['uniacid'],
        'uid' => $_W['member']['uid']
    ));
    ...
    if (checksubmit()) {
        ...
        if ($money > $result['member']['balance']) {
        ...
        if ($ret1 !== false && $ret2 > 0 && $ret3 > 0) {
            pdo_commit();
            $data = array(
                'url' => $this->createMobileUrl('finance', array('act' => 'getcash_log')),
            );
    ...
    $list = pdo_getall('superman_hand2_member_getcash_log', $filter, '', '', '');
    if (!empty($list)) {
        foreach ($list as &$li) {
            $li['status_title'] = SupermanHandUtil::get_getcash_status_title($li['status']);
    ...
    $list = pdo_getall('superman_hand2_member_money_log', $filter, '', '', $orderby);
    if (!empty($list)) {
        foreach ($list as &$li) {
            $li['createtime'] = date('Y-m-d H:i:s', $li['createtime']);
        }
...
include $this->template('finance/'.$act);
