...
$pagesize=10;
$where=' WHERE  a.uniacid=:uniacid  and a.out_trade_no=1';
if($_GPC['express']==1){
//    if($_GPC['express_num'] || $_GPC['express_com']){
    ...
    if($res){
        message('发货成功',$this->createWebUrl('bargainorder',array()),'success');
    }
//    }
}
...
if($operation=='delete'){
   ...
   //$res=pdo_update('chbl_sun_order',array('is_delete'=>1),array('id'=>$_GPC['id']));
	if($res){
		message('删除成功',$this->createWebUrl('bargainorder',array()),'success');
	}else{
		message('删除失败','','error');
	}
}
if($operation=='delivery'){
   ...
   $res=pdo_update('chbl_sun_order',array('state'=>3,'fh_time'=>time()),array('id'=>$_GPC['id']));
	if($res){
		message('操作成功',$this->createWebUrl('bargainorder',array()),'success');
	}else{
		message('操作失败','','error');
	}
}
...
include $this->template('web/bargainorder');