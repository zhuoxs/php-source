...
$type2=pdo_get('chbl_sun_storetype2',array('id'=>$info['storetype2_id']));
//查出已是商家用户
...
//入住类型
//$typein=pdo_getall('chbl_sun_storein',array('uniacid'=>$_W['uniacid']));
   $time=24*60*60*7;//一周
...
if(checksubmit('submit')){
		...
		if($_GPC['user_name']){
			$rst=pdo_get('chbl_sun_store',array('user_name'=>$_GPC['user_name'],'uniacid'=>$_W['uniacid'],'id !='=>$_GPC['id']));
			if($rst){
			...
			if($_GPC['type']){
				...
				$res = pdo_update('chbl_sun_store', $data, array('id' => $_GPC['id']));
				if($res){
		 ...
		 function  getCoade($md_id){
          ...
          function set_msg($md_id){
           ...
           $data2=array(
            "scene"=>$md_id,
            "page"=>"chbl_sun/pages/sellerinfo/sellerinfo",
            "width"=>100
            );
