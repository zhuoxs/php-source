...
if($_GPC['keywords']){
	...
	$pagesize=10;
	$sql="select *  from " . tablename("chbl_sun_user") .$where;
	$select_sql =$sql." LIMIT " .($pageindex - 1) * $pagesize.",".$pagesize;
	$list = pdo_fetchall($select_sql,$data);	

	$total=pdo_fetchcolumn("select count(*) from " . tablename("chbl_sun_user") .$where,$data);
	$pager = pagination($total, $pageindex, $pagesize);
	if($_GPC['op']=='delete'){
		$res4=pdo_delete("chbl_sun_user",array('id'=>$_GPC['id']));
		if($res4){
		...
		if($_GPC['op']=='defriend'){
		$res4=pdo_update("chbl_sun_user",array('state'=>2),array('id'=>$_GPC['id']));
		if($res4){
