...
$info=pdo_get('chbl_sun_vip',array('id'=>$_GPC['id'],'uniacid'=>$_W['uniacid']));
    if(checksubmit('submit')){
        ...
        if(empty($_GPC['id'])){
            $num = count(pdo_getall('chbl_sun_vip',array('uniacid'=>$_W['uniacid'])));
            if($num>=3){
                message('最多只能添加三个','','error');
            }else{
                $res = pdo_insert('chbl_sun_vip', $data,array('uniacid'=>$_W['uniacid']));
            }
            if($res){
            ...
            $res = pdo_update('chbl_sun_vip', $data, array('id' => $_GPC['id'],'uniacid'=>$_W['uniacid']));
        }
        if($res){
