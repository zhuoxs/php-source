...
$info=pdo_get('chbl_sun_information',array('id'=>$_GPC['id']));
//$type=pdo_get('chbl_sun_type',array('id'=>$info['type_id']));
$type=pdo_getall('chbl_sun_type',array('uniacid'=>$_W['uniacid'],'state'=>1),array(),'','num asc');
$type2=pdo_get('chbl_sun_type2',array('id'=>$info['type2_id']));
...
if(checksubmit('submit')){
		if($_GPC['img']){
			...
			if($_GPC['cityname']){
				...
				$res = pdo_update('chbl_sun_information', $data, array('id' => $_GPC['id']));
				if($res){
