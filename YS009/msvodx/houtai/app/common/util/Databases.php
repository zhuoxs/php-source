...
class Databases {
    ...
    public function backup($table, $start){
        //备份表结构
        if(0 == $start){
            $result = $this->myDb->query("SHOW CREATE TABLE `{$table}`");
            $sql  = "\n";
            $sql .= "-- -----------------------------\n";
        ...
        //数据总数
        $result = $this->myDb->query("SELECT COUNT(*) AS count FROM `{$table}`");
        ...
        if($count){
            ...
            //备份数据记录
            $result = $this->myDb->query("SELECT * FROM `{$table}` LIMIT {$start}, 1000");
