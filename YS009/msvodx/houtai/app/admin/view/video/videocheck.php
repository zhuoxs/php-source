    ...
    <div class="layui-form">
        <table class="layui-table mt10" lay-even="" lay-skin="row">
            ...
            <tr>
              ...
              <td>{if condition="$vo['is_check'] eq 0"}
                    <span style="color:blue">未处理</span>
                    {/if}
                    {if condition="$vo['is_check'] eq 2"}
                    <span style="color:red">已拒绝</span>
                    {/if}
                    <input type="checkbox"  {if condition="$vo['is_check'] eq 1"}checked=""{/if} value="{$vo['is_check']}" lay-skin="switch" lay-filter="switchStatus" lay-text="通过|未审" data-href="{:url('check?table=video&ids='.$vo['id'])}">
                </td>
				 <td>
