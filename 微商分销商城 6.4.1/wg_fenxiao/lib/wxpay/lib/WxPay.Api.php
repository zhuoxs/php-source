...
class WxPayApi
{
	...
	public static function unifiedOrder($inputObj, $timeOut = 6)
	{
		...
		if(!$inputObj->IsNotify_urlSet()){
            return [
                'code' => 407,
                'msg' => "notify url not set"
            ];
		}
		
		$inputObj->SetAppid($_W['module_setting']['pay']['appid']);//公众账号ID
		$inputObj->SetMch_id($_W['module_setting']['pay']['mchid']);//商户号
		...
		if($result['return_code'] != 'SUCCESS') {
		    return [
		        'code' => 408,
                'msg' => "统一支付接口失败" . $result['return_msg']
            ];
        }
        return [
            'code' => 0,
