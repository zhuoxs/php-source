...
class Refund_KundianFarmModel{
    ...
    public function refundLand($order_id){
        load()->model('refund');

        $order=pdo_get(TABLE_PRE.'land_order',['id'=>$order_id,'uniacid'=>$this->uniacid]);
        if ($order['is_cancel'] == 0 && $order['is_confirm'] == 0) {

            pdo_begin();
            $r1 = pdo_update(TABLE_PRE . 'land_order', ['is_refund' => 1, 'refund_time' => time()], ['id' => $order_id, 'uniacid' => $this->uniacid]);
            $r2 = pdo_update(TABLE_PRE . 'land_spec', ['status' => 0], ['id' => $order['spec_id'], 'uniacid' => $this->uniacid]);

            if ($r1 && $r2){
                $refundid=refund_create_order($order['order_number'],'kundian_farm');
                $result=$this->refund($refundid);
                if($result['return_code']=="SUCCESS") {
                    pdo_commit();
                    return ['code'=>0,'msg'=>'退款成功'];
    ...
    public function refundApplyStore($order_id){
        load()->model('refund');
        $order=pdo_get("kd_farm_store",['id'=>$order_id,'uniacid'=>$this->uniacid]);

        if ($order['is_refund'] == 0 && $order['is_pay'] == 1) {
            pdo_begin();
            $r1 = pdo_update('kd_farm_store',['is_refund'=>1,'refund_time'=>time()],['id'=>$order_id,'uniacid'=>$this->uniacid]);
            if ($r1){
                $refundid=refund_create_order($order['order_number'],'kundian_farm_plugin_store');
                $result=$this->refund($refundid);
                if($result['return_code']=="SUCCESS") {
                    pdo_commit();
                    return ['code'=>0,'msg'=>'退款成功'];
