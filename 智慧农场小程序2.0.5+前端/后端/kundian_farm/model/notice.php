...
class Notice_KundianFarmModel{
     ...
     * @return bool|string
     */
    public function noticeControlChange($did,$touser,$page,$uid,$info){
        ...
        $dd=$this->returnParam($touser,$wxData['mini_device_template_id'],$page,$form_id['formid'],$value);
    ...
    public function sendMerchantInfo($touser,$orderData,$uniacid,$type,$order_type,$wxData,$row){
        ...
        $template = [
            ...
            "miniprogram"=>[
                "appid"=>$row['key'],
                "pagepath"=>"kundian_farm/pages/manage/center/index"
            ]
        ];
     ...
     * @return mixed
     */
    public function sendTaskCompleteNotice($orderData,$remark,$uid){
        $account_api = WeAccount::create();
        $access_token=$account_api->getAccessToken();
        $playSet=self::$common->getSetData(['task_template_id'],$this->uniacid);
        $user=pdo_get(TABLE_PRE.'user',['uid'=>$uid,'uniacid'=>$this->uniacid]);
        $value = array(
            "keyword1"=>["value"=>$orderData['body'], "color"=>"#4a4a4a"],
        ...
        $url = 'https://api.weixin.qq.com/cgi-bin/message/wxopen/template/send?access_token='.$access_token;
        $dd=$this->returnParam($user['openid'],$playSet['task_template_id'],'/',$orderData['form_id'],$value);
        $result = $this->https_curl_json($url,$dd,'json');
        return $result;
    }
