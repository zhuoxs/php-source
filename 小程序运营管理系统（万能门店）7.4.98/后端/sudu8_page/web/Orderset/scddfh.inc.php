...
global $_GPC, $_W;
        ...
        if($order_id){
            $orders = pdo_fetchall("SELECT * FROM ".tablename('sudu8_page_duo_products_order')." WHERE uniacid = :uniacid and nav = 2 and order_id LIKE :order_id order by creattime desc" , array(':uniacid' => $_W['uniacid'],':order_id' => '%'.$order_id.'%'));
            ...
            $orders = pdo_fetchall("SELECT * FROM ".tablename('sudu8_page_duo_products_order')." WHERE uniacid = :uniacid and nav = 2 order by creattime desc" , array(':uniacid' => $_W['uniacid']));
            foreach ($orders as $key => &$res) {
                $res['jsondata'] = unserialize($res['jsondata']);
                $res['creattime'] = date("Y-m-d H:i:s",$res['creattime']);
...
return include self::template('web/Orderset/scddfh');
