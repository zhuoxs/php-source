		...
		this.each(function () {
			...
			if(typeof(options) == 'object') {
				if(typeof(options.ranges) != 'object') {
					...
					var day = now.getDate();
					options.ranges['ä»Šå¤©'] = [new Date(year, month, day, 0, 0, 0), moment()];
					options.ranges['ä¸€å‘¨å†…'] = [moment().subtract('days', 6), moment()];
					options.ranges['äºŒå‘¨å†…'] = [moment().subtract('days', 13), moment()];
					options.ranges['ä¸€æœˆå†…'] = [moment().subtract('days', 29), moment()];
				}
				if(typeof(options.locale) != 'object') {
					options.locale = {
						applyLabel: "ç¡®å®š",
