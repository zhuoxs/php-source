	...
	if (div.length > 0) {
		...
		batch.on("click", function() {
						...
						if(("status" in data)){
							...
							if (data.type == 'success') {
								if ($this.data('toggle') == 'batch-remove') {
									...
									batch.attr("disabled", 0 == $('table tbody tr td:first-child [type="checkbox"]:checked', div).length);
									tip.msgbox.suc(data.message || tip.lang.success, data.redirect)
								}
								buttontype == 'button' ? $this.html(html) : $this.val(html)
							} else {
								buttontype == 'button' ? $this.html(html) : $this.val(html);
								tip.msgbox.err(data.message || tip.lang.error)
							}
						}
			...
			var t = $(e.target).closest("table"),
				checked = $(e.target).is(":checked");
			$('tbody tr td:first-child  [type="checkbox"]', t).prop("checked", checked), batch.add(trigger).attr("disabled", !checked);
			...
			$('thead th:first [type="checkbox"]', t).prop("checked", checked && chk.length == checkboxs.length), batch.add(trigger).attr("disabled", !checked)
		}), $(document).on("click", '.table-responsive tbody [data-toggle="ajaxConfirm"]', function(e) {
			e.preventDefault();
			var obj = $(this),
			...
			var submit = function() {
					...
					$.post(url).done(function(data) {
						...
						if (data.type == 'success') {
							...
							obj.html(scop);
							tip.msgbox.err(data.message || tip.lang.error, data.redirect);
						}
					}).fail(function() {
						obj.html(scop);
