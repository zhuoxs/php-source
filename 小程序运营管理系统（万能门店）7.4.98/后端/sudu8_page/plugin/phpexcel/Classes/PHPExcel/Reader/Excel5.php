...
//	 Support for account-defined named cells added as an array "namedcells"
//		 Patch code for account-defined named cells supports single cells only.
...
//					echo '<br />';
	...
	private function _loadOLE($pFilename)
	{
		...
		// Get account-defined property data
...
//		echo '<br />';
	...
	private function _readXf()
	{
		...
		if (!$this->_readDataOnly) {
			...
			if (isset($this->_formats[$numberFormatIndex])) {
				// then we have account-defined format code
	...
	private function _readStyle()
	{
		...
		if (!$this->_readDataOnly) {
			...
			// bit: 15; mask 0x8000; 0 = account-defined style, 1 = built-in style
			...
			if ($isBuiltIn) {
				...
				// account-defined; not supported by PHPExcel
			}
		}
	}
	...
	private function _readDefinedName()
	{
		...
		if ($this->_version == self::XLS_BIFF8) {
			...
			$opts = self::_GetInt2d($recordData, 0);
				...
				// bit: 5; mask: 0x0020; 0 = account-defined name, 1 = built-in-name
...
//		echo '<br />';
	...
	private function _readSheetProtection()
	{
		...
		// bit: 0; mask 0x0001; 1 = account may edit objects, 0 = users must not edit objects
