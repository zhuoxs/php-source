...
class Tree implements Renderable
{
    ...
    public function getController(){
        $model = explode('\\', get_class($this->model));
    ...
    public function __construct(Model $model = null, \Closure $callback = null)
    {
        ...
        $this->path =  Url::generate($this->getController(),'edit');//app('request')->getPathInfo();
    ...
    protected function script()
    {
        ...
        $path =  Url::generate($this->getController(),'destroy');
        return <<<SCRIPT
...
SCRIPT;
    ...
    public function getItemTitle(&$item)
    {
        if(is_array($item))
        {
            $item["title"] =call_user_func($this->branchCallback,$item);
            if(isset($item["children"])){
                foreach ($item["children"] as &$subItem)
                $this->getItemTitle($subItem);
            }
        }
    }
