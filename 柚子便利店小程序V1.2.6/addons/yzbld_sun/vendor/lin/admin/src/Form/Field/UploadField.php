...
//            'initialCaption'       => $this->initialCaption($this->value),
        ...
        if ($this->form instanceof Form) {
            $defaultOptions['deleteUrl'] = Url::update($this->form->resource(),$this->form->model()->getKey());//$this->form->resource().'/'.$this->form->model()->getKey();
        }
    ...
    public function objectUrl($path)
    {
        ...
        if($this->isExternalStorage){
            return Config::get("uploadUrl","/").$path;
        }else{
            return "/attachment/images/".$path;
        }
    ...
    {
        return md5(uniqid()).'.'.end(explode(".",basename($file["name"])));
    }
    ...
    public function destroy()
    {
        if(!empty($this->original)){
            $file = $this->getDirectory().$this->original;
            if(file_exists($file)){
                unlink($file);
            }
        }
