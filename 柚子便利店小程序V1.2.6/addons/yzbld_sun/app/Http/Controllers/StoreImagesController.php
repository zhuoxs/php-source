...
class StoreImagesController extends Controller{
    protected $header = '商家轮播图';
    public function grid()
    {
        return $this->admin->grid(StoreImages::class, function (Grid $grid) {
            $grid->model()->ordered();
            $grid->column('id', 'id');
            $grid->column('title', '标题');
            $grid->column('main_image', '图片')->image(null, 150, 70);
            $grid->column('store_id', '店铺')->display(function($store_id){
    ...
    {
        return $this->admin->form(StoreImages::class, function (Form $form) use ($id) {
            $form->text('title', '标题')->addRequired();
            $form->image('main_image', '图片')->help("建议尺寸:750*320");

            //获得店铺id
            $options_store = Store::instance()->pluck('name', 'id');
            $form->select('store_id', '店铺')
            ...
            if (null == $id) {
                ...
                $action_type = StoreImages::instance()->find($id)->action_type;
                if($action_type == 0)
                {
                    $options = config('actions');
                }
