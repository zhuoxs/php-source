...
{
    protected $header = '首页轮播图';

    public function grid()
    {
        return $this->admin->grid(Banner::class, function (Grid $grid) {
            $grid->model()->ordered();
            $grid->column('id', 'id');
    ...
    {
        return $this->admin->form(Banner::class, function (Form $form) use ($id) {
            ...
            if (null == $id) {
                ...
                $action_type = Banner::instance()->find($id)->action_type;
                if($action_type == 0)
                {
                    $options = config('actions');
                }
