...
$list = pdo_getall('yzkm_sun_bargain',array('uniacid' => $_W['uniacid']));

if($_GPC['op']=='delete'){
    if($_W['ispost']){
        $res=pdo_delete('yzkm_sun_bargainlist',array('id'=>$_GPC['id'],'uniacid'=>$_W['uniacid']));
        if($res){
...
if($_GPC['op']=='change') {
    $res = pdo_update('yzkm_sun_bargain', array('status' => $_GPC['status']), array('id' => $_GPC['id']));
    if ($res) {
