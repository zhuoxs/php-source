...
$certfile = IA_ROOT . "/addons/yzkm_sun/cert/" . 'apiclient_cert_' .$_W['uniacid'] . '.pem';
$keyfile = IA_ROOT . "/addons/yzkm_sun/cert/" . 'apiclient_key_' . $_W['uniacid'] . '.pem';
...
// 微信提现证书上传
        ...
        if(is_uploaded_file($_FILES['apiclient_cert']['tmp_name'])) {
            ...
            //我们给每个用户动态的创建一个文件夹
            $user_path=IA_ROOT."/addons/yzkm_sun/cert/";
        ...
        if(is_uploaded_file($_FILES['apiclient_key']['tmp_name'])) {
            ...
            $data['apiclient_cert']=mt_rand(1, 10);
            $data['apiclient_key']=mt_rand(1, 10);
...
//            }
            ...
            if($_GPC['id']==''){                
                $res=pdo_insert('yzkm_sun_system',$data,array('uniacid'=>$_W['uniacid']));
                if($res){
                ...
                $res = pdo_update('yzkm_sun_system', $data, array('id' => $_GPC['id'],'uniacid'=>$_W['uniacid']));
                if($res){
