...
$info=pdo_get('yzkm_sun_vip',array('id'=>$_GPC['id'],'uniacid'=>$_W['uniacid']));
    if(checksubmit('submit')){
        ...
        $data['day']=$_GPC['day'];
        $data['price']=$_GPC['price1'];
        $data['prefix']=$_GPC['prefix'];
        $data['status']=1;
        // $data['card_number']=date('Ymd') . str_pad(mt_rand(1, 99999), 5, '0', STR_PAD_LEFT);
        $data['time']=date('Y-m-d H:i:s',time());

        if(empty($_GPC['id'])){
            $num = count(pdo_getall('yzkm_sun_vip',array('uniacid'=>$_W['uniacid'])));
            if($num>=3){
                message('最多只能添加三个','','error');
            }else{
                $res = pdo_insert('yzkm_sun_vip', $data,array('uniacid'=>$_W['uniacid']));
            }
            if($res){
            ...
            $res = pdo_update('yzkm_sun_vip', $data, array('id' => $_GPC['id'],'uniacid'=>$_W['uniacid']));
        }
        if($res){
