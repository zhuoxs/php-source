...
if($type=='all'){   
  $sql="select * from" . tablename("yzkm_sun_car") .$where."  order by top,time desc ";
  $total=pdo_fetchcolumn("select count(*)  from " . tablename("yzkm_sun_car") .$where,$data);
}else{
...
if($_GPC['op']=='delete'){
    $res=pdo_delete('yzkm_sun_car',array('id'=>$_GPC['id']));
    if($res){
