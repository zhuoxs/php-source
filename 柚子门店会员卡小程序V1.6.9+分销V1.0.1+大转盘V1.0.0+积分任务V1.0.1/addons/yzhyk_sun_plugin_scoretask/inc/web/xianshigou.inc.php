...
$sql = 'SELECT COUNT(*) FROM ' . tablename('yzhyk_sun_goods') .$condition ;

$total = pdo_fetchcolumn($sql, $params);

if (!empty($total)) {
    $sql = 'SELECT * FROM  ' . tablename('yzhyk_sun_goods') .$condition.' ORDER BY  `time`  DESC LIMIT ' . ($pindex - 1) * $psize . ',' . $psize;
...
if($operation == 'delete'){
    ...
    $res = pdo_delete('yzhyk_sun_goods',array('id'=>$id));
    if($res){
