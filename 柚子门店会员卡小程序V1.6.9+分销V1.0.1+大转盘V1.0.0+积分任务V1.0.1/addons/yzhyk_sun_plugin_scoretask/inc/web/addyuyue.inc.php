...
$info=pdo_get('yzhyk_sun_goods',array('id'=>$_GPC['id']));
$spec=pdo_getall('yzhyk_sun_goods_spec');
$type=pdo_getall('yzhyk_sun_type',array('uniacid'=>$_W['uniacid'],'state'=>1));
...
if(checksubmit('submit')){
    ...
    if(empty($_GPC['id'])){
        $res = pdo_insert('yzhyk_sun_goods', $data);
    }else{

        $res = pdo_update('yzhyk_sun_goods', $data, array('id' => $_GPC['id']));
    }
    if($res){
        message('编辑成功',$this->createWebUrl('yuyue',array()),'success');
    }else{
        message('编辑失败','','error');
    }
}
include $this->template('web/addyuyue');