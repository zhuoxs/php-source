...
$category=pdo_getall('yzhyk_sun_activity_category',array('uniacid'=>$_W['uniacid']));
$store=pdo_getall('yzhyk_sun_store',array('uniacid'=>$_W['uniacid'],'state'=>2));
if(checksubmit('submit')){
    ...
    if(empty($_GPC['id'])){
        $res = pdo_insert('yzhyk_sun_activity', $data);
        if($res){
        ...
        unset($data['add_time']);
        $res = pdo_update('yzhyk_sun_activity', $data, array('id' => $_GPC['id']));
        if($res){
