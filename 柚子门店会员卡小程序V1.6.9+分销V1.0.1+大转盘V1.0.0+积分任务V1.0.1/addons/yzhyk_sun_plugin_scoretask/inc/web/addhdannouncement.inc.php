...
$info=pdo_get('yzhyk_sun_announcement',array('id'=>$_GPC['id']));
if(checksubmit('submit')){
    ...
    $data['add_time']= time();  
    if(empty($_GPC['id'])){
        $res = pdo_insert('yzhyk_sun_announcement', $data);
        if($res){
        ...
        $res = pdo_update('yzhyk_sun_announcement', $data, array('id' => $_GPC['id']));
        if($res){
