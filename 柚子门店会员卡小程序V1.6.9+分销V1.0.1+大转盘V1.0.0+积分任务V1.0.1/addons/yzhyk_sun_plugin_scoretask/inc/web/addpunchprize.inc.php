...
$result=$result[$num];
$coupon=pdo_getall('yzhyk_sun_coupon',array('uniacid'=>$_W['uniacid'],'is_punch'=>1));
$info=pdo_get('yzhyk_sun_punch_prize',array('id'=>$_GPC['id']));
if(checksubmit('submit')){
    ...
    $coupon=pdo_get('yzhyk_sun_coupon',array('uniacid'=>$_W['uniacid'],'id'=>$_GPC['coupon_id']));
    if(empty($coupon)){
    ...
    if(empty($_GPC['id'])){
        $res = pdo_insert('yzhyk_sun_punch_prize', $data);
        if($res){
        ...
        unset($data['add_time']);
        $res = pdo_update('yzhyk_sun_punch_prize', $data, array('id' => $_GPC['id']));
        if($res){
