...
$info=pdo_get('yzhyk_sun_plugin_scoretask_taskset',array('id'=>$_GPC['id']));
...
if(checksubmit('submit')){
    ...
    $data['add_time']= time();
    if(empty($_GPC['id'])){
        $res = pdo_insert('yzhyk_sun_plugin_scoretask_taskset', $data);
        if($res){
        ...
        unset($data['add_time']);
        $res = pdo_update('yzhyk_sun_plugin_scoretask_taskset', $data, array('id' => $_GPC['id']));
        if($res){
