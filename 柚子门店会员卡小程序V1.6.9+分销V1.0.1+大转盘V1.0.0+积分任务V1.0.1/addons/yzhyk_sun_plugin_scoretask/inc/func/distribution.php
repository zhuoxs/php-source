...
class Distribution{
    ...
    private function getgoodsdistribution(){
        ...
        switch ($ordertype) {
            case '1'://充值续费
                $orderinfo = pdo_get('yzhyk_sun_order', array('id' => $order_id, 'uniacid' => $uniacid),array("activity_id"));
                break;
            case '3'://活动
                $orderinfo = pdo_get('yzhyk_sun_order', array('id' => $order_id, 'uniacid' => $uniacid),array("activity_id"));
                break;
        ...
        if($orderinfo){
            if($ordertype==3){
                $goods = pdo_get('yzhyk_sun_activity', array('id' => $orderinfo["activity_id"], 'uniacid' => $uniacid),array("distribution_open","distribution_commissiontype","firstmoney","secondmoney","thirdmoney"));
            }
            if($goods){
