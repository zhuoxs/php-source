...
//    批量推荐
    case "batchhot":
        $ids = $_GPC['ids'];
        $ids = explode(',',$ids);
        $ret=pdo_update("yzhyk_sun_storegroupgoods",['is_hot'=>1],array('id'=>$ids));
        if($ret){
...
//    批量取消推荐
    case "batchnohot":
        $ids = $_GPC['ids'];
        $ids = explode(',',$ids);
        $ret=pdo_update("yzhyk_sun_storegroupgoods",['is_hot'=>0],array('id'=>$ids));
        if($ret){
            echo json_encode(array(
                'code'=>0,
            ));
...
//    批量新增
    case "batchadd":
        ...
        $sql = "";
        $sql .= "insert into ".tablename('yzhyk_sun_storegroupgoods')."(store_id,groupgoods_id,stock,uniacid) ";
        $sql .= "select t2.id as store_id,t1.id,t1.stock,$uniacid as uniacid ";
        $sql .= "from ".tablename('yzhyk_sun_groupgoods')." t1 ";
