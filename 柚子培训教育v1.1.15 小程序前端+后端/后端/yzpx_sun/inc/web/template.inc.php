...
$item=pdo_get('yzpx_sun_system',array('uniacid'=>$_W['uniacid']));
...
if($_GPC['op']=='order'){
    ...
    if($template_id){
        ...
        $system=pdo_get('yzpx_sun_system',array('uniacid'=>$_W['uniacid']),array('appid','appsecret','token_expires_in','token'));
...
//        }else{
            ...
            pdo_update('yzpx_sun_system',array('token'=>$info['access_token'],'token_expires_in'=>time()+7000));
            $token=$system['token'];
//        }
        $url="https://api.weixin.qq.com/cgi-bin/wxopen/template/add?access_token=".$token;
        $data['id']='AT0060';
        $data['keyword_id_list']=[1,4,7,8];
        ...
        if($add['errcode']==0&&$add['errmsg']=='ok'){
            $res=pdo_update('yzpx_sun_system',array('order_template_id'=>$add['template_id']),array('uniacid'=>$_W['uniacid']));
            if($res){
                message('课程预约成功模板消息设置成功',$this->createWebUrl('template',array()),'success');
            }else{
                message('设置失败','','error');
            }
        }else{
...
//        }
        ...
        if($add['errcode']==0&&$add['errmsg']=='ok'){
            $res=pdo_update('yzpx_sun_system',array('card_template_id'=>$add['template_id']),array('uniacid'=>$_W['uniacid']));
            if($res){
...
//        var_dump($add);exit;
        if($add['errcode']==0&&$add['errmsg']=='ok'){
            $res=pdo_update('yzpx_sun_system',array('sign_template_id'=>$add['template_id']),array('uniacid'=>$_W['uniacid']));
            if($res){
