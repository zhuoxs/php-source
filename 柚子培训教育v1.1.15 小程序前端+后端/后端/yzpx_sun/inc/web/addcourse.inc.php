...
//    $data['sid']=$_GPC['sid'];
    ...
    if($_GPC['id']==''){
        if ($_GPC['signchoose']==null){
            message('请选择分校跟老师','','error');
        }
        ...
        if($res){
            ...
            message('添加成功,请继续添加课时',$this->createWebUrl('lessonlist',array('couid'=>$cid)),'success');
        }else{
            message('添加失败','','error');
        }
    }else{
        ...
        if($res){
            message('修改成功',$this->createWebUrl('courselist',array()),'success');
        }else{
            message('修改失败','','error');
        }
    }
