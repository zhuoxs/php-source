...
$info=pdo_get('yzpx_sun_card',array('id'=>$_GPC['id']));
if($info){
...
//    var_dump($data);exit;
    if($_GPC['id']==''){
        $res=pdo_insert('yzpx_sun_card',$data);
        if($res){
            message('添加成功，请继续添加卡片才能进行抽奖',$this->createWebUrl('cardfont',array('cid'=>pdo_insertid())),'success');
        }else{
            message('添加失败','','error');
        }
    }else{

        $res=pdo_update('yzpx_sun_card',$data,array('id'=>$_GPC['id']));
        if($res){
