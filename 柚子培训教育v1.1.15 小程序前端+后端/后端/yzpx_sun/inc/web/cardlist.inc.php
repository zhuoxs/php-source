...
$total=pdo_fetchcolumn("select count(*) from " . tablename("yzpx_sun_card") .$where);
$pager = pagination($total, $page, $size);

$sql = 'SELECT * FROM '.tablename('yzpx_sun_card')."{$where} ORDER BY id DESC LIMIT ".(($page-1) * $size).','.$size;
...
//var_dump($info);exit;
if($_GPC['op']=='delete'){
    $res=pdo_update('yzpx_sun_card',array('status'=>0),array('id'=>$_GPC['id']));
    if($res){
