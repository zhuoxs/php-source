...
$sql="SELECT * FROM".tablename('yzpx_sun_active').$where;
...
if($_GPC['op']=='delete'){
    ...
    $res=pdo_delete('yzpx_sun_active',array('aid'=>$_GPC['aid'],'uniacid'=>$_W['uniacid']));
...
if($_GPC['op']=='tg'){
    $res=pdo_update('yzpx_sun_active',array('status'=>2),array('aid'=>$_GPC['aid'],'uniacid'=>$_W['uniacid']));
    if($res){
...
if($_GPC['op']=='jj'){
    $res=pdo_update('yzpx_sun_active',array('status'=>1),array('aid'=>$_GPC['aid'],'uniacid'=>$_W['uniacid']));
    if($res){
