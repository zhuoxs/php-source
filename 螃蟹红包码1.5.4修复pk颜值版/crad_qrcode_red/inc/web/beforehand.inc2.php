...
if ($op == "create_qrcode") {
    ...
    $count_qrcode = pdo_fetchcolumn("select COUNT(*) from " . tablename(TABLE_QRCODE) . " WHERE bid = :id OR sbid=:id", array(":id" => $id));
    if ($count_qrcode >= $set["qrcode_num"]) {
        echo json_encode(array("sta" => 2, "count" => $set["qrcode_num"]));
        exit;
    }
    $temp_min = 50;
    $qrcode_max_code = pdo_fetch("select code from " . tablename(TABLE_QRCODE) . " WHERE bid = :id OR sbid=:id ORDER BY code DESC", array(":id" => $id));
            ...
            if ($op == "get_band_beforehand") {
                ...
                if ($op == "get_band") {
                    ...
                    if ($op == "download_qrcode") {
                        ...
                        $start_code = intval($_GPC["start_code"]);
                        if ($start_code) {
                            $where_code .= " AND code>={$start_code}";
                        }
                        $end_code = intval($_GPC["end_code"]);
                        if ($end_code) {
                            $where_code .= " AND code<={$end_code}";
                        }
                        $count_qrcode = pdo_fetchcolumn("select COUNT(*) from " . tablename(TABLE_QRCODE) . " where bid = :id {$where_code}", array(":id" => $id));
                        ...
                        $psize = 20;
                        $list_qrcode = pdo_fetchall("SELECT id,bid,uuid,code,qrcode FROM " . tablename(TABLE_QRCODE) . " where bid = :id {$where_code} ORDER BY id DESC LIMIT " . ($pindex - 1) * $psize . "," . $psize, array(":id" => $id));
                        ...
                        if ($op == "download_qrcode_notuse") {
                            ...
                            $count_qrcode = pdo_fetchcolumn("select COUNT(*) from " . tablename(TABLE_QRCODE) . " WHERE bid = :id AND aid<1 AND sid<1 {$where_code}", array(":id" => $id));
                            if (empty($set) || $count_qrcode < 1) {
                            ...
                            $pindex = max(1, intval($_GPC["page"]));
                            $psize = 20;
                            $list_qrcode = pdo_fetchall("SELECT id,bid,uuid,code,qrcode FROM " . tablename(TABLE_QRCODE) . " WHERE bid = :id AND aid<1 AND sid<1 {$where_code} ORDER BY id DESC LIMIT " . ($pindex - 1) * $psize . "," . $psize, array(":id" => $id));
                            if (empty($list_qrcode)) {
                                echo json_encode(array("sta" => 2));
                                exit;
                            }
                            ...
                            if ($op == "download_qrcode_zip") {
                                ...
                                if ($op == "download_excel") {
                                    ...
                                    $list_excel = pdo_fetchall("SELECT id,uuid,bid,sbid,code,qrcode FROM " . tablename(TABLE_QRCODE) . " WHERE bid = :id {$where_code}", array(":id" => $id));
                                    foreach ($list_excel as $key => $value) {
                                        $arr[$i]["id"] = $value["id"];
                                        $arr[$i]["code"] = ($value["sbid"] ? $value["sbid"] : $value["bid"]) . "_" . $value["code"];
                                        $arr[$i]["url"] = $value["qrcode"] ? $value["qrcode"] : $this->domain_site() . "app/index.php?i=" . $uniacid . "&c=entry&m=crad_qrcode_red&do=index&uuid=" . $value["uuid"] . "&code=" . $value["code"];
                                        $i++;
                                    }
                                } else {
                                    if ($op == "download_txt") {
                                        ...
                                        header("Content-Disposition:attachment;filename=二维码列表-" . $set["name"] . "[" . $start_code . "-" . $end_code . "]" . ".txt");
                                        header("Expires:0");
                                        ...
                                        foreach ($list_excel as $key => $value) {
                                            ...
                                            $url_q = $value["qrcode"] ? $value["qrcode"] : $this->domain_site() . "app/index.php?i=" . $uniacid . "&c=entry&m=crad_qrcode_red&do=index&uuid=" . $value["uuid"] . "&code=" . $value["code"];
