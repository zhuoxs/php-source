...
class LotteryModel extends PluginModel
{
        ...
        if (!empty($lotterylist)) {
            if ($type == 1) {
                ...
                foreach ($lotterylist as $key => $value) {
                    ...
                    if (($value["task_data"]["pay_type"] == 0 || $data["paytype"] == $value["task_data"]["pay_type"]) && $value["task_data"]["pay_money"] <= $data["money"] && $join_info["money"] < $value["task_data"]["pay_money"]) {
                ...
                if ($type == 2) {
                    ...
                    foreach ($lotterylist as $key => $value) {
                        $value["task_data"] = unserialize($value["task_data"]);
                        if (($value["task_data"]["sign_day"] == 1 || $value["task_data"]["sign_day"] <= $data["day"] && $data["day"] % $value["task_data"]["sign_day"] == 0) && 0 < $value["task_data"]["sign_num"]) {
                    ...
                    if ($type == 3) {
                        ...
                        foreach ($lotterylist as $key => $value) {
                            $value["task_data"] = unserialize($value["task_data"]);
                            if (($data["taskid"] == $value["task_data"]["poster_id"] || $value["task_data"]["poster_id"] == 0) && 0 < $value["task_data"]["poster_num"]) {
                        ...
                        if ($type == 4) {
                            ...
                            foreach ($lotterylist as $key => $value) {
                                $value["task_data"] = unserialize($value["task_data"]);
                                if ($data["taskid"] == $value["task_data"]["poster_id"] && 0 < $value["task_data"]["poster_num"]) {
                                    for ($i = 1; $i <= $value["task_data"]["poster_num"]; $i++) {
                                        $join_data = array("uniacid" => $_W["uniacid"], "join_user" => $openid, "lottery_id" => $value["lottery_id"], "lottery_num" => 1, "lottery_tag" => "完成任务海报,赠" . $value["task_data"]["poster_num"] . "次", "addtime" => time());
                                        pdo_insert("ewei_shop_lottery_join", $join_data);
                                    }
    ...
    public function getLotteryList($openid, $param = array())
    {
        ...
        if (!empty($lottery_list)) {
            ...
            $remark = "\n<a href='" . $url . "'>点击去抽奖</a>";
            $text = "恭喜您获得抽奖机会 \n" . $remark;
            $message = array("first" => array("value" => "恭喜您获得抽奖机会", "color" => "#000000"), "keyword1" => array("title" => "业务类型", "value" => "游戏中奖通知", "color" => "#000000"), "keyword2" => array("title" => "业务内容", "value" => $lottery_list["title"], "color" => "#000000"), "keyword3" => array("title" => "处理结果", "value" => "获得抽奖机会", "color" => "#000000"), "keyword4" => array("title" => "操作时间", "value" => date("Y-m-d H:i:s", time()), "color" => "#000000"), "remark" => array("value" => "恭喜您获得抽奖机会！！", "color" => "#000000"));
