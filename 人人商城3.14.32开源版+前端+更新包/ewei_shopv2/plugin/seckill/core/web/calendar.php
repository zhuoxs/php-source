...
class Calendar_EweiShopV2Page extends SeckillWebPage
{
    ...
    public function dates()
    {
        ...
        $calendar = redis()->hGetAll((string) $redis_prefix . "calendar_" . $year . "_" . $month);
        if (empty($calendar)) {
            $calendar = array();
            for ($i = 1; $i <= $day; $i++) {
                if ($i < 10) {
                ...
                $calendar[date((string) $year . "-" . $month . "-" . $i)] = false;
            }
        } else {
            $result = array();
            for ($i = 1; $i <= $day; $i++) {
        ...
        $week = date("w", strtotime(date((string) $year . "-" . $month . "-1")));
        include $this->template();
    }
    public function set()
    {
        ...
        redis()->hSet((string) $redis_prefix . "calendar_" . $year . "_" . $month, date("Y-m-d", $time), $taskid);
    ...
    public function clear()
    {
        ...
        $redis_prefix = $this->model->get_prefix();
        redis()->delete((string) $redis_prefix . "calendar_" . $year . "_" . $month);
        show_json(1);
    }
    public function batch_set()
    {
        ...
        $redis_prefix = $this->model->get_prefix();
        redis()->hMset((string) $redis_prefix . "calendar_" . $year . "_" . $month, $arr);
    ...
    public function batch_delete()
    {
        ...
        for ($i = 1; $i <= $maxday; $i++) {
            ...
            if (in_array($week, $days) && is_array($calendar) && isset($calendar[$date])) {
                unset($calendar[$date]);
                $redis->hDel((string) $redis_prefix . "calendar_" . $year . "_" . $month, $date);
                $dates[] = $date;
            }
