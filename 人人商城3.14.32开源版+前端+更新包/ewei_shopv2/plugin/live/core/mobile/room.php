...
class Room_EweiShopV2Page extends PluginMobileLoginPage
{
            ...
            foreach ($room_coupon as $key => &$coupon) {
                ...
                if (!empty($roomcoupon)) {
                    if (intval($couponset["couponlimit"]) <= floatval($roomcoupon["limit"]) && 0 < intval($couponset["couponlimit"])) {
                        ...
                        if (0 < intval($couponset["couponlimit"]) && intval($couponset["couponlimit"]) <= intval($roomcoupon["limit"])) {
                            $coupon["iscoupon"] = false;
                            $coupon["couponmessage"] = "您已领取过了";
                        }
                    }
    ...
    public function roomcoupon()
    {
        ...
        if ($open_redis) {
            $redis_key = (string) $_W["setting"]["site"]["key"] . "_" . $_W["account"]["key"] . "_" . $uniacid . "_" . $roomid . "_roomcoupon_" . $openid;
            $redis = redis();
            if (!is_error($redis)) {
    ...
    public function roompacket()
    {
        ...
        $open_redis = function_exists("redis") && !is_error(redis());
        if ($open_redis) {
            $redis_key = (string) $_W["setting"]["site"]["key"] . "_" . $_W["account"]["key"] . "_" . $uniacid . "_" . $roomid . "_roompacket_" . $openid;
            $redis = redis();
            if (!is_error($redis)) {
                if ($redis->setnx($redis_key, time())) {
                    $redis->expireAt($redis_key, time() + 3);
