        ...
        if ($_W["ispost"]) {
            ...
            if (in_array(2, $select)) {
                ...
                if ($balance_type == 1) {
                    ...
                    if ($balance_type == 2 && (intval($balance_left) <= 0 || intval($balance_right) <= 0)) {
                        show_json(0, "余额面值必须大于0");
                    }
                }
            }
            if (in_array(3, $select)) {
                ...
                if ($red_type == 1) {
                    ...
                    if ($red_type == 2 && (intval($red_left) < 0.3 || intval($red_right) < 0.3)) {
                        show_json(0, "红包面值不能小于0.3元");
                    }
                }
            ...
            if (!empty($id)) {
                ...
                if ($goods["goods"] != false) {
                    foreach ($goods["goods"] as $k => $v) {
                        $all[$k] = pdo_fetchall("SELECT * FROM " . tablename("ewei_shop_goods") . " WHERE uniacid = :uniacid AND deleted = 0 \r        AND hasoption = 0 AND id = :id", array(":id" => $v, ":uniacid" => $_W["uniacid"]));
                    }
                }
