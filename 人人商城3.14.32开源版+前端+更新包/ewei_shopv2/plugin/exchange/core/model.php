...
class ExchangeModel extends PluginModel
{
        ...
        if (!empty($articles)) {
            ...
            if (is_error($resp)) {
                ...
                if (is_error($ret)) {
                    ...
                    $msg = array("first" => array("value" => "亲爱的" . $member["nickname"] . "恭喜您获得优惠券", "color" => "#ff0000"), "keyword1" => array("title" => "业务类型", "value" => "优惠券通知", "color" => "#000000"), "keyword2" => array("title" => "处理进度", "value" => $coupon["resptitle"], "color" => "#000000"), "keyword3" => array("title" => "处理内容", "value" => $coupon["respdesc"], "color" => "#4b9528"), "remark" => array("value" => "点击查看详情", "color" => "#000000"));
                    if (!empty($templateid)) {
    ...
    public function sendExchangeMessage($openid, $num, $type = 0)
    {
        ...
        if ($type == 0) {
            ...
            $text = "恭喜您" . $credittext . "兑换成功！\n" . $credittext . "兑换：" . $num . "积分\n兑换时间：" . $time . "\n充值方式：积分兑换\n" . $credittext . "余额：" . (int) $member["credit1"] . (string) $credittext . " \n" . $remark;
