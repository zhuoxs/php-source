...
if (!class_exists("AbonusModel")) {
    class AbonusModel extends PluginModel
    {
        ...
        public function sendMessage($openid = "", $data = array(), $message_type = "")
        {
            ...
            if ($message_type == TM_ABONUS_PAY && empty($usernotice["abonus_pay"])) {
                if ($tm["is_advanced"]) {
                    ...
                    $message = array("first" => array("value" => "亲爱的" . $member["nickname"] . "，您的" . $set["texts"]["aagent"] . "的" . $set["texts"]["bonus"] . "已打款", "color" => "#ff0000"), "keyword1" => array("title" => "业务类型", "value" => "会员通知", "color" => "#000000"), "keyword2" => array("title" => "业务内容", "value" => "您的区域代理分红已发放", "color" => "#000000"), "keyword3" => array("title" => "处理结果", "value" => "区域代理分红发放通知", "color" => "#000000"), "keyword4" => array("title" => "操作时间", "value" => date("Y-m-d H:i:s", time()), "color" => "#000000"), "remark" => array("value" => "\n感谢您的支持", "color" => "#000000"));
                    $toopenid = $openid;
                } else {
                    $message = "";
                    $paytitle = "";
                    ...
                    $msg = array("keyword1" => array("value" => "会员通知", "color" => "#73a68d"), "keyword2" => array("value" => $paytitle, "color" => "#73a68d"), "keyword3" => array("value" => $message, "color" => "#73a68d"));
                ...
                if ($message_type == TM_ABONUS_UPGRADE && empty($usernotice["abonus_upgrade"])) {
                    if ($tm["is_advanced"]) {
                        ...
                        $message = array("first" => array("value" => "亲爱的" . $member["nickname"] . "，恭喜您升级为" . $data["newlevel"]["levelname"], "color" => "#ff0000"), "keyword1" => array("title" => "业务类型", "value" => "会员通知", "color" => "#000000"), "keyword2" => array("title" => "业务内容", "value" => "您的区域代理等级从" . $data["oldlevel"]["levelname"] . "升级为" . $data["newlevel"]["levelname"] . "特此通知！", "color" => "#000000"), "keyword3" => array("title" => "处理结果", "value" => "区域等级升级通知", "color" => "#000000"), "keyword4" => array("title" => "操作时间", "value" => date("Y-m-d H:i:s", time()), "color" => "#000000"), "remark" => array("value" => "\n感谢您的支持", "color" => "#000000"));
                        $toopenid = $openid;
                    } else {
                        $message = $tm["upgrade"];
                        $upgradetitle = "";
                        ...
                        $msg = array("keyword1" => array("value" => "会员通知", "color" => "#73a68d"), "keyword2" => array("value" => $upgradetitle, "color" => "#73a68d"), "keyword3" => array("value" => $message, "color" => "#73a68d"));
                        return $this->sendNotice($openid, $tm, $advanced, $data, $member, $msg);
                    }
                } else {
                    if ($message_type == TM_ABONUS_BECOME && empty($usernotice["abonus_become"])) {
                        if ($tm["is_advanced"]) {
                            ...
                            $message = array("first" => array("value" => "亲爱的" . $member["nickname"] . "，恭喜您成为" . $aagenttype, "color" => "#ff0000"), "keyword1" => array("title" => "业务类型", "value" => "会员通知", "color" => "#000000"), "keyword2" => array("title" => "业务内容", "value" => "您已成为" . $aagenttype, "color" => "#000000"), "keyword3" => array("title" => "处理结果", "value" => "成为区域代理通知", "color" => "#000000"), "keyword4" => array("title" => "操作时间", "value" => date("Y-m-d H:i:s", time()), "color" => "#000000"), "remark" => array("value" => "\n感谢您的支持", "color" => "#000000"));
                            $toopenid = $openid;
                            ...
                            $msg = array("keyword1" => array("value" => "会员通知", "color" => "#73a68d"), "keyword2" => array("value" => !empty($tm["becometitle"]) ? $tm["becometitle"] : "成为区域代理通知", "color" => "#73a68d"), "keyword3" => array("value" => $message, "color" => "#73a68d"));
        ...
        public function upgradeLevelByBonus($openid)
        {
            ...
            if (!empty($oldlevel["id"]) && $newlevel["bonusmoney"] < $oldlevel["bonusmoney"]) {
                return NULL;
            }
            pdo_update("ewei_shop_member", array("aagentlevel" => $newlevel["id"]), array("id" => $m["id"]));
            $member = m("member")->getMember($openid);
        ...
        public function getBonusData($year = 0, $month = 0, $week = 0, $openid = "")
        {
            ...
            foreach ($orders as $o) {
                $ordermoney += $o["price"];
                foreach ($aagents as &$a) {
                    if (empty($set["selfbuy"]) && $a["openid"] == $o["openid"]) {
                        continue;
                    }
                    !isset($a["bonusmoney1"]) and $a["bonusmoney1"] = 0;
                    !isset($a["bonusmoney2"]) and $a["bonusmoney2"] = 0;
                    !isset($a["bonusmoney3"]) and $a["bonusmoney3"] = 0;
                    ...
                    if ($a["aagenttype"] == 1) {
                        ...
                        if ($a["aagenttype"] == 2) {
                            ...
                            if ($a["aagenttype"] == 3 && in_array($address["province"] . $address["city"] . $address["area"], $a["aagentareas"])) {
                                $bonusmoney3 = round($o["price"] * $a["bonus3"] / 100, 2);
                            }
                        }
                    }
