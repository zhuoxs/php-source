...
class GoodscircleModel extends PluginModel
{
    ...
    public function updateOrder($openid = "", $order_id = 0)
    {
        ...
        foreach ($order["order_goods"] as $goods) {
            if (!empty($goods["expresssn"])) {
                ...
                $express_package_info_list[] = array("express_company_id" => $express_company["express_company_id"], "express_company_name" => $express_company["express_company_name"], "express_code" => $goods["expresssn"], "ship_time" => $goods["sendtime"], "express_page" => array("path" => "/pages/order/detail/index?id=" . $order["id"]), "express_goods_info_list" => array($goods_info_list));
            } else {
                $goods_info_list[] = array("item_code" => $goods["goodsid"], "sku_id" => $goods["optionid"]);
            }
        }
        if (!$single_express) {
            ...
            $express_package_info_list[] = array("express_company_id" => $express_company["express_company_id"], "express_company_name" => $express_company["express_company_name"], "express_code" => $order["expresssn"], "ship_time" => $order["sendtime"], "express_page" => array("path" => "/pages/order/detail/index?id=" . $order["id"]), "express_goods_info_list" => $goods_info_list);
        }
        ...
        foreach ($all_order as $order) {
            ...
            foreach ($order_goods as $goods) {
                if ($order["id"] == $goods["orderid"]) {
                    $goods_item = array("item_code" => $goods["goodsid"], "sku_id" => $goods["optionid"], "amount" => $goods["total"], "total_fee" => $goods["price"] * 100, "thumb_url" => tomedia($goods["thumb"]), "title" => $goods["title"], "desc" => empty($goods["description"]) ? $goods["subtitle"] : $goods["description"], "unit_price" => $goods["price"] * 100, "original_price" => $goods["productprice"] * 100, "category_list" => $this->getGoodsCate($goods["cates"]), "item_detail_page" => array("path" => "/pages/goods/detail/index?id=" . $goods["goodsid"]));
                    if ($goods["optionid"]) {
                        $goods_item["stock_attr_info"][] = array("attr_name" => array("name" => "规格"), "attr_value" => array("name" => $goods["optionname"]));
                    }
            ...
            $brand_info = array("phone" => empty($wxapp_set["phone"]) ? "10086" : $wxapp_set["phone"], "contact_detail_page" => array("path" => "/pages/index/index"));
            $payment_method = 1;
            if ($order["paytype"] != 21) {
                $payment_method = 2;
            }
    ...
    private function getCartGoods($openid, $cart_id, $goods_id)
    {
        ...
        $product = array("user_open_id" => ltrim($openid, "sns_wa_"), "sku_product_list" => array($data));
    ...
    {
        $goods_circle_express = array("ems" => array("express_company_id" => "2000", "express_company_name" => "EMS"), "yuantong" => array("express_company_id" => "2001", "express_company_name" => "圆通"), "dhl" => array("express_company_id" => "2002", "express_company_name" => "DHL"), "zhongtong" => array("express_company_id" => "2004", "express_company_name" => "中通"), "yunda" => array("express_company_id" => "2005", "express_company_name" => "韵达"), "shl" => array("express_company_id" => "2006", "express_company_name" => "畅灵"), "huitongkuaidi" => array("express_company_id" => "2008", "express_company_name" => "百世汇通"), "debangwuliu" => array("express_company_id" => "2009", "express_company_name" => "德邦"), "shentong" => array("express_company_id" => "2010", "express_company_name" => "申通"), "shunfeng" => array("express_company_id" => "2011", "express_company_name" => "顺丰速运"), "shunxing" => array("express_company_id" => "2012", "express_company_name" => "顺兴"), "rufengda" => array("express_company_id" => "2014", "express_company_name" => "如风达"), "youshuwuliu" => array("express_company_id" => "2015", "express_company_name" => "优速"));
