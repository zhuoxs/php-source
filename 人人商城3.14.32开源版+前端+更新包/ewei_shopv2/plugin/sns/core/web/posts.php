...
class Posts_EweiShopV2Page extends PluginWebPage
{
    ...
    public function post()
    {
        ...
        $set = $this->set;
        if (isset($set["managers"]) && !empty($set["managers"])) {
            $openids = array();
    ...
    public function top()
    {
        ...
        foreach ($items as $item) {
            $content = $this->model->replaceContent($item["content"]);
            if ($top) {
                plog("sns.posts.edit", (string) $msg . "置顶 ID: " . $item["id"] . " 标题: " . $item["title"] . " 内容: " . $content);
    ...
    public function best()
    {
        ...
        foreach ($items as $item) {
            $content = $this->model->replaceContent($item["content"]);
            if ($best) {
                plog("sns.posts.edit", (string) $msg . "精华 ID: " . $item["id"] . " 标题: " . $item["title"] . " 内容: " . $content);
                pdo_update("ewei_shop_sns_post", array($field => 1), array("id" => $item["id"]));
