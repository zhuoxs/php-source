    ...
    class GlobonusModel extends PluginModel
    {
        ...
        public function sendMessage($openid = "", $data = array(), $message_type = "")
        {
            ...
            if ($message_type == TM_GLOBONUS_PAY && empty($usernotice["globonus_pay"])) {
                if ($tm["is_advanced"]) {
                    ...
                    $message = array("first" => array("value" => "亲爱的" . $member["nickname"] . "，您的" . $set["texts"]["center"] . "的" . $set["texts"]["bonus"] . "已打款", "color" => "#ff0000"), "keyword1" => array("title" => "业务类型", "value" => "会员通知", "color" => "#000000"), "keyword2" => array("title" => "业务内容", "value" => "您的分红打款成功", "color" => "#000000"), "keyword3" => array("title" => "处理结果", "value" => "分红发放通知", "color" => "#000000"), "keyword4" => array("title" => "操作时间", "value" => date("Y-m-d H:i:s", time()), "color" => "#000000"), "remark" => array("value" => "\n感谢您的支持", "color" => "#000000"));
                    $toopenid = $openid;
                ...
                if ($message_type == TM_GLOBONUS_UPGRADE && empty($usernotice["globonus_upgrade"])) {
                    if ($tm["is_advanced"]) {
                        ...
                        $message = array("first" => array("value" => "亲爱的" . $member["nickname"] . "，恭喜您成为" . $data["newlevel"]["levelname"] . $set["texts"]["partner"], "color" => "#ff0000"), "keyword1" => array("title" => "业务类型", "value" => "会员通知", "color" => "#000000"), "keyword2" => array("title" => "业务内容", "value" => "您的股东等级从" . $data["oldlevel"]["levelname"] . "升级到" . $data["newlevel"]["levelname"] . ",特此通知！", "color" => "#000000"), "keyword3" => array("title" => "处理结果", "value" => "股东等级升级通知", "color" => "#000000"), "keyword4" => array("title" => "操作时间", "value" => date("Y-m-d H:i:s", time()), "color" => "#000000"), "remark" => array("value" => "\n感谢您的支持", "color" => "#000000"));
                        $toopenid = $openid;
                        $datas[] = array("name" => "昵称", "value" => $member["nickname"]);
                        $datas[] = array("name" => "时间", "value" => $time);
                        ...
                        $msg = array("keyword1" => array("value" => "会员通知", "color" => "#73a68d"), "keyword2" => array("value" => !empty($tm["upgradetitle"]) ? $tm["upgradetitle"] : "股东等级升级通知", "color" => "#73a68d"), "keyword3" => array("value" => $message, "color" => "#73a68d"));
                    ...
                    if ($message_type == TM_GLOBONUS_BECOME && empty($usernotice["globonus_become"])) {
                        if ($tm["is_advanced"]) {
                            ...
                            $message = array("first" => array("value" => "亲爱的" . $member["nickname"] . "，恭喜您成为" . $set["texts"]["center"] . "的股东", "color" => "#ff0000"), "keyword1" => array("title" => "业务类型", "value" => "会员通知", "color" => "#000000"), "keyword2" => array("title" => "业务内容", "value" => "恭喜您成为股东", "color" => "#000000"), "keyword3" => array("title" => "处理结果", "value" => "成为股东通知", "color" => "#000000"), "keyword4" => array("title" => "操作时间", "value" => date("Y-m-d H:i:s", time()), "color" => "#000000"), "remark" => array("value" => "\n感谢您的支持", "color" => "#000000"));
                            $toopenid = $openid;
                            $datas[] = array("name" => "昵称", "value" => $member["nickname"]);
                            $datas[] = array("name" => "时间", "value" => $time);
                        } else {
                            ...
                            $msg = array("keyword1" => array("value" => "会员通知", "color" => "#73a68d"), "keyword2" => array("value" => !empty($tm["becometitle"]) ? $tm["becometitle"] : "成为股东通知", "color" => "#73a68d"), "keyword3" => array("value" => $message, "color" => "#73a68d"));
        ...
        public function getBonusData($year = 0, $month = 0, $week = 0, $openid = "")
        {
            ...
            foreach ($orders as $o) {
                ...
                foreach ($partners as &$p) {
                    if (empty($set["selfbuy"]) && $p["openid"] == $o["openid"]) {
                        continue;
                    }
                    $price = $o["price"] * $set["bonusrate"] / 100;
                    !isset($p["bonusmoney"]) and $p["bonusmoney"] = 0;
