...
class Task_EweiShopV2Page extends PluginWebPage
{
    ...
    /**     * 需要验证是否非空的字段以及其回复     * @var array     */
    private $categoryMessage = array("签到" => array("feed" => "请填写签到所获取的饲料数"), "任务中心" => array("core_feed" => "请填写完成任务所获取的饲料数", "core" => "请选择任务"), "购买商品" => array("goods_feed" => "请填写购买一个商品所获取的饲料数", "goods_id" => "请选择商品"), "商城下单" => array("order_feed" => "请填写每单可获取的饲料数", "money_feed" => "请填写多少元可获得一克饲料"), "会员领取" => array("member_level_feed" => "请填写会员一次可领取的饲料数", "member_level" => "请选择会员等级"));
    /**     * 需要验证图片文件是否存在     * @var array     */
    ...
    private function getTaskName($data)
    {
        ...
        foreach ($data as $key => $value) {
            if ($value["category"] === "任务中心" && $value["core"]) {
                $idArr[] = $value["core"];
            }
        }
