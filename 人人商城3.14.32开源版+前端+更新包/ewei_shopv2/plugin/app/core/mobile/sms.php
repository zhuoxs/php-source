...
class Sms_EweiShopV2Page extends AppMobilePage
{
	...
	protected function loginSMS($temp)
	{
		...
		if (empty($mobile)) {
			echo  app_error(AppError::$ParamsError, '手机号不能为空');
          return;
		}
		...
		if (!empty($wapset['smsimgcode'])) {
			...
			if ($imgcodehash != trim($verifyCode)) {
				echo  app_error(AppError::$ParamsError, '图形验证码错误');
              return;
			}
		}
		...
		if (empty($sms_id)) {
			echo  app_error(AppError::$SMSTplidNull);
          return;
		}
		...
		if ($time < 60) {
			echo  app_error(AppError::$SMSRateError);
          return;
		}
		...
		echo  app_error(AppError::$SystemError, $ret['message']);
      return;
	}
