...
class Index_EweiShopV2Page extends MerchWebPage
{
    ...
    public function updatepassword()
    {
        ...
        if ($_W["ispost"]) {
            ...
            strlen($newpassword) < 6 and show_json(0, "密码至少是6位!");
            $newpassword != $surenewpassword and show_json(0, "两次输入密码不一致!");
            ...
            $item["pwd"] != md5($password . $item["salt"]) and show_json(0, "原密码输入不正确!");
    ...
    public function searchlist()
    {
        ...
        foreach ($menu as $index => $item) {
            if ((strexists($item["title"], $keyword) || strexists($item["desc"], $keyword) || strexists($item["keywords"], $keyword) || strexists($item["topsubtitle"], $keyword)) && m("system")->cv($item["route"])) {
                $return_arr[] = $item;
            }
        }
        show_json(1, array("menu" => $return_arr));
        ...
        if (!empty($keyword)) {
            ...
            if (!empty($submenu)) {
                foreach ($submenu as $index => $submenu_item) {
                    ...
                    if ((strexists($submenu_item["title"], $keyword) || strexists($submenu_item["desc"], $keyword) || strexists($submenu_item["keywords"], $keyword) || strexists($submenu_item["topsubtitle"], $keyword)) && m("system")->cv($submenu_item["route"])) {
