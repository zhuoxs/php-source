            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (typeof $data == "undefined") {
                        $('.commonList > ul').empty();
                        $('#pager').empty();
                        var jqueryDom = $(`<li><div class="no"><span>暂无评价</span></div></li>`);
                        $('.commonList >  ul').append(jqueryDom);
...
<!--                                    </div>-->
    ...
    shopCartEvent: function () {
        ...
        // 添加购物车事件
        $('#addCartBtn_J').on('click', function () {

            if (window.GLOBAL.userinfo.length === 0) {
                layer.msg("请登陆后再进行操作");
                return false;
            }
            ...
            if ($('[data-attr="spec"]').length > 0) {
                console.log(window.goodsOptionId)
    ...
    specsGoodsClientEvent: function () {
        ...
        $('.size ul li a').click(function (e) {
            var selectedOption = [];
            var $this = $(this);
            var optionid = parseInt($this.data('optionid'));
