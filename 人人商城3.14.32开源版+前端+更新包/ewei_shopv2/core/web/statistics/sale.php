    ...
    public function main()
    {
        ...
        if (!empty($year) && !empty($month) && !empty($day)) {
            for ($hour = 0; $hour < 24; $hour++) {
                $nexthour = $hour + 1;
                $dr = array("data" => $hour . "点 - " . $nexthour . "点", "count" => pdo_fetchcolumn("SELECT ifnull(" . $countfield . ",0) as cnt FROM " . tablename("ewei_shop_order") . " WHERE uniacid=:uniacid and status>=1 and isparent=0 and createtime >=:starttime and createtime <=:endtime", array(":uniacid" => $_W["uniacid"], ":starttime" => strtotime((string) $year . "-" . $month . "-" . $day . " " . $hour . ":00:00"), ":endtime" => strtotime((string) $year . "-" . $month . "-" . $day . " " . $hour . ":59:59"))));
                $totalcount += $dr["count"];
