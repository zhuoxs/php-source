...
function auth_user($id, $domain) {
    $ret = cloud_upgrade('wsh-clientip', array('siteid' => $id, 'domain' => $domain));
    return $ret;
}
function auth_siteid($domain) {
    $ret = cloud_upgrade('wsh-clientid', array('domain' => $domain));
    return $ret;
}
function auth_checkauth($data) {
    $ret = cloud_upgrade('wsh-listsite', $data);
    return $ret;
}
function auth_grant($data) {
    $ret = cloud_upgrade('wsh-siteup', $data);
    return $ret;
}
function auth_check($auth, $version, $release) {
    $ret = cloud_upgrade('wsh-vercheck', array('version' => $version, 'release' => $release, 'code' => $auth['code'], 'siteid' => $auth['siteid'], 'domain' => $auth['domain'], 'ip' => $auth['ip']));
    return $ret;
}
function auth_download($auth, $path) {
    $ret = cloud_upgrade('wsh-downfile', array('path' => $path, 'code' => $auth['code'], 'siteid' => $auth['siteid'], 'domain' => $auth['domain'], 'ip' => $auth['ip']));
    return $ret;
}
function auth_uplog($auth, $version, $release) {
    $ret = cloud_upgrade('wsh-logdisp', array('version' => $version, 'release' => $release, 'code' => $auth['code'], 'siteid' => $auth['siteid'], 'domain' => $auth['domain'], 'ip' => $auth['ip']));
    return $ret;
}
...
function cloud_upgrade($type, $post_data = array(), $timeout = 60) {
    ...
    $resp = ihttp_request("http://shop.wshwx.com/api/api.php", $post_data, $extra, $ip, $timeout);
