...
class Sale_EweiShopV2ComModel extends ComModel
{
    ...
    public function getCredit1($openid, $price = 0, $paytype = 1, $type = 1, $refund = 0, $desc = "")
    {
        ...
        if (is_array($enoughs)) {
            foreach ($enoughs as $e) {
                if (floatval($e["enough" . $type . "_1"]) <= $price && $price <= floatval($e["enough" . $type . "_2"]) && 0 < floatval($e["give" . $type])) {
    ...
    {
        global $_W;
        $res = array("万水千山总是情,这单帮我一定行", array("无名侠", "支持一下,么么哒!"), "self_peerpay" => 0, "peerpay_price" => 0, "peerpay_privilege" => 0);
        $data = m("common")->getPluginset("sale");
        $data = $data["peerpay"];
        if (empty($data["open"])) {
            return false;
