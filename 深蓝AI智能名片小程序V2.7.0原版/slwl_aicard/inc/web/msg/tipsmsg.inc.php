...
$params = array(':uniacid' => $_W['uniacid'], ':setting_name'=>'msg_settings');
...
if ($_W['ispost']) {
	...
	if (($type == 'Alidayu') && ($mobile) && (intval($_GPC['test']) > 0)){
		$res = sendsms($mobile, array('time'=>$curr_date, 'phone'=>$mobile), $sms_id, array('appkey'=>$appkey, 'secret'=>$secret, 'sign'=>$sign));
	} else if (($type == 'Alidysmslite') && (intval($_GPC['test']) > 0)) {

		include_once MODULE_ROOT . '/lib/Alidysmslite/smsfunctions.php';
		$res = Aliyun\DySDKLite\Sms\send_sys_msg_admin(1);
	}
	sl_ajax(0, '更新短信设置成功！');
}
