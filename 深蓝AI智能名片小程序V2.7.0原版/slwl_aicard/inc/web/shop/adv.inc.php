...
if ($operation == 'display') {
	...
	$psize = 10;
	$sql = "SELECT * FROM " . tablename('slwl_aicard_shop_adv'). ' WHERE 1 '
		. $condition . " ORDER BY displayorder DESC, id DESC LIMIT " . ($pindex - 1) * $psize .',' .$psize;

	$list = pdo_fetchall($sql, $params);
	$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('slwl_aicard_shop_adv')
	...
	$adv = pdo_fetch("select * from " . tablename('slwl_aicard_shop_adv')
