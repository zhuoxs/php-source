...
if ($operation == 'display') {
	...
	$psize = 10;
	$sql = "SELECT * FROM " . tablename('slwl_aicard_shop_brand'). ' WHERE 1 '
		. $condition . " ORDER BY displayorder DESC, id DESC LIMIT " . ($pindex - 1) * $psize .',' .$psize;

	$list = pdo_fetchall($sql, $params);
	$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('slwl_aicard_shop_brand')
	...
	if ($_W['ispost']) {
		...
		if ($id) {
			pdo_update('slwl_aicard_shop_brand', $data, array('id' => $id));
		} else {
			$data['addtime'] = $_W['slwl']['datetime']['now'];
			pdo_insert('slwl_aicard_shop_brand', $data);
			$id = pdo_insertid();
		}
		sl_ajax(0, '保存成功！');
	}
	...
	$one = pdo_fetch('SELECT * FROM ' . tablename('slwl_aicard_shop_brand')
		. ' WHERE 1 ' . $condition, $params);
	...
	$condition = " AND uniacid=:uniacid AND id=:id ";
	$params = array(':uniacid' => $_W['uniacid'], ':id' => $id);
	$one = pdo_fetch('SELECT id FROM ' . tablename('slwl_aicard_shop_brand')
		. ' WHERE 1 ' . $condition, $params);
	if (empty($one)) {
		sl_ajax(1, '抱歉，不存在或是已经被删除！');
	}
	pdo_delete('slwl_aicard_shop_brand', array('id' => $id));
	sl_ajax(0, '删除成功！');


} else if ($operation == 'money_show') {
	$brandid = intval($_GPC['brandid']);
	$condition = ' AND weid=:weid AND brandid=:brandid ';
	$params = array(':weid' => $_W['uniacid'], ':brandid' => $brandid);
	$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('slwl_aicard_shop_order_goods')
		. ' WHERE 1 ' . $condition, $params);
	...
	$condition = " AND weid=:weid AND deleted='0' AND brandid=:brandid ";
	$params = array(':weid' => $_W['uniacid'], ':brandid'=>$brandid);
	$pindex = max(1, intval($_GPC['page']));
	$psize = 10;
	...
	$list = pdo_fetchall($sql, $params);
	$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('slwl_aicard_shop_goods')
		. ' WHERE 1 ' . $condition, $params);
	$pager = pagination($total, $pindex, $psize);

} else if ($operation == 'goods_post') {
	$id = intval($_GPC['id']);
	$brandid = intval($_GPC['brandid']);

	$sql = 'SELECT * FROM ' . tablename('slwl_aicard_shop_category')
		. ' WHERE `weid` = :weid ORDER BY `parentid`, `displayorder` DESC';
	...
	if ($_W['ispost']) {
		...
		$check_one_spec = pdo_fetch("SELECT * FROM " . tablename('slwl_aicard_shop_spec')
			. ' WHERE 1 ' . $check_condition_spec, $check_params_spec);
	...
	$condition = " AND weid=:weid AND id=:id ";
	$params = array(':weid' => $_W['uniacid'], ':id' => $id);
	$one = pdo_fetch('SELECT * FROM ' . tablename('slwl_aicard_shop_goods')
		. ' WHERE 1 ' . $condition, $params);
	$piclist = array();
	if ($one) {
	...
	// 处理，多规格
	$one_spec = pdo_fetch("SELECT * FROM " . tablename('slwl_aicard_shop_spec')
		. " where goodsid=:goodsid and weid=:weid", array(":goodsid" => $id, ":weid" => $_W['uniacid']));
	if (!(empty($one_spec['content']))) {
		$spec_items = json_decode($one_spec['content'], true);
	}

	// 处理，规格项目表
	$one_option = pdo_fetchall("SELECT * FROM " . tablename('slwl_aicard_shop_goods_option')
		. " where goodsid=:goodsid ORDER BY id ASC ", array(":goodsid" => $id));
...
} else if ($operation == 'goods_delete') {
	$id = intval($_GPC['id']);
	$one = pdo_fetch("SELECT id  FROM " . tablename('slwl_aicard_shop_goods') . " WHERE id = '{$id}' AND weid=" . $_W['uniacid'] . "");

	if (empty($one)) {
		sl_ajax(1, '抱歉，不存在或是已经被删除！');
	}
	pdo_delete('slwl_aicard_shop_goods', array('id' => $id));
	pdo_delete('slwl_aicard_shop_goods_option', array('goodsid' => $id));
