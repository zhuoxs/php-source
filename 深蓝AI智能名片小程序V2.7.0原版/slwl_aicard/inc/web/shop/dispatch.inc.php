...
if ($operation == 'display') {
	...
	$psize = 10;
	$sql = "SELECT * FROM " . tablename('slwl_aicard_shop_dispatch'). ' WHERE 1 '
		. $condition . " ORDER BY displayorder DESC, id DESC LIMIT " . ($pindex - 1) * $psize .',' .$psize;

	$list = pdo_fetchall($sql, $params);
	$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('slwl_aicard_shop_dispatch')
	...
	$one = pdo_fetch("select * from " . tablename('slwl_aicard_shop_dispatch')
		. " where id=:id and weid=:weid", array(":id" => $id, ":weid" => $_W['uniacid']));
	$express = pdo_fetchall("select * from " . tablename('slwl_aicard_shop_express')
		. " WHERE weid = '{$_W['uniacid']}' ORDER BY displayorder DESC");


} else if ($operation == 'delete') {
	$id = intval($_GPC['id']);
	$one = pdo_fetch("SELECT id  FROM " . tablename('slwl_aicard_shop_dispatch')
