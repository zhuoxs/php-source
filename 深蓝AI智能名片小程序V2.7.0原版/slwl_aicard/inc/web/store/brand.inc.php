...
if ($operation == 'display') {
	...
	$psize = 10;
	$sql = "SELECT * FROM " . tablename('slwl_aicard_store_brand'). ' WHERE 1 '
	...
	$list = pdo_fetchall($sql, $params);
	$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('slwl_aicard_store_goods')
	...
	$sql = 'SELECT * FROM ' . tablename('slwl_aicard_store_category')
		. ' WHERE `uniacid` = :uniacid ORDER BY `parentid`, `displayorder` DESC';
	...
	$condition = " AND uniacid=:uniacid AND id=:id ";
	$params = array(':uniacid' => $_W['uniacid'], ':id' => $id);
	$one = pdo_fetch('SELECT * FROM ' . tablename('slwl_aicard_store_goods')
		. ' WHERE 1 ' . $condition, $params);
	$piclist = array();
	if ($one) {
	...
	// 处理，多规格
	$one_spec = pdo_fetch("SELECT * FROM " . tablename('slwl_aicard_store_spec')
		. " WHERE goodsid=:goodsid and uniacid=:uniacid",
	...
	// 处理，规格项目表
	$one_option = pdo_fetchall("SELECT * FROM " . tablename('slwl_aicard_store_goods_option')
	...
	$id = intval($_GPC['id']);
	$one = pdo_fetch("SELECT id  FROM " . tablename('slwl_aicard_store_goods')
