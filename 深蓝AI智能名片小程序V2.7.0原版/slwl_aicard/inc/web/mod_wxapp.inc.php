...
if ($operation == 'display') {
    ...
    if ($_W['ispost']) {
        ...
        if ($id) {
            pdo_update('slwl_aicard_mod_wxapp', $data, array('id' => $id));
        } else {
            $data['addtime'] = $_W['slwl']['datetime']['now'];
            pdo_insert('slwl_aicard_mod_wxapp', $data);
            $id = pdo_insertid();
        }
        iajax(0, '保存成功！');
        exit;
    ...
    $one = pdo_fetch('SELECT * FROM ' . tablename('slwl_aicard_mod_wxapp') . ' WHERE 1 ' . $condition, $params);
    ...
    $rst = @pdo_delete('slwl_aicard_mod_wxapp', $where);
