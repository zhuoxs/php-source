...
if ($operation == 'display') {
	...
	$condition = " AND uniacid=:uniacid AND `enabled` <> '2'  ";
	...
	if ($list) {
		foreach ($list as $k => $v) {
			...
			$list[$k]['qrcode_url'] = $v['qrcode'] ? tomedia($v['qrcode']) : MODULE_URL.'template/public/image/nopic.png';
		...
		if ($list_qy['errcode'] == '0') {
			...
			foreach ($list_qy['data'] as $k => $v) {
				...
				if ($card_count > $card_num) {
					sl_ajax(1, '已超出最大名片数，可以在名片其他设置->名片设置中配置');
					break;
				}
				if ($list_alluser) {
					$ins = '0';
			...
			sl_ajax(1, $list_qy['errmsg'].'-'.$list_qy['data']);
		}
	}

} else if ($operation == 'work_handover') {
	$a_id = intval($_GPC['aid']); // 交接人
	$b_id = intval($_GPC['bid']); // 接收人

	if ($_W['ispost']) {
		$join_timestamp = $_W['slwl']['datetime']['timestamp']; // 交接时间戳
		...
		// 交接人
		$where_a = ['uniacid'=>$_W['uniacid'], 'id'=>$a_id];
		$one_a = pdo_get(sl_table_name('card'), $where_a);

		// 接收人
		$where_b = ['uniacid'=>$_W['uniacid'], 'id'=>$b_id];
		$one_b = pdo_get(sl_table_name('card'), $where_b);
		...
		if ($one_a) {
			...
			if ($one_a['join_id']) {
				$where_join = [
					'uniacid' => $_W['uniacid'],
					'join_id' => $one_a['join_id'],
				];
				$list_join = pdo_getall(sl_table_name('card'), $where_join, ['userid']);
				// $list_join = pdo_getall(sl_table_name('card'), $where_join);

				if ($list_join) {

					$ids = sl_array_column($list_join, 'userid');
					$where_update_a = [
						'uniacid' => $_W['uniacid'],
						'userid IN' => $ids,
					];
					// 更新交接时间戳
					$rst_a_1 = pdo_update(sl_table_name('card'), ['join_id'=>$join_timestamp,], $where_update_a);
					$rst_a_2 = pdo_update(sl_table_name('card'), ['enabled'=>2,], $where_a);

					if ($rst_a_1 === FALSE || $rst_a_2 === FALSE) {
						pdo_rollback();
						@putlog('健身房-更新交接人出错-1', $rst_a);
						sl_ajax(1, '更新交接人出错');
					}
				}
			} else {
				...
				// 更新接收人
				$rst_a = pdo_update(sl_table_name('card'), ['join_id'=>$join_timestamp], $where_update_a);

				if ($rst_a === FALSE) {
					pdo_rollback();
					@putlog('健身房-更新接收人出错-2', $rst_a);
					sl_ajax(1, '更新接收人出错');
				}
			}
			...
			if ($one_b) {
				...
				$rst_b_1 = pdo_update(sl_table_name('card'), ['join_id'=>$join_timestamp], $where_update_b);

				// 更新浏览用户
				$rst_b_2 = pdo_update(sl_table_name('myuser'), ['card_id'=>$one_b['id']], $where_update_a);

				if ($rst_b_1 === false) {
					pdo_rollback();
					@putlog('健身房-更新接收人出错', $rst_b_1);
					sl_ajax(1, '更新接收人出错');
				}

				if ($rst_b_2 === false) {
					pdo_rollback();
					@putlog('健身房-更新浏览用户', $rst_b_2);
					sl_ajax(1, '更新接收人出错');
				}

				$data = [
					'user_id'=>$one_a['userid'],
					'handover_id'=>$one_b['userid'],
				];
				$rst_change = card_change_card($data);

				if ($rst_change && $rst_change['errcode']==0) {
					pdo_commit();
					sl_ajax(0, '交接成功');
				} else {
					pdo_commit();
...
} else if ($operation == 'post_switch_ai') {
	$id = intval($_GPC['id']);
	...
	if ($rst['errcode'] == '0') {
		...
		sl_ajax(1, '调整失败-'.$rst['errmsg']);
	}
	exit;


} else if ($operation == 'post_switch_boss') {
	$id = intval($_GPC['id']);
	$s_status = intval($_GPC['status']);
...
} else if ($operation == 'post_switch_enabled') {
	$id = intval($_GPC['id']);
	$s_status = intval($_GPC['status']);
...
} else if ($operation == 'post_switch_isdefault') {
	$id = intval($_GPC['id']);
	$s_status = intval($_GPC['status']);
...
} else if ($operation == 'post_goods_table') {
	$card_id = intval($_GPC['id']);
...
} else if ($operation == 'delete_post_goods') {
...
} else if ($operation == 'delete_good_all') {
	...
	if ($rst !== false) {
		$condition_card = " AND uniacid=:uniacid ";
		$condition_card .= $where;
		$params_card = array(':uniacid' => $_W['uniacid']);
		$list_card = pdo_fetchall('SELECT id FROM ' . tablename('slwl_aicard_card')
			. ' WHERE 1 ' . $condition_card, $params_card);
...
// 我的印象-添加-编辑
	...
	if (empty($card_id)) { sl_ajax(1, '名片ID不存在'); }
...
} else if ($operation == 'delete_impression') {
...
include $this->template('web/content/card');
