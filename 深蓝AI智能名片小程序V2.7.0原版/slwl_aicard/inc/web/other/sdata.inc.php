...
if ($_W['ispost']) {
	...
	if ($sys_users_clear == '1') {
		$rst_users = pdo_delete('slwl_aicard_users', array('uniacid' => $_W['uniacid']));
		if ($rst_users === false) {
			sl_ajax(1, '清除客户数据表失败');
		}
	}
	...
	if ($sys_mycard_clear == '1') {
		$rst_mycard = pdo_delete('slwl_aicard_mycard', array('uniacid' => $_W['uniacid']));
		if ($rst_mycard === false) {
			sl_ajax(1, '清除客户数据表失败');
		}
	}
	...
	if ($sys_formid_clear == '1') {
		$rst_formid = pdo_delete('slwl_aicard_formid', array('uniacid' => $_W['uniacid']));
		if ($rst_formid === false) {
			sl_ajax(1, '清除客户数据表失败');
		}
	}
	...
	if ($sys_oplog_clear == '1') {
		$rst_oplog = pdo_delete('slwl_aicard_oplog', array('uniacid' => $_W['uniacid']));
		if ($rst_oplog === false) {
			sl_ajax(1, '清除客户数据表失败');
		}
	}
