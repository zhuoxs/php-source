...
if ($operation == 'display') {
	...
	$sql = "SELECT * FROM " . tablename('slwl_aicard_adact'). ' WHERE 1 '
	...
	if ($_W['ispost']) {
		...
		if ($id) {
			pdo_update('slwl_aicard_adact', $data, array('id' => $id));
		} else {
			$data['addtime'] = $_W['slwl']['datetime']['now'];
			pdo_insert('slwl_aicard_adact', $data);
			$id = pdo_insertid();
		}
		sl_ajax(0, '保存成功');
	}
	...
	$rst = @pdo_delete('slwl_aicard_adact', $where);
