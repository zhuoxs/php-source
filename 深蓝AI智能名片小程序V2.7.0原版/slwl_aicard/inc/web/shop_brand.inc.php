...
if ($operation == 'display') {
    ...
    $psize = 10;
    $sql = "SELECT * FROM " . tablename('slwl_aicard_shop_brand'). ' WHERE 1 ' . $condition . " ORDER BY displayorder DESC, id DESC LIMIT " . ($pindex - 1) * $psize .',' .$psize;

    $list = pdo_fetchall($sql, $params);
    $total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('slwl_aicard_shop_brand') . ' WHERE 1 ' . $condition, $params);
    ...
    $one = pdo_fetch('SELECT * FROM ' . tablename('slwl_aicard_shop_brand') . ' WHERE 1 ' . $condition, $params);
    ...
    $condition = " AND uniacid=:uniacid AND id=:id ";
    $params = array(':uniacid' => $_W['uniacid'], ':id' => $id);
    $one = pdo_fetch('SELECT id FROM ' . tablename('slwl_aicard_shop_brand') . ' WHERE 1 ' . $condition, $params);
    if (empty($one)) {
    ...
    $total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('slwl_aicard_shop_order_goods') . ' WHERE 1 ' . $condition, $params);
    ...
    $pindex = max(1, intval($_GPC['page']));
    $psize = 10;
    $sql = "SELECT * FROM " . tablename('slwl_aicard_shop_goods'). ' WHERE 1 ' . $condition . " ORDER BY displayorder DESC, id DESC LIMIT " . ($pindex - 1) * $psize .',' .$psize;
    ...
    $one = pdo_fetch('SELECT * FROM ' . tablename('slwl_aicard_shop_goods') . ' WHERE 1 ' . $condition, $params);
    $piclist = array();
    if ($one) {
