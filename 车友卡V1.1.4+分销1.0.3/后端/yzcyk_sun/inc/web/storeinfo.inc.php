...
$info=pdo_get('yzcyk_sun_store',array('id'=>$_GPC['id']));
$area=pdo_getall('yzcyk_sun_area',array('uniacid'=>$_W['uniacid']));
$type=pdo_getall('yzcyk_sun_storetype',array('uniacid'=>$_W['uniacid'],'state'=>1),array(),'','num asc');
$type2=pdo_get('yzcyk_sun_storetype2',array('id'=>$info['storetype2_id']));
//查出已是商家用户
...
//入住类型
//$typein=pdo_getall('yzcyk_sun_storein',array('uniacid'=>$_W['uniacid']));
   $time=24*60*60*7;//一周
...
if(checksubmit('submit')){
		...
		if($_GPC['user_name']){
			$rst=pdo_get('yzcyk_sun_store',array('user_name'=>$_GPC['user_name'],'uniacid'=>$_W['uniacid'],'id !='=>$_GPC['id']));
			if($rst){
