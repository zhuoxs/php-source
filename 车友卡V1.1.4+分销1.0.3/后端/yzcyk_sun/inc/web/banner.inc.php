...
$type = pdo_getall('yzcyk_sun_type', array('uniacid' => $_W['uniacid'], 'state' => 1), array(), '', 'id asc');
$shop_url=array();
foreach($type as $k=>$val){
  $shop_url[$k]['name']=$val['type_name'];
  $shop_url[$k]['value']='yzcyk_sun/pages/shop/shop?tid='.$val['id'];
}
$info=pdo_get('yzcyk_sun_banner',array('uniacid' => $_W['uniacid']));
if($info['lb_imgs']){
...
if(checksubmit('submit')){
    ...
    if($_GPC['id']==''){
        $res=pdo_insert('yzcyk_sun_banner',$data);
        if($res){
        ...
        $res = pdo_update('yzcyk_sun_banner', $data, array('id' => $_GPC['id']));
        if($res){
