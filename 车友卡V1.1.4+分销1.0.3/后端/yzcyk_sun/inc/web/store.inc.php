...
$pagesize=10;
//$sql="SELECT * FROM ".tablename('yzcyk_sun_store_active'). " a"  . " left join " . tablename("yzcyk_sun_in") . " b on b.type=a.time_type".$where." ORDER BY a.id DESC";
$sql="SELECT * FROM ".tablename('yzcyk_sun_store').$where." ORDER BY id DESC";
$total=pdo_fetchcolumn("select count(*) from " .tablename('yzcyk_sun_store').$where,$data);
$select_sql =$sql." LIMIT " .($pageindex - 1) * $pagesize.",".$pagesize;
$list=pdo_fetchall($select_sql,$data);
foreach($list as &$val){
    if($val['storelimit_id']>0){
        $val['lt_day']=pdo_getcolumn('yzcyk_sun_storelimit',array('id'=>$val['storelimit_id']),'lt_day',1);
        $order=pdo_getall('yzcyk_sun_order',array('uniacid'=>$_W['uniacid'],'order_lid'=>4,'openid'=>$val['openid'],'store_id'=>$val['id'],'pay_status'=>1),'',array(),'id desc');
...
if($_GPC['op']=='tg'){
    $store=pdo_get('yzcyk_sun_store',array('id'=>$_GPC['id']));
    if($store['storelimit_id']>0){
        $storelimit=pdo_get('yzcyk_sun_storelimit',array('id'=>$store['storelimit_id']));
        $store_rz_record=pdo_get('yzcyk_sun_store_rz_record',array('uniacid'=>$_W['uniacid'],'openid'=>$store['openid'],'store_id'=>$store['id'],'order_id'=>$store['order_id']));
        if(!$store_rz_record){
...
if($_GPC['op']=='jj'){
    $res=pdo_update('yzcyk_sun_store',array('state'=>3,'rz_time'=>time()),array('id'=>$_GPC['id']));
    if($res){
