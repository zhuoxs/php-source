...
//$sql="SELECT a.*,b.store_name as seller_name FROM ".tablename('yzcyk_sun_order') .  " a"  . " left join " . tablename("yzcyk_sun_store") . " b on a.store_id=b.id".$where." ORDER BY a.time DESC";

$sql="select * from ".tablename('yzcyk_sun_order') .$where." order by id desc";

$data[':uniacid']=$_W['uniacid'];
//$total=pdo_fetchcolumn("SELECT count(*) FROM ".tablename('yzcyk_sun_order') .  " a"  . " left join " . tablename("yzcyk_sun_store") . " b on a.store_id=b.id".$where." ORDER BY a.time DESC",$data);
$total=pdo_fetchcolumn("SELECT count(*) FROM ".tablename('yzcyk_sun_order').$where." ORDER BY id DESC",$data);
$select_sql =$sql." LIMIT " .($pageindex - 1) * $pagesize.",".$pagesize;
$list=pdo_fetchall($select_sql,$data);
foreach($list as &$val){
	$detail=pdo_getall('yzcyk_sun_order_detail',array('order_id'=>$val['id']));
    foreach($detail as $v){
  	   $val['title'].=$v['title'];
    }
    $store_id=pdo_getcolumn('yzcyk_sun_activity',array('id'=>$val['activity_id']),'store_id',1);
    $val['store_name']=pdo_getcolumn('yzcyk_sun_store',array('id'=>$store_id),'store_name',1);
}

$pager = pagination($total, $pageindex, $pagesize);
if($operation=='delete'){
	$res=pdo_delete('yzcyk_sun_order',array('id'=>$_GPC['id']));
   //$res=pdo_update('yzcyk_sun_order',array('is_delete'=>1),array('id'=>$_GPC['id']));
	if($res){
...
if($operation=='delivery'){
   $res=pdo_update('yzcyk_sun_order',array('order_status'=>3,'queren_time'=>time()),array('id'=>$_GPC['id']));
...
if($operation=='receipt'){
   ...
   $res=pdo_update('yzcyk_sun_order',array('state'=>4,'complete_time'=>time()),array('id'=>$_GPC['id']));
	if($res){
...
if($operation=='refund'){
    $id=$_GPC['id'];
    include_once IA_ROOT . '/addons/yzcyk_sun/cert/WxPay.Api.php';
    load()->model('account');
    load()->func('communication');
    $WxPayApi = new WxPayApi();
    $input = new WxPayRefund();
    $path_cert = IA_ROOT . "/addons/yzcyk_sun/cert/".'apiclient_cert_' . $_W['uniacid'] . '.pem';
    $path_key = IA_ROOT . "/addons/yzcyk_sun/cert/".'apiclient_key_' . $_W['uniacid'] . '.pem';
    $account_info = $_W['account'];
    $refund_order =pdo_get('yzcyk_sun_order',array('id'=>$id));  
   ...
   // var_dump($result);die;
    ...
    //更改订单操作
        pdo_update('yzcyk_sun_order',array('status'=>6),array('id'=>$id));           
...
/*if(checksubmit('export_submit', true)) {
    ...
    $time="'%$time%'";
        $count = pdo_fetchcolumn("SELECT COUNT(*) FROM". tablename("yzcyk_sun_order")." WHERE time LIKE ".$time);
        ...
        for ($j = 1; $j <= $pagesize; $j++) {
            $sql = "select a.*,b.store_name as seller_name from " . tablename("yzcyk_sun_order")."  a"  . " inner join " . tablename("yzcyk_sun_store")." b on a.store_id=b.id  WHERE a.time LIKE ".$time."  limit " . ($j - 1) * 5000 . ",5000 ";
