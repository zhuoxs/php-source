...
$info=pdo_get('yzcyk_sun_activity',array('id'=>$_GPC['id']));
if($info['lb_pics']){
...
$category=pdo_getall('yzcyk_sun_activity_category',array('uniacid'=>$_W['uniacid']));
$store=pdo_getall('yzcyk_sun_store',array('uniacid'=>$_W['uniacid'],'state'=>2));
if(checksubmit('submit')){
    ...
    if(empty($_GPC['id'])){
        $res = pdo_insert('yzcyk_sun_activity', $data);
        if($res){
        ...
        unset($data['add_time']);
        $res = pdo_update('yzcyk_sun_activity', $data, array('id' => $_GPC['id']));
        if($res){
