...
$pagesize=10;
//$sql="SELECT * FROM ".tablename('yzcyk_sun_activity_active'). " a"  . " left join " . tablename("yzcyk_sun_in") . " b on b.type=a.time_type".$where." ORDER BY a.id DESC";
$sql="SELECT * FROM ".tablename('yzcyk_sun_activity').$where." ORDER BY id DESC";
$total=pdo_fetchcolumn("select count(*) from " .tablename('yzcyk_sun_activity').$where,$data);
$select_sql =$sql." LIMIT " .($pageindex - 1) * $pagesize.",".$pagesize;
$list=pdo_fetchall($select_sql,$data);
foreach($list as &$val){
    $val['store_name']=pdo_getcolumn('yzcyk_sun_store',array( 'uniacid' => $_W['uniacid'],'id'=>$val['store_id']),'store_name',1);
    $val['category_title']=pdo_getcolumn('yzcyk_sun_activity_category',array( 'uniacid' => $_W['uniacid'],'id'=>$val['category_id']),'title',1);
}
