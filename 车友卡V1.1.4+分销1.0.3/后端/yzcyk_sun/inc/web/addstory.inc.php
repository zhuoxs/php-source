...
$info=pdo_get('yzcyk_sun_story',array('id'=>$_GPC['id']));
$category=pdo_getall('yzcyk_sun_story_category',array('uniacid'=>$_W['uniacid']));
$album=pdo_getall('yzcyk_sun_story_album',array('uniacid'=>$_W['uniacid']));
if(checksubmit('submit')){
    $data['uniacid']=$_W['uniacid'];
    $data['album_id']=$_GPC['album_id'];
    if($_GPC['album_id']>0){
        $album=pdo_get('yzcyk_sun_story_album',array('id'=>$_GPC['album_id']));
    ...
    $data['sort']=$_GPC['sort'];
    if(empty($_GPC['id'])){
        $res = pdo_insert('yzcyk_sun_story', $data);
        if($res){
        ...
        unset($data['add_time']);
        $res = pdo_update('yzcyk_sun_story', $data, array('id' => $_GPC['id']));
        if($res){
